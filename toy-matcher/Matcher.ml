...
let construct_backref_set pat_val =
  ...
  | Cons (atom, pat) ->
    ( match atom with
    | Backref name ->
        Names.add name pat.backrefs
    | Any_symbol | Symbol _ | Ellipsis ->
        pat.backrefs )
...
let update_env_after_seeing_backref k env pat =
  if not (has_backref k pat)
  then { env with min_env = Bindings.remove k env.min_env }
  else env
  ...
  | Cons (atom, pat) ->
      let new_env =
        ...
        | Some name ->
            ...
            then failwith ("multiple atoms have the same name: " ^ name)
            else name :: env
      in
      ( match atom with
      | Any_symbol ->
      ...
      | Ellipsis ->
          ()
      | Backref name ->
          if not (List.mem name env)
          then failwith ("backreference to invalid name: " ^ name) ) ;
...
let init_ellipsis pat init_acc =
  ...
  | None ->
      In_anon_ellipsis
  | Some _name ->
      In_named_ellipsis (pat, init_acc)
...
let extend_ellipsis opt_ellipsis symbol =
  ...
  | In_named_ellipsis (name, acc) ->
      In_named_ellipsis (name, symbol :: acc)
...
let print_ellipsis oc ellipsis =
  ...
  | In_named_ellipsis (pat, acc) ->
      let name =
        match pat.capture_name with Some name -> name | _ -> assert false
      in
...
let trace_match_call ~trace stat ellipsis env pat input =
  ...
  if trace
  then
    if match_calls <= max_trace_lines
    then
      printf
        "match%a%a%a%a%a\n"
        print_ellipsis
        ...
        print_input_head
        input
    else if match_calls = max_trace_lines + 1
    then printf "[exceeded max trace lines = %i]\n%!" max_trace_lines
  ...
  *)
  ...
  let create ?(cache_when = Always) compute =
    ...
    *)
    ...
    let cache_every = match cache_when with Always -> 1 | Optimal -> 3 in
...
let check_match
    ?cache ?min_match_calls ?max_match_calls pat input_str expected_opt_bindings
    =
