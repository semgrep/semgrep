...
import argparse
import json
from util import debug_print, print_error_exit
...
def invoke_sgrep_lint(verbose: bool, strict: bool, test_files: List[Path], config: Path):
...
def generate_file_pairs(
    ...
    for filename in filenames:
        if (
            ...
            # invoke sgrep            
            try:
                output_json = invoke_sgrep_lint(sgrep_verbose, strict, test_files, filename)
                tested.append(
                    (filename, score_output_json(output_json, test_files, ignore_todo))
                )
            except Exception as ex:
                print(f"sgrep error running with config {filename} on {test_files}:\n{ex}")
