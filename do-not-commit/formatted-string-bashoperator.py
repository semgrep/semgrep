...
t1 = BashOperator(task_id="print_date", bash_command="echo " + message, dag=dag)

howlong = requests.get("https://fakeurl.asdf/howlong").text
# ruleid: formatted-string-bashoperator
command = "sleep {}".format(howlong)
t2 = BashOperator(
    task_id="sleep", depends_on_past=False, bash_command=command, retries=3, dag=dag
)
...
# ok
t5 = BashOperator(task_id="safe", bash_command="echo hello world!", dag=dag)

# ruleid: formatted-string-bashoperator
echo_message = f"echo {message}"
t5 = BashOperator(task_id="safe", bash_command=echo_message, dag=dag)
