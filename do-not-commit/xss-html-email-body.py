...
def notify_users_about_challenge(request):
    ...
    if request.user.is_authenticated() and request.user.is_superuser:
        if request.method == "GET":
            ...
            email = EmailMessage(
                subject, body_html, sender, [settings.EMAIL], bcc=users,
            )
            ...
            try:
                ...
                return render(request, "notification_email_data.html", {"errors": 1})
        else:
...
def send_an_email(request):
    ...
    email = EmailMessage(subject, body_html, sender, [settings.EMAIL],)
