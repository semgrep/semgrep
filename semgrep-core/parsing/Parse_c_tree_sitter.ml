...
let rec abstract_declarator (env : env) (x : CST.abstract_declarator)
 : (type_ -> type_) =
  ...
  | `Abst_func_decl (v1, v2) ->
      ...
      let v1 =
        (match v1 with
        | Some x -> let f = abstract_declarator env x in
...
and declaration_specifiers (env : env) ((v1, v2, v3) : CST.declaration_specifiers) : type_ =
  let _v1 =
...
and field_declaration_list_item (env : env) (x : CST.field_declaration_list_item) : field_def list =
  (match x with
...
let anon_choice_decl_f8b0ff3 (env : env) (x : CST.anon_choice_decl_f8b0ff3) =
  ...
  | `Decl x ->
        let (id, f) = declarator env x in
        (fun t ->
            { v_name = id; v_type = f t; v_storage = DefaultStorage;
              v_init = None
            }
        )
  | `Init_decl (v1, v2, v3) ->
...
let type_definition (env : env) ((v1, v2, v3, v4, v5, v6) : CST.type_definition) : type_def list =
