...
and map_resolved_name env (v1, v2) =
  let v1 = map_resolved_name_kind env v1 and v2 = map_sid env v2 in
  (v1, v2)

and map_resolved_name_kind env = function
  | Global -> ()
  | LocalVar -> ()
  | Parameter -> ()
  | EnclosedVar -> ()
...
and map_expr env { e = v_e; e_id = v_e_id; e_range = v_e_range } =
  ...
  let v_e_range =
    map_of_option
      (fun (v1, v2) ->
        let v1 = map_token_location env v1 and v2 = map_token_location env v2 in
        (v1, v2))
      v_e_range
  in
...
and map_expr_kind env ekind =
  ...
  | Container (v1, v2) ->
      ...
      and v2 = map_bracket env (map_of_list (map_expr env)) v2 in
      nothing env (v1, v2)
  | Comprehension (v1, v2) ->
      let v1 = map_container_operator env v1
      and v2 = map_bracket env (map_comprehension env) v2 in
      nothing env (v1, v2)
  | Record v1 ->
...
and map_svalue env = function
  ...
  | Lit v1 ->
      ...
      nothing env v1
  | Cst v1 ->
      let v1 = map_const_type env v1 in
      nothing env v1
  | Sym v1 ->
...
and map_stmt env
  ...
  in
  let v_s_strings =
    (* map_of_option (Set_.map_t env map_of_string) *) v_s_strings
  in
  let v_s_backrefs =
    (* map_of_option (AST_utils.String_set.map_t env) *) v_s_backrefs
  in
...
and map_definition env def =
  ...
  (* Boilerplate *)
  (* ----------- *)
  let v1 = map_entity env v1 and v2 = map_definition_kind env v2 in
  (v1, v2)
