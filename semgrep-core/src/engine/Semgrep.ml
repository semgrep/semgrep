...
let intersect_ranges xs ys =
  ...
  let surviving_xs =
    xs
    |> List.filter (fun x ->
           ys |> List.exists (fun y -> x $<=$ y && inside_compatible x y))
  in
  let surviving_ys =
    ys
    |> List.filter (fun y ->
           xs |> List.exists (fun x -> y $<=$ x && inside_compatible y x))
  in
 ...
 fun env e ->
  ...
  | R.And xs -> (
      ...
      | posr :: posrs ->
          let res = posr in
          let res =
            posrs |> List.fold_left (fun acc r -> intersect_ranges acc r) res
          in
          ...
          let res =
            neg
            |> List.fold_left
                 (fun acc x -> difference_ranges acc (evaluate_formula env x))
                 res
          in
  ...
  |> List.map (fun r ->
         Common.profile_code (spf "real_rule:%s" r.R.id) (fun () ->
             ...
             if not relevant_rule then (
               ...
               in
               let env = { pattern_matches = pattern_matches_per_id; file } in
