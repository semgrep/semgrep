...
and map_template_arguments env v =
  map_angle env (map_of_list (map_template_argument env)) v

and map_template_argument env v =
  map_either env (map_type_ env) (map_expr env) v
...
and map_stmt_or_decl env = function
  | S v1 ->
...
and map_compound env v =
  map_brace env (map_of_list (map_sequencable env (map_stmt_or_decl env))) v

and map_declarations env v =
  map_brace env (map_of_list (map_sequencable env (map_stmt_or_decl env))) v

and map_entity env { name = v_name; specs = v_specs } =
  let v_specs = map_of_list (map_specifier env) v_specs in
  let v_name = map_name env v_name in
  todo env ()

and map_decl env = function
  | DeclList v1 ->
...
and map_initialiser env = function
  | InitExpr v1 ->
      let v1 = map_expr env v1 in
      todo env v1
  | InitList v1 ->
...
and map_func_definition env (v1, v2) =
...
and map_lambda_definition env (v1, v2) =
...
and map_class_definition env (v1, v2) =
...
and map_class_member env = function
  | Access (v1, v2) ->
...
and map_template_parameter env = function
  | TP v1 ->
      let v1 = map_parameter env v1 in
      todo env v1
  | TPClass (v1, v2, v3) ->
...
and map_template_parameters env v =
  map_angle env (map_of_list (map_template_parameter env)) v

and map_specifier env = function
  | A v1 ->
...
and map_cpp_directive env = function
  | Define (v1, v2, v3, v4) ->
      let v1 = map_tok env v1
      and v2 = map_ident env v2
...
and map_sequencable : 'a. env -> ('a -> unit) -> 'a sequencable -> unit =
...
let map_program env v = map_of_list (map_toplevel env) v

let map_any env = function
  | Expr v1 ->
      let v1 = map_expr env v1 in
      todo env v1
  | Stmt v1 ->
