...
let module_name_of_stable_id tk (v1, v2) =
  match v1 with
  | Left id -> G.DottedName (id :: v2)
  | Right _ -> error tk "complex import path not handled"

let rec v_import_expr tk (v1, v2) =
  let v1 = v_stable_id v1 in
  let module_name = module_name_of_stable_id tk v1 in
