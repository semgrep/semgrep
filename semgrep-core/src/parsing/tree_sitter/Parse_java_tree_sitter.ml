...
let module_directive (env : env) ((v1, v2) : CST.module_directive) =
  let _v1 =
    ...
    | `Requis_rep_requis_modi_choice_id (v1, v2, v3) ->
        ...
        let _v2 = Ls.map (requires_modifier env) v2 in
...
let module_body (env : env) ((v1, v2, v3) : CST.module_body) =
  let _v1 = token env v1 (* "{" *) in
  let _v2 = Ls.map (module_directive env) v2 in
  ...
  | `Lambda_exp (v1, v2, v3) ->
      let v1 =
        ...
        | `Infe_params x ->
            ...
            xs |> Ls.map (fun id -> ParamClassic (AST.entity_of_id id))
      in
  ...
  | `Array_crea_exp (v1, v2, v3) ->
      ...
      let exprs, dims, init =
        ...
        | `Rep1_dimens_expr_opt_dimens (v1, v2) ->
            let v1 = Ls.map (dimensions_expr env) v1 in
...
and statement_aux env x : Ast_java.stmt list =
  ...
  | `Try_stmt (v1, v2, v3) ->
      ...
      let v3 =
        match v3 with
        | `Rep1_catch_clause xs -> (Ls.map (catch_clause env) xs, None)
        | `Rep_catch_clause_fina_clause (v1, v2) ->
            let v1 = Ls.map (catch_clause env) v1 in
  ...
  | `Try_with_resous_stmt (v1, v2, v3, v4, v5) ->
      ...
      let v3 = block env v3 in
      let v4 = Ls.map (catch_clause env) v4 in
      let v5 =
...
and switch_block (env : env) ((v1, v2, v3) : CST.switch_block) =
  ...
  let v2 =
    ...
    | `Rep_switch_blk_stmt_group stmt_blocks ->
        Ls.map
          (fun (cases, stmts) ->
            ( Ls.flatten (Ls.map (fun (x, _) -> switch_label env x) cases),
              Ls.map (statement env) stmts ))
          stmt_blocks
...
and enum_body_declarations (env : env) ((v1, v2) : CST.enum_body_declarations) =
  let _v1 = token env v1 (* ";" *) in
  let v2 = Ls.map (fun x -> class_body_decl env x) v2 in
  Ls.flatten v2
...
and constructor_body (env : env) ((v1, v2, v3, v4) : CST.constructor_body) =
  ...
  let v2 =
    ...
    | None -> []
  in
  let v3 = Ls.map (statement env) v3 in
  let v4 = token env v4 (* "}" *) in
  Block (v1, v2 @ v3, v4)
...
and method_header (env : env) ((v1, v2, v3, v4) : CST.method_header) =
  let v1 =
    ...
    | Some (v1, v2) ->
        ...
        let _v2 = Ls.map (annotation env) v2 in
        v1
    | None -> []
  in
  let v2 = unannotated_type env v2 in
...
let program (env : env) (xs : CST.program) = Ls.map (statement env) xs
