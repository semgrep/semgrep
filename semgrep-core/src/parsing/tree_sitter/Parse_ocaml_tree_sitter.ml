...
let _map_conversion_specification (env : env) (tok : CST.conversion_specification) =
...
let _map_rel_operator (env : env) (tok : CST.rel_operator) =
...
let _map_prefix_operator (env : env) (tok : CST.prefix_operator) =
  token env tok (* prefix_operator *)

let _map_concat_operator (env : env) (tok : CST.concat_operator) =
  token env tok (* concat_operator *)

let _map_match_operator (env : env) (tok : CST.match_operator) =
  token env tok (* match_operator *)

let _map_pat_d43393f (env : env) (tok : CST.pat_d43393f) =
  token env tok (* pattern "[^\\\\']" *)

let _map_hash_operator (env : env) (tok : CST.hash_operator) =
...
let _map_pat_60fc52b (env : env) (tok : CST.pat_60fc52b) =
...
let _map_pat_6cdf4be (env : env) (tok : CST.pat_6cdf4be) =
  token env tok (* pattern \\u\{[0-9A-Fa-f]+\} *)

let _map_pat_21333c0 (env : env) (tok : CST.pat_21333c0) =
  token env tok (* pattern \\o[0-3][0-7][0-7] *)

let _map_let_operator (env : env) (tok : CST.let_operator) =
  token env tok (* let_operator *)

let _map_pat_9465c8b (env : env) (tok : CST.pat_9465c8b) =
  token env tok (* pattern \\\n[\t ]* *)

let _map_add_operator (env : env) (tok : CST.add_operator) =
  token env tok (* add_operator *)

let _map_left_quoted_string_delimiter (env : env) (tok : CST.left_quoted_string_delimiter) =
  token env tok (* left_quoted_string_delimiter *)

let _map_pat_86b875b (env : env) (tok : CST.pat_86b875b) =
...
let _map_pat_3d340f6 (env : env) (tok : CST.pat_3d340f6) =
  token env tok (* pattern \s+ *)

let _map_pat_19aaf34 (env : env) (tok : CST.pat_19aaf34) =
  token env tok (* pattern "[^\\\\\"%@]+|%|@" *)

let _map_right_quoted_string_delimiter (env : env) (tok : CST.right_quoted_string_delimiter) =
  token env tok (* right_quoted_string_delimiter *)

let _map_and_operator (env : env) (tok : CST.and_operator) =
  token env tok (* and_operator *)

let _map_pat_3bf11d1 (env : env) (tok : CST.pat_3bf11d1) =
  token env tok (* pattern \\x[0-9A-Fa-f][0-9A-Fa-f] *)


let _map_null (env : env) (tok : CST.null) =
  token env tok (* null *)

let _map_pat_714c625 (env : env) (tok : CST.pat_714c625) =
  token env tok (* pattern [^%@|]+|%|@|\| *)
...
let _map_shebang (env : env) (tok : CST.shebang) =
  token env tok (* pattern #!.* *)
...
let _map_ocamlyacc_value (env : env) (tok : CST.ocamlyacc_value) =
  token env tok (* pattern \$[0-9]+ *)

let _map_pretty_printing_indication (env : env) (tok : CST.pretty_printing_indication) =
  token env tok (* pattern @([\[\], ;.{}?]|\\n|<[0-9]+>) *)
...
and map_binding_pattern_ext (env : env) (x : CST.binding_pattern_ext) =
  (match x with
   ...
   | `Exte x -> let _ = map_extension env x in todo env ()
  )
...
and map_simple_expression_ext (env : env) (x : CST.simple_expression_ext) =
  (match x with
   | `Simple_exp x -> let _ = map_simple_expression env x in todo env ()
   | `Exte x -> let _ = map_extension env x in todo env ()
  )
...
and map_simple_pattern (env : env) (x : CST.simple_pattern) : pattern =
  (match x with
   | `Value_pat x -> let _ = map_value_pattern env x in todo env ()
   | `Signed_cst x -> let _ = map_signed_constant env x in todo env ()
   ...
   | `Local_open_pat (v1, v2, v3) ->
       ...
       in
       todo env (v1, v2, v3)
   | `Pack_pat x -> let _ = map_package_pattern env x in todo env ()
   | `Paren_pat (v1, v2, v3) ->
       let v1 = token env v1 (* "(" *) in
       let v2 = map_pattern_ext env v2 in
       let v3 = token env v3 (* ")" *) in
...
and map_type_ext (env : env) (x : CST.type_ext) =
  (match x with
   | `Type x -> let _ = map_type_ env x in todo env ()
   | `Exte x -> let _ = map_extension env x in todo env ()
  )
