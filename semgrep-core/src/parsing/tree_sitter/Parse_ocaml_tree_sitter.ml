...
let map_infix_operator (env : env) (x : CST.infix_operator) =
  match x with
  | `Hash_op tok -> str env tok (* hash_operator *)
  | `Pow_op x -> map_pow_operator env x
  | `Mult_op x -> map_mult_operator env x
  | `Add_op tok -> str env tok (* add_operator *)
...
let map_anon_choice_inst_var_name_cbd841f (env : env)
    (x : CST.anon_choice_inst_var_name_cbd841f) : ident =
  match x with
...
and map_constructor_declaration (env : env)
  ...
  let v2 =
    ...
    | Some x -> (
        ...
        (* GADTs TODO *)
        | `Simple_typed x ->
            let _t = map_simple_typed env x in
            []
...
and map_external_ (env : env) ((v1, v2, v3, v4, v5, v6, v7) : CST.external_) :
  ...
  let _v2 = map_attribute_opt env v2 in
  let v3 = map_value_name env v3 in
  let typ = map_typed env v4 in
  let _v5 = token env v5 (* "=" *) in
  let v6 = List.map (map_string_ env) v6 in
  let _v7 = List.map (map_item_attribute env) v7 in
  External (v1, v3, typ, v6) |> mki
...
and map_infix_expression (env : env) (x : CST.infix_expression) : expr =
  ...
  | `Choice_exp_add_op_choice_exp (v1, v2, v3) ->
      let v1 = map_expression_ext env v1 in
      let v2 = str env v2 (* add_operator *) in
      let v3 = map_expression_ext env v3 in
      Infix (v1, v2, v3)
...
and map_labeled_argument (env : env) (x : CST.labeled_argument) =
  ...
  | `Label x -> (
      ...
      match x with
      | Left _t, id -> ArgKwd (id, name_of_id id)
      | Right _t, id -> ArgQuestion (id, name_of_id id))
  | `Label_imm_tok_COLON_choice_simple_exp (v1, v2, v3) -> (
      let v1 = map_label env v1 in
      let _v2 = token env v2 (* ":" *) in
      let v3 = map_simple_expression_ext env v3 in
      match v1 with
      | Left _t, id -> ArgKwd (id, v3)
      | Right _t, id -> ArgQuestion (id, v3))
...
and map_let_binding (env : env) ((v1, v2, v3) : CST.let_binding) : let_binding =
  ...
  let v2 =
    ...
    | Some (v1, v2, v3, v4, v5) -> (
        ...
        (* TODO: grannar js is wrong there too, this can not happen *)
        | _ -> raise (PI.Parsing_error v4))
...
and map_module_definition (env : env)
  ...
  let _v2 = map_attribute_opt env v2 in
  let _v3 =
    match v3 with Some tok -> Some (token env tok) (* "rec" *) | None -> None
  in
...
and map_module_type_definition (env : env)
  ...
  let v1 = token env v1 (* "module" *) in
  let v2 = token env v2 (* "type" *) in
  let v3 = map_attribute_opt env v3 in
...
and map_object_expression (env : env)
  ...
  let v3 =
    ...
    | Some (v1, v2, v3, v4) ->
        let v1 = token env v1 (* "(" *) in
        let v2 = map_pattern_ext env v2 in
        let v3 = map_typed_opt env v3 in
        let v4 = token env v4 (* ")" *) in
        todo env (v1, v2, v3, v4)
    | None -> todo env ()
  in
...
and map_parameter (env : env) (x : CST.parameter) =
  ...
  | `Paren_abst_type x ->
      let x = map_parenthesized_abstract_type env x in
      todo env x
...
and map_parameter_ (env : env) (x : CST.parameter_) =
  ...
  | `Choice_TILDE_LPAR_id_opt_typed_opt_EQ_seq_exp_ext_RPAR
      ...
      let v4 = map_typed_opt env v4 in
      let v5 =
        match v5 with
        | Some (v1, v2) ->
            let v1 = token env v1 (* "=" *) in
  ...
  | `Label_imm_tok_COLON_LPAR_pat_ext_opt_typed_EQ_seq_exp_ext_RPAR
      ...
      let v5 = map_typed_opt env v5 in
      let v6 = token env v6 (* "=" *) in
      let v7 = map_sequence_expression_ext env v7 in
...
and map_sequence_expression_ (env : env) (x : CST.sequence_expression_) : expr =
  ...
  | `Seq_exp (v1, v2, v3) ->
      ...
      let v3 =
        match v3 with
        | Some (v1, v2) ->
            let v1 = map_attribute_opt env v1 in
            let v2 = map_sequence_expression_ext env v2 in
            todo env (v1, v2)
        | None -> todo env ()
      in
...
and map_signature_item (env : env) (x : CST.signature_item) =
  ...
  | `Exc_defi x ->
      ...
      todo env x
  | `Module_defi x ->
      let x = map_module_definition env x in
      todo env x
  | `Module_type_defi x ->
      let x = map_module_type_definition env x in
      todo env x
  | `Open_module x ->
      let x = map_open_module env x in
      todo env x
...
and map_simple_expression (env : env) (x : CST.simple_expression) : expr =
  ...
  | `Array_exp x ->
      let x = map_array_expression env x in
      todo env x
...
and map_structure_item (env : env) (x : CST.structure_item) : item =
  match x with
  | `Value_defi x ->
      let x = map_value_definition env x in
      todo env x
  | `Exte x ->
      let x = map_external_ env x in
      todo env x
  | `Type_defi x ->
...
and map_typed (env : env) ((v1, v2) : CST.typed) : type_ =
  let _v1 = token env v1 (* ":" *) in
  let v2 = map_type_ext env v2 in
  v2

and map_typed_opt env v =
  match v with Some x -> Some (map_typed env x) | None -> None
