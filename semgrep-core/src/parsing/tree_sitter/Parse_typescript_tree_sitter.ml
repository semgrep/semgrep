...
(*
   When upgrading the typescript grammar, get rid of this and use
   JS.string_ instead.
*)
...
and primary_type (env : env) (x : CST.primary_type) : type_ =
  ...
  | `Array_type x -> (
      ...
      | `Read_prim_type_LBRACK_RBRACK (v1, v2, v3, v4) ->
          let _v1 = token env v1 (* "readonly" *) in
          (* TODO add to AST *)
...
and export_statement (env : env) (x : CST.export_statement) : stmt list =
  ...
  | `Export_opt_defa_func_sign (v1, v2, v3) ->
      let _v1 = token env v1 (* "export" *) in
      let _v2 =
        match v2 with None -> None | Some x -> Some (token env x)
        (* "default" *)
      in
...
and parameter_name (env : env) ((v1, v2, v3) : CST.parameter_name) :
  ...
  in
  let _v2 =
    match v2 with Some tok -> [ token env tok ] (* "readonly" *) | None -> []
  in
  let v3 =
    match v3 with
    | `Id tok ->
