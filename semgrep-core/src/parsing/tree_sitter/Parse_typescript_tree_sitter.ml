...
let accessibility_modifier (env : env) (x : CST.accessibility_modifier) =
  ...
  | `Prot tok -> (Protected, JS.token env tok)
...
let predefined_type (env : env) (x : CST.predefined_type) =
  ...
  | `Void tok -> JS.identifier env tok
...
let rec anon_choice_type_id_42c0412 (env : env)
    (x : CST.anon_choice_type_id_42c0412) : ident list =
  match x with
...
let anon_choice_rese_id_515394d (env : env)
    (x : CST.anon_choice_rese_id_515394d) : ident =
  match x with
  | `Choice_decl x -> reserved_identifier env x
  | `Id tok -> JS.identifier env tok

(* identifier *)

let anon_choice_type_id_dd17e7d (env : env)
    (x : CST.anon_choice_type_id_dd17e7d) : ident =
  match x with
  | `Id tok -> JS.identifier env tok (* identifier *)
  | `Choice_decl x -> reserved_identifier env x
...
let export_clause (env : env) ((v1, v2, v3, v4) : CST.export_clause) =
  ...
  in
  let _v3 =
    match v3 with Some tok -> Some (JS.token env tok) (* "," *) | None -> None
  in
  let _v4 = JS.token env v4 (* "}" *) in
  v2
...
and jsx_expression_some env x =
  ...
  match eopt with
  | None -> JS.todo_any "jsx_expression_some got a None expr" t1 (Program [])
  | Some e -> (t1, e, t2)
...
and type_arguments (env : env) ((v1, v2, v3, v4, v5) : CST.type_arguments) :
  ...
  in
  let _v4 =
    match v4 with Some tok -> Some (JS.token env tok) (* "," *) | None -> None
  in
  let v5 = JS.token env v5 (* ">" *) in
  (v1, v2 :: v3, v5)
  ...
  let rec aux acc_decorators xs =
    ...
    | x :: xs -> (
        ...
        | `Meth_defi_opt_choice_auto_semi (v1, v2) ->
            ...
            let _v2 =
              match v2 with Some x -> Some (JS.semicolon env x) | None -> None
            in
...
and member_expression (env : env) ((v1, v2, v3) : CST.member_expression) : expr
  ...
  let v2 =
    match v2 with
    | `DOT tok (* "." *) | `QMARKDOT (* "?." *) tok -> JS.token env tok
  in
  let v3 = JS.identifier env v3 (* identifier *) in
...
and subscript_expression (env : env)
  ...
  let v1 = anon_choice_exp_6ded967 env v1 in
  let _v2 =
    match v2 with None -> None | Some tok -> Some (JS.token env tok)
    (* "?." *)
  in
  let v3 = JS.token env v3 (* "[" *) in
...
and primary_expression (env : env) (x : CST.primary_expression) : expr =
  ...
  | `Arrow_func (v1, v2, v3, v4) ->
      ...
      let f =
        { f_attrs = v1; f_params = v2; f_body = v4; f_rettype = tret; f_kind }
      in
      Fun (f, None)
...
and anon_choice_type_id_21dd422 (env : env)
    (x : CST.anon_choice_type_id_21dd422) =
  match x with
...
and for_header (env : env) ((v1, v2, v3, v4, v5, v6) : CST.for_header) =
  ...
  let v4 =
    ...
    | `Of tok -> ForOf (var_or_expr, JS.token env tok, v5)
    (* "of" *)
  in
  v4
...
and expression (env : env) (x : CST.expression) : expr =
  ...
  | `New_exp (v1, v2, v3, v4) ->
      ...
      in
      let t1, xs, t2 =
        match v4 with Some x -> arguments env x | None -> JS.fb []
      in
...
and anon_choice_paren_exp_9c42f0a (env : env)
    (x : CST.anon_choice_paren_exp_9c42f0a) =
  match x with
  | `Paren_exp x -> parenthesized_expression env x
...
and primary_type (env : env) (x : CST.primary_type) : type_ =
  ...
  | `Array_type x -> (
      ...
      | `Read_prim_type_LBRACK_RBRACK (v1, v2, v3, v4) ->
          let _v1 = JS.token env v1 (* "readonly" *) in
          (* TODO add to AST *)
...
and type_query (env : env) ((v1, v2) : CST.type_query) =
  ...
  let v2 =
    match v2 with
    | `Id tok -> JS.identifier env tok (* identifier *) |> idexp_or_special
    | `Nested_id x ->
        nested_identifier env x |> concat_nested_identifier |> idexp_or_special
    | `Gene_type x ->
...
and statement (env : env) (x : CST.statement) : stmt list =
  ...
  | `Throw_stmt (v1, v2, v3) ->
      ...
      let v3 = JS.semicolon env v3 in
      [ Throw (v1, v2, v3) ]
  | `Empty_stmt tok -> [ JS.empty_stmt env tok (* ";" *) ]
  | `Labe_stmt (v1, v2, v3) ->
...
and export_statement (env : env) (x : CST.export_statement) : stmt list =
  ...
  | `Export_opt_defa_func_sign (v1, v2, v3) ->
      let _v1 = JS.token env v1 (* "export" *) in
      let _v2 =
        match v2 with None -> None | Some x -> Some (JS.token env x)
        (* "default" *)
      in
...
and public_field_definition (env : env)
    ((v1, v2, v3, v4, v5, v6) : CST.public_field_definition) =
  let _v1 =
    match v1 with Some x -> [ accessibility_modifier env x ] | None -> []
  in
  let v2 =
  ...
  in
  let v5 =
    match v5 with Some x -> Some (type_annotation env x |> snd) | None -> None
  in
  let v6 = match v6 with Some x -> Some (initializer_ env x) | None -> None in
...
and lexical_declaration (env : env) ((v1, v2, v3, v4) : CST.lexical_declaration)
  ...
  let v1 =
    match v1 with
    | `Let tok -> (Let, JS.token env tok) (* "let" *)
    | `Const tok -> (Const, JS.token env tok)
    (* "const" *)
  in
  let v2 = variable_declarator env v2 in
  let v3 =
...
and todo_function_signature (env : env)
    ((v1, v2, v3, v4, v5) : CST.function_signature) =
  let v1 =
    match v1 with
  ...
  | `Ambi_decl (v1, v2) ->
      ...
      let v2 =
        ...
        | `Module_DOT_id_COLON_type (v1, v2, v3, v4, v5) ->
            ...
            [
              ( basic_entity name,
                VarDef { v_kind = (Const, v1); v_init = None; v_type = Some v5 }
              );
            ]
      in
      v2
