...
let rec type_case (env : env) ((v1, v2, v3, v4, v5) : CST.type_case) =
  ...
  let xs = v2 :: v3 in
  (CaseExprs (v1, xs |> Ls.map (fun x -> Right x)), stmt1 v5)
...
and expression_case (env : env) ((v1, v2, v3, v4) : CST.expression_case) =
  ...
  (CaseExprs (v1, v2 |> Ls.map (fun x -> Left x)), stmt1 v4)
...
and communication_case (env : env) ((v1, v2, v3, v4) : CST.communication_case) =
  ...
  let v2 =
    ...
    | `Rece_stmt x -> (
        ...
        | Some (xs, (_lr, tk)) ->
            CaseAssign (v1, xs |> Ls.map (fun e -> Left e), tk, e))
  in
  let _v3 = token env v3 (* ":" *) in
  let v4 = match v4 with Some x -> statement_list env x | None -> [] in
  (v2, stmt1 v4)
...
let top_level_declaration (env : env) (x : CST.top_level_declaration) :
  ...
  | `Import_decl (v1, v2) ->
      ...
      in
      v2 |> Ls.map (fun (a, b) -> Import { i_tok = v1; i_path = b; i_kind = a })
