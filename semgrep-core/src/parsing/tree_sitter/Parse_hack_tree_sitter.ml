...
let qualified_identifier (env : env) (x : CST.qualified_identifier) : G.name =
  ...
  | `Opt_id_rep1_back_id (v1, v2) ->
      ...
      let ids = List.rev (match v1 with Some v1 -> v1 :: v2 | None -> v2) in
      let ident = List.hd ids in
      (* Q: Bad OCaml? But we know lists can't be empty... *)
...
and anon_choice_field_spec_0e0e023 (env : env)
  ...
  | `Field_spec (v1, v2, v3, v4) ->
      ...
      let def : G.variable_definition =
        {
          vinit = None;
          (* Note: This could never exist so I feel like I'm using the wrong stmt here *)
          vtype = Some v4;
        }
      in
...
and class_const_declaration (env : env)
  ...
  let attrs = v1 @ v2 in
  let v3 = match v3 with Some x -> Some (type_ env x) | None -> None in
...
and declaration (env : env) (x : CST.declaration) =
  ...
  | `Class_decl (v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11) ->
      ...
      let v4 =
        ...
        | Some tok ->
            ...
            [
              G.NamedAttr
                ( token env tok,
                  Id (str env tok, G.empty_id_info ()),
                  G.fake_bracket [] );
            ]
        | None -> []
      in
      ...
      let v10 =
        ...
        match v10 with Some x -> where_clause env x | None -> None
      in
  ...
  | `Inte_decl (v1, v2, v3, v4, v5, v6, v7) ->
      ...
      let v6 =
        match v6 with Some x -> where_clause env x (* TODO *) | None -> None
      in
  ...
  | `Trait_decl (v1, v2, v3, v4, v5, v6, v7) ->
      ...
      let v6 = match v6 with Some x -> where_clause env x | None -> None in
      let v7 = member_declarations env v7 in
      let def : G.class_definition =
        {
          ckind = (G.Trait, v2);
...
and embedded_brace_expression_ (env : env) (x : CST.embedded_brace_expression_)
  ...
  | `Embe_brace_subs_exp (v1, v2, v3, v4) ->
      ...
      let v3 =
        ...
        (* TODO: Should match to other subscript *)
      in
      let v4 = (* "]" *) token env v4 in
...
and function_declaration_header (env : env)
  ...
  in
  let where_clause =
    match v7 with Some x -> where_clause env x | None -> None
  in
  ( {
  ...
  let v2 =
    List.map
      (fun x ->
        match x with
        | `Class_const_decl x ->
            G.FieldStmt (class_const_declaration env x |> G.s)
        ...
        | `Xhp_attr_decl x ->
            G.FieldStmt (xhp_attribute_declaration env x |> G.s)
...
and parameters (env : env) ((v1, v2, v3) : CST.parameters) =
  ...
  let v2 =
    ...
    | Some x -> (
        match x with
        | `Vari_modi tok ->
            (* "..." *) todo env tok (* Added TODO from `token env tok` *)
...
and type_ (env : env) (x : CST.type_) : G.type_ =
  ...
  | `Type_spec (v1, v2, v3) ->
      ...
      let v2 =
        ...
        | `Choice_xhp_id x ->
            G.TyN (Id (xhp_identifier_ env x, G.empty_id_info ()))
      in
      let v3 = match v3 with Some x -> type_arguments env x | None -> None in
      v2
  | `Type_cst (v1, v2) ->
  ...
  | `Shape_type_spec (v1, v2, v3, v4, v5) ->
      ...
      let v4 =
        ...
        | Some (v1, v2, v3) ->
            let v1 =
              G.FieldStmt (anon_choice_field_spec_0e0e023 env v1 |> G.s)
            in
            let v2 =
              List.map
                (fun (v1, v2) ->
                  let v1 = (* "," *) token env v1 in
                  let v2 =
                    G.FieldStmt (anon_choice_field_spec_0e0e023 env v2 |> G.s)
                  in
                  v2)
                v2
            in
...
and variablish (env : env) (x : CST.variablish) =
  ...
  | `Subs_exp (v1, v2, v3, v4) ->
      ...
      let v3 =
        match v3 with Some x -> expression env x | None -> todo env ()
        (* TODO: This creates a new element... *)
      in
      let v4 = (* "]" *) token env v4 in
      G.ArrayAccess (v1, (v2, v3, v4))
  ...
  let v4 =
    ...
    | Some x -> (
        ...
        | `ATre tok ->
            ...
            [
              G.NamedAttr
                (_fk, Id (str env tok, G.empty_id_info ()), G.fake_bracket []);
            ]
        | `ATla tok ->
            (* "@lateinit" *)
            [
