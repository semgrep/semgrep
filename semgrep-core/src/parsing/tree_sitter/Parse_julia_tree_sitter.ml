  ...
  *)
  ...
  let cmd_id =
    ( s2 ^ String.concat "" (List.map fst v3) ^ s4,
      Parse_info.combine_infos t2 (List.map snd v3 @ [ t4 ]) )
  in
  let v5 =
    match v5 with
...
let map_prefixed_string_literal (env : env)
  ...
  let tok = Parse_info.combine_infos t2 (List.map snd v3 @ [ t4 ]) in
...
and map_anon_choice_id_00cc266 (env : env) (x : CST.anon_choice_id_00cc266) =
  ...
  | `Interp_exp x -> Right (map_interpolation_expression env x)
...
and map_anon_choice_id_00cc266_ent ?(attrs = []) ?(tparams = []) (env : env)
  ...
  | `Interp_exp x ->
      { name = EDynamic (map_interpolation_expression env x); attrs; tparams }
...
and map_definition (env : env) (x : CST.definition) : stmt =
  ...
  | `Struct_defi (v1, v2, v3, v4, v5, v6, v7, v8) ->
      ...
      let v6 = map_terminator_opt env v6 in
  ...
  | `Func_defi (v1, v2, v3) -> (
      ...
      | `Choice_func_sign_opt_choice_LF_opt_blk (v1, v2, v3) -> (
          ...
          | `Param_list_rep_where_clause (v1, v2) ->
              ...
              let v2 = List.map (map_where_clause env) v2 in
  ...
  | `Macro_defi (v1, v2, v3, v4, v5, v6, v7) -> (
      ...
      let ent =
        ...
        | `Interp_exp x ->
            ...
            *)
            {
              name = EDynamic (map_interpolation_expression env x);
              attrs = [];
              tparams = [];
            }
      in
      ...
      let macroparams =
         ...
         *)
          ...
          v4
      in
      let v5 = map_terminator_opt env v5 in
  ...
  | `Func_exp (v1, v2, v3) ->
      let fparams =
        ...
        | `Param_list x ->
            ...
            xs
        | `Typed_exp x ->
            let exp, tok, ty = map_typed_expression env x in
...
and map_field_expression (env : env) ((v1, v2, v3) : CST.field_expression) =
  ...
  let field =
    ...
    | `Interp_exp x -> FDynamic (map_interpolation_expression env x)
...
and map_for_binding (env : env) ((v1, v2, v3) : CST.for_binding) =
  let v1 =
    ...
    *)
    ...
    | `Interp_exp x ->
        let exp = map_interpolation_expression env x in
        OtherPat (("pat", fake "pat"), [ G.E exp ])
  in
  let v2 =
    match v2 with
...
and map_function_signature ~body ~func_tok (env : env)
  ...
  let ent =
    let attrs = List.map (map_where_clause env) v6 in
    let tparams =
      match v2 with
      | Some (v1, v2) ->
...
and map_matrix_expression (env : env)
  ...
  let inner =
    ...
    | `Matrix_row_choice_LF_opt_LF (v1, v2, v3) ->
        ...
        let v2 = map_terminator env v2 in
        let v3 = map_newline_opt env v3 in
        [ v1 ]
    | `Matrix_row_rep_choice_LF_opt_LF_matrix_row (v1, v2) ->
        ...
        let v2 =
          List.map
            (fun (v1, v2, v3) ->
              let v1 = map_terminator env v1 in
              let v2 = map_newline_opt env v2 in
  ...
  in
  let _v3 = map_terminator_opt env v3 in
  let v4 = map_newline_opt env v4 in
  ...
  | `Broa_call_exp (v1, v2, v3, v4, v5) ->
      ...
      *)
      ...
      let args =
        ...
        | None -> args
      in
      Call (OtherExpr (v2, []) |> G.e, (l, args, r)) |> G.e
...
and map_statement (env : env) (x : CST.statement) : stmt list =
  ...
  | `Import_stmt (v1, v2) -> (
      ...
      | `Import_list x ->
          ...
          |> List.map (fun dk -> DirectiveStmt (dk |> G.d) |> G.s)
      | `Sele_import x -> (
...
and map_typed_parameter (env : env) ((v1, v2, v3, v4) : CST.typed_parameter) :
    parameter =
  let v2 = (* "::" *) token env v2 in
  ...
  | Some x -> (
      match x with
      | `Id tok ->
          Param (param_of_id ~pattrs ~ptype:(Some v3) (map_identifier env tok))
      | `Tuple_exp x ->
          let pat = map_tuple_pat env x in
          OtherParam (("param", fake "param"), [ G.P pat ])
      | `Interp_exp x ->
          let exp = map_interpolation_expression env x in
          OtherParam (("param", fake "param"), [ G.E exp ]))
  | None ->
...
and map_typed_parameter_classic (env : env)
    ((v1, v2, v3, v4) : CST.typed_parameter) : parameter_classic =
  match map_typed_parameter env (v1, v2, v3, v4) with
