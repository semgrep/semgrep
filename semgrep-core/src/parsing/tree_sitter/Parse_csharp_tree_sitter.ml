...
and variable_declarator (env : env) ((v1, v2, v3) : CST.variable_declarator) =
  ...
  in
  let v3 =
    match v3 with Some x -> Some (equals_value_clause env x) | None -> None
  in
  let vinit =
...
and argument (env : env) ((v1, v2, v3) : CST.argument) : AST.argument =
  let v1 = match v1 with Some x -> Some (name_colon env x) | None -> None in
  let v2 =
    match v2 with
    | Some x -> (
        match x with
...
and initializer_expression (env : env)
    ((v1, v2, v3, v4) : CST.initializer_expression) =
  let v1 = token env v1 (* "{" *) in
...
and expression (env : env) (x : CST.expression) : AST.expr =
  ...
  | `With_exp (v1, v2, v3, v4, v5) -> todo env (v1, v2, v3, v4, v5)
...
and statement (env : env) (x : CST.statement) =
  ...
  | `For_stmt (v1, v2, v3, v4, v5, v6, v7, v8, v9) ->
      ...
      let v5 =
        match v5 with
        (* TODO use `Common.map_opt` function for constructs like this? *)
...
and query_clause (env : env) (x : CST.query_clause) =
  ...
  | `Join_clause (v1, v2, v3, v4, v5, v6, v7, v8, v9, v10) ->
      ...
      let v2 =
        match v2 with Some x -> Some (type_constraint env x) | None -> None
      in
      let v3 = identifier env v3 (* identifier *) in
      ...
      let v10 =
        match v10 with Some x -> Some (join_into_clause env x) | None -> None
      in
...
and attribute_list (env : env)
    ((v1, v2, v3, v4, vtodo, v5) : CST.attribute_list) : attribute list =
  let v1 = token env v1 (* "[" *) in
  let v2 =
    match v2 with
...
and pattern (env : env) (x : CST.pattern) : AST.pattern =
  ...
  | `Var_pat (v1, v2) ->
      ...
      let v2 = variable_designation env v2 in
      v2
  | `Nega_pat (v1, v2) -> todo env (v1, v2)
  | `Paren_pat (v1, v2, v3) -> todo env (v1, v2, v3)
  | `Rela_pat x -> todo env x
...
and recursive_pattern env (v1, v2, v3) =
  let pat =
    ...
    | `Posi_pat_clause_opt_prop_pat_clause (v1, v2) ->
        ...
        let v2 = match v2 with None -> None | Some x -> todo env x in
        v1
...
and property_pattern_clause (env : env)
  ...
  let v1 = token env v1 (* "{" *) in
  let v2 = match v2 with Some x -> todo env x | None -> [] in
  let v4 = token env v4 (* "}" *) in
  todo env (v1, v2, v3, v4)
...
and parameter (env : env) ((v1, v2, v3, v4, v5) : CST.parameter) =
  ...
  *)
  let v1 = List.concat_map (attribute_list env) v1 in
  let v3 =
    match v3 with Some x -> Some (type_constraint env x) | None -> None
  in
  let v4 = identifier env v4 (* identifier *) in
  let v5 =
    match v5 with Some x -> Some (equals_value_clause env x) | None -> None
  in
  {
    pname = Some v4;
    ptype = v3;
...
and enum_declaration env (v1, v2, v3, v4, v5, v6, v7) =
  ...
  let v7 =
    match v7 with Some tok -> Some (token env tok) (* ";" *) | None -> None
  in
  let idinfo = empty_id_info () in
  let ent = { name = EN (Id (v4, idinfo)); attrs = v1 @ v2; tparams = [] } in
...
and record_declaration env x = todo env x
