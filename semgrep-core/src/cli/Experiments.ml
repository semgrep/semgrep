...
let ebnf_to_menhir file =
  ...
  let rec insert_space_when_needed = function
    ...
    | ((T.Ident _|T.String _) as x)::((T.Ident _|T.String _|T.Kwd "(") as y)::xs ->
        x::T.Kwd " "::insert_space_when_needed (y::xs)
  ...
  htokens |> Common.hashset_to_list |> List.iter (fun s ->
    pr (spf "%%token <unit> %s" s)
  );

  let i = ref 0 in
  hkwd |> Common.hashset_to_list |> List.iter (fun s ->
    incr i;
