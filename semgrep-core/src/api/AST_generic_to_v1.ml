...
let rec map_tok v : B.token =
  match v.PI.token with
  | PI.FakeTokStr (s, _) -> `FakeTok s
  | PI.OriginTok loc -> let loc = map_token_location loc in `OriginTok loc
  | _ -> error (Tk v)

and map_wrap:'a. ('a -> 'a) -> 'a wrap -> 'a B.wrap_ = fun _of_a (v1, v2) ->
  let v1 = _of_a v1 and v2 = map_tok v2 in (v1, v2)

and map_bracket:'a. ('a -> 'a) -> 'a bracket -> 'a B.bracket =
  fun of_a (v1, v2, v3) ->
  let v1 = map_tok v1 and v2 = of_a v2 and v3 = map_tok v3 in (v1, v2, v3)

and map_ident (v: ident) : B.ident = map_wrap map_of_string v

and map_dotted_ident v : B.dotted_ident = map_of_list map_ident v
