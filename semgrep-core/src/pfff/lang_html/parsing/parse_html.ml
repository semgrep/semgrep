...
let rec skip_space p_string call_scan =
  let tok =
    if p_string then call_scan Lexer_html.scan_element_after_Eq
  ...
  let rec parse_atts_lookahead next =
    ...
    | T.Name (tok1, n) -> (
        ...
        | T.Relement _ ->
            ...
            ( [
                ( Attr (String.lowercase_ascii n, tok1),
                  Val (String.lowercase_ascii n, Ast.fakeInfo ()) );
              ],
              false )
        | T.Relement_empty _ ->
            (* <tag name> <==> <tag name="name"> *)
        ...
        | next' ->
            ...
            let toks, is_empty = parse_atts_lookahead next' in
            ( ( Attr (String.lowercase_ascii n, tok1),
                Val (String.lowercase_ascii n, Ast.fakeInfo ()) )
              :: toks,
              is_empty ))
...
let model_of ~dtd_hash (Tag (element_name, _tok)) =
  try
    ...
    | m -> m
  with
  | Not_found -> (Dtd.Everywhere, Dtd.Any)
...
let is_possible_subelement ~dtd_hash parent_element parent_excl sub_element =
  ...
  let rec eval m =
    ...
    | Dtd.Block2 -> sub_class =*= Dtd.Block || sub_class =*= Dtd.Essential_block
    | Dtd.Flow ->
        sub_class =*= Dtd.Inline || sub_class =*= Dtd.Block
...
let parse2 file =
  Common.with_open_infile file (fun chan ->
      ...
      let current =
        ref
          {
            name = Tag ("", Ast.fakeInfo ());
            atts = [];
            subs = [];
            excl = StringSet.empty;
          }
      in
      ...
      let unwind_stack sub_name =
        ...
        try
          while
            not
              (is_possible_subelement ~dtd_hash !current.name !current.excl
                 sub_name)
          do
            (* Maybe we are not allowed to end the current element: *)
            let current_class, _ = model_of ~dtd_hash !current.name in
            if current_class =*= Dtd.Essential_block then raise Stack.Empty;
            ...
            current :=
              {
                ...
                subs =
                  Ast.Element
                    (!current.name, !current.atts, List.rev !current.subs)
                  :: grant_parent.subs;
              }
          done
        with
                  ...
                  current :=
                    {
                      ...
                      excl =
                        StringSet.union
                          (StringSet.of_list new_excl)
                          !current.excl;
                    });
                parse_next ())
        | T.Cdata (tok, data) ->
            current :=
              { !current with subs = Ast.Data (data, tok) :: !current.subs };
            parse_next ()
      ...
      let xs =
        try parse_next () with
        (* never returns. Will get a warning X *)
        (* assert false *)
        | End_of_scan ->
