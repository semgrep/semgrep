...
*)
type element_class = Inline | Block | Essential_block | None | Everywhere
...
let html40_dtd =
  [
    ...
    ("textarea", (Inline, Elements []));
    (* #PCDATA *)
    ("label", (Inline, Sub_exclusions ([ "label" ], Inline2)));
    ( "button",
      ( Inline,
        Sub_exclusions
          ( [
    ...
    (* transitional: *)
    ("dir", (Block, Sub_exclusions (block_elements, Elements [ "li" ])));
    ("menu", (Block, Sub_exclusions (block_elements, Elements [ "li" ])));
    (* %preformatted; *)
    ( "pre",
      ( Block,
        Sub_exclusions
    ...
    ("form", (Block, Sub_exclusions ([ "form" ], Flow |. Elements [ "script" ])));
    (* strict DTD has Block here *)
    ("hr", (Block, Empty));
    ( "table",
      ( Block,
