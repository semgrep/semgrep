...
let visit_toplevel ~tag_hook _prefs (toplevel, toks) =
  ...
  let rec visit = function
    | Element (Tag (s_tag, _tok_t), attrs, xs) ->
        attrs
        |> List.iter (fun (Attr (s_attr, _tok_a), Val (_s_val, tok_v)) ->
               match s_attr with
               | "href"
               | "xmlns" ->
                   tag tok_v EmbededUrl
        ...
        | "h1", _ ->
            ...
            |> List.iter (function
                 | Element _ -> ()
                 | Data (_s, tok) -> tag tok CommentSection1)
        | "h2", _ ->
            xs
            |> List.iter (function
                 | Element _ -> ()
                 | Data (_s, tok) -> tag tok CommentSection2)
        | "h3", _ ->
            xs
            |> List.iter (function
                 | Element _ -> ()
                 | Data (_s, tok) -> tag tok CommentSection3)
        | _ -> ());
        xs |> List.iter visit
    | Data _ -> ()
  in
  ...
  |> List.iter (fun tok ->
         ...
         | T.Literal (ii, _s) ->
             (* can be a href *)
             if not (Hashtbl.mem already_tagged ii) then tag ii String
         | T.Other ii -> tag ii NotParsed
         | T.Cdata (_ii, _s)
         | T.CdataSpecial (_ii, _s) ->
