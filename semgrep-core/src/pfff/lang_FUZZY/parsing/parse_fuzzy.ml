...
let parse_and_tokens_with_lang lang file =
  ...
  | Lang_fuzzy.PHP ->
      let toks =
        Parse_php.tokens file |> fun toks ->
        if !Flag_parsing.sgrep_mode then
          Common.exclude Token_helpers_php.is_eof toks
        else toks
      in
      let hooks =
        {
          Lib_ast_fuzzy.tokf = Token_helpers_php.info_of_tok;
  ...
  | Lang_fuzzy.OCaml ->
      let toks =
        Parse_ml.tokens file |> fun toks ->
        if !Flag_parsing.sgrep_mode then
          Common.exclude Token_helpers_ml.is_eof toks
        else toks
      in
      let hooks =
        {
          Lib_ast_fuzzy.tokf = Token_helpers_ml.info_of_tok;
  ...
  | Lang_fuzzy.Skip ->
      let toks =
        ...
        if !Flag_parsing.sgrep_mode then
          Common.exclude Token_helpers_skip.is_eof toks
        else toks
      in
      let hooks =
        {
          Lib_ast_fuzzy.tokf = Token_helpers_skip.info_of_tok;
 ...
 *)
  | Lang_fuzzy.Java ->
      let toks =
        ...
        if !Flag_parsing.sgrep_mode then
          Common.exclude Token_helpers_java.is_eof toks
        else toks
      in
      let hooks =
        {
          Lib_ast_fuzzy.tokf = Token_helpers_java.info_of_tok;
  ...
  | Lang_fuzzy.Javascript ->
      let toks =
        Parse_js.tokens file |> fun toks ->
        if !Flag_parsing.sgrep_mode then
          Common.exclude Token_helpers_js.is_eof toks
        else toks
      in
      let hooks =
        {
          Lib_ast_fuzzy.tokf = Token_helpers_js.info_of_tok;
  ...
  | Lang_fuzzy.Go ->
      let toks =
        Parse_go.tokens file |> fun toks ->
        if !Flag_parsing.sgrep_mode then
          Common.exclude Token_helpers_go.is_eof toks
        else toks
      in
      let hooks =
        {
          Lib_ast_fuzzy.tokf = Token_helpers_go.info_of_tok;
...
let parse_with_lang lang file = parse_and_tokens_with_lang lang file |> fst
