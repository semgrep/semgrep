...
let test_tokens_css file =
  if not (file =~ ".*\\.css") then pr2 "warning: seems not a css file";
  (*
...
let actions () =
  [
    ("-tokens_css", "   <file>", Common.mk_action_1_arg test_tokens_css);
    ("-parse_css", "   <files or dirs>", Common.mk_action_n_arg test_parse_css);
    ("-dump_css", "   <file>", Common.mk_action_1_arg test_dump_css);
  ]
