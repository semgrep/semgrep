...
let tokens a = Common.profile_code "Parse_css.tokens" (fun () -> tokens2 a)
...
let parse2 filename =
  ...
  | Parsing.Parse_error ->
      ...
      if not !Flag.error_recovery then
        raise (Parse_info.Parsing_error (TH.info_of_tok current));

      if !Flag.show_parsing_error then
        pr2 ("parse error \n = " ^ error_msg_tok current);
      ([], toks)

let parse a = Common.profile_code "Parse_css.parse" (fun () -> parse2 a)
