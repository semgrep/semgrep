...
let visit_toplevel ~tag_hook _prefs (_toplevel, toks) =
  ...
  let rec aux_toks xs =
    ...
    | T.TComment ii
      ...
      :: xs ->
        ...
        | _ -> ());
        aux_toks
          (T.TComment ii3 :: T.TCommentNewline ii4 :: T.TComment ii5 :: xs)
    ...
    | T.TOParen ii1 :: T.TIdent (s2, _ii2) :: T.TIdent (_s3, ii3) :: xs
      when PI.col_of_info ii1 = 0 ->
        ...
        | "defun" -> tag ii3 (Entity (Function, Def2 NoUse))
        | "defconst" -> tag ii3 (Entity (Constant, Def2 NoUse))
        | _ -> ());
        aux_toks xs
    (* scheme/racket *)
    | T.TOParen ii1
  ...
  |> List.iter (fun tok ->
         ...
         | T.TNumber (_s, ii) -> tag ii Number
         | T.TIdent (s, ii) -> (
             match s with
             (* lisp *)
             | "defun" -> tag ii Keyword
             | "setq"
             | "defvar" ->
                 tag ii KeywordObject (* hmm not really *)
             (* scheme *)
             | "define" -> tag ii Keyword
             | "let"
             | "let*" ->
                 tag ii KeywordObject
             | "defconst" -> tag ii KeywordObject
             | "require"
             | "provide"
             | "module" (* racket *) ->
                 tag ii KeywordModule
             | "t" -> tag ii Boolean
             | "nil" -> tag ii Boolean (* or Null ? *)
             | "cond"
             | "if" ->
