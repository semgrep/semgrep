...
let of_list_pos chars : char_class =
  List.map snd chars |> of_list

(*
   Each byte of the list becomes a unicode code point in the set.
   Use with ascii strings only.
*)
let of_string s : char_class =
  of_list (AST.code_points_of_ascii_string s)
...
let posix_cntrl =
  Union (
    Range (Char.code '\x00', Char.code '\x1F'),
    Singleton (Char.code '\x7F')
  )
...
let unicode_horizontal_whitespace = of_list [
...
let unicode_vertical_whitespace = of_list [
  0x000A; (* Linefeed (LF) *)
...
let unicode_digit =
  Abstract (Unicode_character_property "Nd")
