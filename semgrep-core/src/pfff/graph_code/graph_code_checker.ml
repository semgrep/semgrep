  ...
  |> G.iter_nodes (fun n ->
         ...
         |> Option.iter (fun info ->
                ...
                (if ps = [] then
                 ...
                 | _ -> Error.warning_loc info.G.pos (Error.Deadcode n));
                ...
                in
                n_def_opt
                |> Option.iter (fun n_def ->
                       let n_decl = n in
                       if not (G.has_node n_def g) then
                          ...
                          *)
                         if G.has_node (fst n_def, E.Function) g then ()
                         else
                           Error.warning_loc info.G.pos
                             (Error.UndefinedDefOfDecl n_decl));
                ...
                in
                n_decl_opt
                |> Option.iter (fun n_decl ->
                       let n_def = n in
                       if G.has_node n_decl g then (
                         ...
                         if
                           users_outside = [] && ps <> []
                           && is_header_file file_decl
                         then
                           Error.warning_loc info_decl.G.pos
