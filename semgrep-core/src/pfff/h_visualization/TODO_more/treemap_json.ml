...
let rec treemap_of_json j =
  ...
  | J.Object
      ...
      let rect = { label = s; color = Color.black; size } in
  ...
  | J.Object
      ...
      let rect = { label = lbl; color = Color.color_of_string scolor; size } in
 ...
 *)
let rec (json_of_treemap : ('a, 'b) Treemap.treemap -> J.json_type) = function
  | Common2.Node ((rect, _a), xs) ->
      let { size = _v_sizeTODO; color = _v_colorTODO; label = v_label } =
        rect
      in

      let bnds = [] in

      let children = J.Array (List.map json_of_treemap xs) in
...
let test_json_of dir =
  let maxc = 256 in
  let tree =
    tree_of_dirs_or_files ~file_hook:(fun file -> Common2.filesize file) [ dir ]
  in
  let treemap =
    treemap_of_tree
...
let actions () =
  [
    ...
    ("-test_json_of", "<dir>", Common.mk_action_1_arg test_json_of);
    ("-test_of_json", "<file>", Common.mk_action_1_arg test_of_json)
