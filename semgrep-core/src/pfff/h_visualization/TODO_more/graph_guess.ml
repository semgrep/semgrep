  ...
  Common.with_open_outfile output (fun (pr_no_nl, _chan) ->
      ...
      let node_name_of_n n =
        ...
        | "ml" ->
            ...
            if str = "Math" then "Math_xxx" else str
        | "mli" -> b ^ "." ^ e
        | "NOEXT" ->
            (* can be directory like external/foo *)
            s ^ "/"
        | _ -> failwith (spf "PB: weird node: %s" s)
      in
      ...
      pr
        (spf
           "nodedef> name, dir1 varchar(200), dir2 varchar(200), dir \
            varchar(200)");
      nodes
      |> List.iter (fun n ->
      ...
      |> List.iter (fun n1 ->
             ...
             |> List.iter (fun n2 ->
                    pr
                      (spf "%s,%s,true" (node_name_of_n n1) (node_name_of_n n2))));
      ())
