...
and treemap_data = { size : int; color : Simple_color.color; label : string }

(*e: type treemap *)
(* with tarzan *)

(*s: type algorithm *)
type algorithm = Classic | Squarified | SquarifiedNoSort | Ordered of pivot
and pivot = PivotBySize | PivotByMiddle
(*e: type algorithm *)
 ...
 *)
let rect_ortho () = { p = { x = 0.0; y = 0.0 }; q = { x = !xy_ratio; y = 1.0 } }
...
let treemap_of_tree2 ~size_of_leaf ~color_of_leaf ?(label_of_file = fun _ -> "")
    ?(label_of_dir = fun _ -> "") tree =
  let rec aux tree =
    match tree with
    | Node (nodeinfo, xs) ->
        let sizeme = ref 0 in
...
let (slice_and_dicing_layout : ('a, 'b) layout_func) =
 fun children depth rect ->
  ...
  children
  |> List.map (fun (size, child) ->
         q.(axis_split) <- p.(axis_split) +. (size /. stotal *. width);
...
let dim_rect_orig = { p = { x = 0.0; y = 0.0 }; q = { x = 6.0; y = 4.0 } }
...
let worst elems_in_row size_side_row =
  ...
  (* cf formula in paper *)
  max
    (Common2.square size_side_row *. rplus /. Common2.square s)
    (Common2.square s /. (Common2.square size_side_row *. rminus))

let _ = assert (worst [ 6.0 ] 4.0 = 8.0 /. 3.0) (* 2.66667 *)
let _ = assert (worst [ 6.0; 6.0 ] 4.0 = 3.0 /. 2.0)
(* 1.5, which is close to 1 so better *)

let _ = assert (worst [ 6.0; 6.0; 4.0 ] 4.0 = 4.0)
...
let layout row rect =
  ...
  let children =
    children
    |> List.map (fun (size, info) ->
           (size /. stotal (* percentage *), size, info))
  in

  let res = ref [] in
  let spread =
    if rect_width rect >= rect_height rect then SpreadHorizontally
  ...
  children
  |> List.iter (fun (percent_child, size_child, info) ->
         q.(axis_split) <- p.(axis_split) +. (percent_child *. width);
         let rect_here =
           { p = { x = p.(0); y = p.(1) }; q = { x = q.(0); y = q.(1) } }
         in
...
let rec (squarify_orig :
 ...
 fun ?(verbose = false) children current_row rect ->
  ...
  let spread =
    if rect_width rect >= rect_height rect (* e.g. 6 x 4 rectangle *) then
      SpreadHorizontally
    else SpreadVertically
  in
  ...
  | c :: cs ->
      ...
      then
        ...
        let row_rect, remaining_rect =
          match spread with
          | SpreadHorizontally ->
              let middle_x = ((q.x -. p.x) *. portion_for_row) +. p.x in
              ( { p; q = { x = middle_x; y = q.y } },
                { p = { x = middle_x; y = p.y }; q } )
          | SpreadVertically ->
              let middle_y = ((q.y -. p.y) *. portion_for_row) +. p.y in
              ( { p; q = { x = q.x; y = middle_y } },
                { p = { x = p.x; y = middle_y }; q } )
        in
...
let squarify children rect =
  ...
  let children' =
    children |> List.map (fun (x, info) -> (x /. total *. area, info))
  in
...
let (squarify_layout_no_sort_size : ('a, 'b) layout_func) =
 fun children _depth rect -> squarify children rect
(*e: layout squarify *)
...
let children_ex_ordered_2001 =
  [ 1; 5; 3; 4; 5; 1; 10; 1; 1; 2; 7; 3; 5; 2; 10; 1; 2; 1; 1; 2 ]
(*e: ordered examples *)
...
let compute_rects_pivotized childs_pivotized rect spread =
  ...
  | SpreadHorizontally ->
      ...
      let middle_x1 = p.x +. (rect_width rect *. portion_for_left) in
      let middle_x2 = q.x -. (rect_width rect *. portion_for_right) in
      let middle_y = p.y +. (rect_height rect *. portion_for_pivot_vs_above) in
      {
        left = { p; q = { x = middle_x1; y = q.y } };
        right = { p = { x = middle_x2; y = p.y }; q };
        pivot =
          {
            p = { x = middle_x1; y = p.y };
  ...
  | SpreadVertically ->
      ...
      let middle_y1 = p.y +. (rect_height rect *. portion_for_left) in
      let middle_y2 = q.y -. (rect_height rect *. portion_for_right) in

      let middle_x = p.x +. (rect_width rect *. portion_for_pivot_vs_above) in
      {
        left = { p; q = { x = q.x; y = middle_y1 } };
        right = { p = { x = p.x; y = middle_y2 }; q };
        pivot =
          {
            p = { x = p.x; y = middle_y1 };
    ...
    | _x :: _y :: _ys ->
        let left, pivot, right =
          ...
          | PivotBySize ->
              ...
              Common2.split_when (fun x -> fst x = pivot_max) xs
          | PivotByMiddle ->
        ...
        let scores_and_rects =
          right_combinations
          |> List.map (fun (above_pivot, right) ->
                 let childs_pivotized =
                   { left; pivot = [ pivot ]; right; above_pivot }
                 in
                 let rects =
                   compute_rects_pivotized childs_pivotized rect spread
                 in
                 ( ratio_rect_dim
...
let render_treemap_algo2 ?(algo = Classic) ?(big_borders = false) treemap =
  ...
  let rec aux_treemap root rect ~depth =
    ...
    if not (valid_rect rect) then () (* TODO ? warning ? *)
    else
      match root with
      ...
      | Node (mode, children) ->
          ...
          let border =
            if not big_borders then
              ...
              | _ -> 0.0002
          in
          let p = { x = p.x +. border; y = p.y +. border } in
          let q = { x = q.x -. border; y = q.y -. border } in
          ...
          let children' =
            children
            |> List.map (fun child ->
                   (float_of_int (size_of_treemap_node child), child))
          in
...
(* specialized version *)
let tree_of_dir3 ?(filter_file = fun _ -> true) ?(filter_dir = fun _ -> true)
    ?(sort = SortDirAndFilesCaseInsensitive) ~file_hook dir =
  if sort <> SortDirAndFilesCaseInsensitive then
    failwith "Only SortDirAndFilesCaseInsensitive is handled";

  let rec aux dir =
    let children = Sys.readdir dir in
    let children =
      Array.map (fun x -> (String.lowercase_ascii x, x)) children
    in
...
let add_intermediate_nodes root_path nodes =
  ...
  let rec aux current_root xs =
    ...
    in
    let leaves = files_here |> List.map (fun ((_dir, _base), node) -> node) in
    nodes @ leaves
  in
  aux root_path xs
...
let rec remove_singleton_subdirs tree =
  ...
  | Node (x, ys) -> Node (x, List.map remove_singleton_subdirs ys)
...
let treemap_rectangles_ex =
  [
    ( [ (0.0, 0.0) ],
      [ (1.0, 1.0) ],
      (0.17778372236496054, 0.75183542244426871, 0.77892130219255096) );
    ( [ (0.0, 0.0) ],
      [ (0.27659574468085107, 1.0) ],
      (0.54757582213226441, 0.945582381819014, 0.26427761420055917) );
    ( [ (0.0, 0.0) ],
      [ (0.27659574468085107, 0.38461538461538464) ],
      (0.71931501307446211, 0.95905644995588246, 0.28633110533256656) );
    ( [ (0.0, 0.38461538461538464) ],
      [ (0.27659574468085107, 1.0) ],
      (0.29508972521695809, 0.35521829137775873, 0.46070336222733932) );
    ( [ (0.0, 0.38461538461538464) ],
      [ (0.10372340425531915, 1.0) ],
      (0.51529552034735771, 0.53725734991812635, 0.22430742368105949) );
    ( [ (0.10372340425531915, 0.38461538461538464) ],
      [ (0.27659574468085107, 1.0) ],
      (0.43861905319415506, 0.16281118710897469, 0.60250203640050937) );
    ( [ (0.27659574468085107, 0.0) ],
      [ (0.36170212765957449, 1.0) ],
      (0.3743827201120038, 0.07170428778373239, 0.09006244270341246) );
    ( [ (0.36170212765957449, 0.0) ],
      [ (0.8936170212765957, 1.0) ],
      (0.39117531981521536, 0.16579633978705666, 0.63690597944460248) );
    ( [ (0.36170212765957449, 0.0) ],
      [ (0.8936170212765957, 0.20000000000000001) ],
      (0.34982099039431447, 0.54618822154424429, 0.19282777912183513) );
    ( [ (0.36170212765957449, 0.20000000000000001) ],
      [ (0.8936170212765957, 0.28000000000000003) ],
      (0.14570785913376116, 0.88033416430670342, 0.51911403487550056) );
    ( [ (0.36170212765957449, 0.28000000000000003) ],
      [ (0.8936170212765957, 0.76000000000000001) ],
      (0.79691567717907263, 0.3307536109585284, 0.95607296382731199) );
    ( [ (0.36170212765957449, 0.28000000000000003) ],
      [ (0.45035460992907805, 0.76000000000000001) ],
      (0.7038680786604008, 0.12714028216462059, 0.17131117338368551) );
    ( [ (0.45035460992907805, 0.28000000000000003) ],
      [ (0.58333333333333337, 0.76000000000000001) ],
      (0.036414279679915174, 0.94100891978030599, 0.017007582879843386) );
    ( [ (0.58333333333333337, 0.28000000000000003) ],
      [ (0.8936170212765957, 0.76000000000000001) ],
      (0.63659306932350279, 0.25303150185397794, 0.81066700006123815) );
    ( [ (0.58333333333333337, 0.28000000000000003) ],
      [ (0.8936170212765957, 0.48571428571428577) ],
      (0.38368601825375115, 0.083946154840038423, 0.048274714595522017) );
    ( [ (0.58333333333333337, 0.48571428571428577) ],
      [ (0.8936170212765957, 0.62285714285714289) ],
      (0.70513207607633877, 0.95785105976069096, 0.87735329563400943) );
    ( [ (0.58333333333333337, 0.62285714285714289) ],
      [ (0.8936170212765957, 0.76000000000000001) ],
      (0.80565735169264896, 0.75578523763882166, 0.10757369310766951) );
    ( [ (0.36170212765957449, 0.76000000000000001) ],
      [ (0.8936170212765957, 1.0) ],
      (0.57042872206220896, 0.9335301149492965, 0.86254084187238389) );
    ( [ (0.36170212765957449, 0.76000000000000001) ],
      [ (0.62765957446808507, 1.0) ],
      (0.31530318311042171, 0.97066142447913661, 0.93180609525183578) );
    ( [ (0.62765957446808507, 0.76000000000000001) ],
      [ (0.8936170212765957, 1.0) ],
      (0.18330061581424317, 0.82234170300788867, 0.38303955663618716) );
    ( [ (0.8936170212765957, 0.0) ],
      [ (1.0, 1.0) ],
      (0.20641218447120302, 0.35715481613716149, 0.86620796882602547) );
    ( [ (0.8936170212765957, 0.0) ],
      [ (1.0, 0.59999999999999998) ],
      (0.7942020522649591, 0.27351921049542915, 0.86191731793444748) );
    ( [ (0.8936170212765957, 0.59999999999999998) ],
      [ (1.0, 1.0) ],
      (0.27214488578650742, 0.41635201268319189, 0.1301335726270938) );
  ]
  ...
  Node
    ( ninfo,
      [
        Leaf 12;
        Leaf 6;
        Node (ninfo, [ Leaf 2; Leaf 2; Leaf 2; Leaf 2; Leaf 2 ]);
        Node
          ( ninfo,
            [
              Node (ninfo, [ Leaf 5; Leaf 20 ]);
              Node (ninfo, [ Leaf 5 ]);
              Leaf 40;
            ] );
      ] )
...
let tree_ex_wijk_1999 =
  let ninfo = () in
  Node (ninfo, [ Leaf 6; Leaf 6; Leaf 4; Leaf 3; Leaf 2; Leaf 2; Leaf 1 ])
...
let (treemap_ex_ordered_2001 : (unit, unit) treemap) =
  ...
  Node
    ( ({ size = total_size; color = Color.black; label = "" }, ()),
      children_treemap )
