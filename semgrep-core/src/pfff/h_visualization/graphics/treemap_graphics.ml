...
let current_dim ~w_legend ~h_status =
  ...
  { w; h; w_view; h_view; h_status; w_legend }
...
let display_treemap_generic ?(drawing_file_hook = fun _rect _file _mat -> ())
    (treemap : ('dir, 'file) treemap) (w, h) flayout =
  let mat = Array.make_matrix w h None in
  ...
  let rec aux_treemap root rect ~depth =
    ...
    if not (valid_rect rect) then () (* TODO ? warning ? *)
      ...
      | Leaf (tnode, fileinfo) ->
          ...
          let rect_opt =
            draw_rect_treemap_float_ortho ((p.x, p.y), (q.x, q.y)) color (w, h)
          in
...
let display_treemap_algo ?(algo = Classic) ?drawing_file_hook treemap (w, h) =
  (* old: display_treemap            treemap (w, h) *)
  let layoutf = layoutf_of_algo algo in
  display_treemap_generic ?drawing_file_hook treemap (w, h) layoutf
(*e: function display_treemap_algo *)

(*s: function display_treemap_interactive *)
let display_treemap_interactive ?algo ?drawing_file_hook
    ?(info_of_file_under_cursor = fun _ _ -> "") treemap dim =
  let dim = ref dim in
  let matrix_info =
    ref
      (display_treemap_algo ?algo ?drawing_file_hook treemap
  ...
  while true do
    ...
    if x >= 0 && y >= 0 && x < !dim.w_view && y < !dim.h_view then (
...
let info_of_file_under_cursor_default status (f, _) =
  let s = f in
  if status.Graphics.button then pr2 (spf "%s" f)
    (* Sys.command (spf "/home/pad/packages/Linux/bin/emacsclient -n %s" f) +> ignore; *);
  if status.Graphics.keypressed (* Graphics.key_pressed () *) then
    raise (UnixExit 0);
  s
...
let test_treemap_manual () =
  ...
  treemap_rectangles_ex
  |> List.iter (fun (upper, lower, (r, g, b)) ->
...
let test_treemap_dir dir algo =
  ...
  Graphics.set_color (Graphics.rgb 1 1 1);
  let w_view, h_view = (Graphics.size_x (), Graphics.size_y () - h_status) in
  let w, h = (Graphics.size_x (), Graphics.size_y ()) in

  let maxc = 256 in
  let dim = { w; h; w_view; h_view; h_status; w_legend = 10 } in

  (* work ? Graphics.set_line_width 2; *)
  let tree =
    tree_of_dir ~file_hook:(fun file -> (file, Common.filesize file)) dir
  in
  ...
  display_treemap_interactive ~algo treemap dim
    ~info_of_file_under_cursor:(fun status (f, size) ->
      let s = f in
      if status.Graphics.button then (
        pr2 (spf "%s" f);
        Sys.command (spf "/home/pad/packages/Linux/bin/emacsclient -n %s" f)
        |> ignore);
...
let actions () =
  [
    ...
    ("-test_treemap_manual", "<>", Common.mk_action_0_arg test_treemap_manual);
    ( "-test_treemap",
      "<algorithm>",
