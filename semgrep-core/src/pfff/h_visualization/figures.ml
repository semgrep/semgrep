...
let rect_area rect = rect_width rect *. rect_height rect
let valid_rect r = r.q.x >= r.p.x && r.q.y >= r.p.y
let s_of_rectangle r = spf "p:%f, %f;  q:%f, %f" r.p.x r.p.y r.q.x r.q.y

let point_is_in_rectangle p r =
  p.x >= r.p.x && p.x <= r.q.x && p.y >= r.p.y && p.y <= r.q.y

let intersection_rectangles r1 r2 =
  let r =
    {
      p = { x = max r1.p.x r2.p.x; y = max r1.p.y r2.p.y };
      q = { x = min r1.q.x r2.q.x; y = min r1.q.y r2.q.y };
    }
  in
