...
let main_action _file = raise Todo
...
let test_matcher_all () =
  ...
  all_types
  |> List.iter (fun t ->
         try test_matcher_gen t with
         | Todo -> pr2 ("PB TODO with : " ^ t)
         | Not_found -> pr2 ("PB Not_found with : " ^ t));
  ()

let ffi_extra_actions () =
  [
    ("-matcher_gen", "<type>", Common.mk_action_1_arg test_matcher_gen);
    ("-matcher_gen_all", "", Common.mk_action_0_arg test_matcher_all);
  ]
...
let all_actions () = ffi_extra_actions () @ []
