...
let parenize_and_comma xs =
  if null xs then "" else "(" ^ Common.join ", " xs ^ ")"

let rec call_to_matcher t =
  match t with
  | O.Var s -> spf "m_%s" s
  | O.Apply (s, t) -> spf "(m_%s " s ^ call_to_matcher t ^ ")"
  | O.List t ->
      let s = "list" in
...
let (gen_matcher : string * OCaml.t -> unit) =
 fun (s, t) ->
  (* pr2 (OCaml.string_sexp_of_t t); *)
  ...
  let aux t =
    ...
    | O.Sum xs ->
        ...
        |> List.iter (fun (s, args) ->
               let aa =
                 Common.index_list_1 args
                 |> List.map (fun (t, i) -> (spf "a%d" i, t))
               in
               let bb =
                 Common.index_list_1 args
                 |> List.map (fun (t, i) -> (spf "b%d" i, t))
               in
               ...
               if not (null args) then
                 e.p (Common2.repeat ")" (List.length args) |> Common.join ""));
    ...
    | O.Tuple args ->
        let aa =
          Common.index_list_1 args |> List.map (fun (t, i) -> (spf "a%d" i, t))
        in
        let bb =
          Common.index_list_1 args |> List.map (fun (t, i) -> (spf "b%d" i, t))
        in
        ...
        |> List.iter (fun ((aa1, t), (aa2, _t2)) ->
               e.p
                 (spf "%s %s %s >>= (fun (%s, %s) -> " (call_to_matcher t) aa1
                    aa2 aa1 aa2));
        ...
        if not (null args) then
          e.p (Common2.repeat ")" (List.length args) |> Common.join "")
    | O.Var s -> e += "(a, b) -> %s a b" $ call_to_matcher (O.Var s)
    | O.List x -> e += "(a, b) -> %s a b" $ call_to_matcher (O.List x)
    | O.Apply (x, y) ->
        e += "(a, b) -> %s a b" $ call_to_matcher (O.Apply (x, y))
    | O.Dict xs ->
        let aa =
          Common.index_list_1 xs |> List.map (fun (t, i) -> (spf "a%d" i, t))
        in
        let bb =
          Common.index_list_1 xs |> List.map (fun (t, i) -> (spf "b%d" i, t))
        in

        e += "{ A. ";
        aa |> List.iter (fun (aa1, (fld, _, _t)) -> e += "%s = %s;" $ fld $ aa1);
        e += "},";
        e += "{ B. ";
        bb |> List.iter (fun (bb1, (fld, _, _t)) -> e += "%s = %s;" $ fld $ bb1);
        e += "} -> ";

        ( e --> fun e ->
          (* ... *)
          Common2.zip aa bb
          |> List.iter (fun ((aa1, (_fld, _, t)), (aa2, _)) ->
                 e.p
                   (spf "%s %s %s >>= (fun (%s, %s) -> " (call_to_matcher t) aa1
                      aa2 aa1 aa2));
          e += "return (";
          ( e --> fun e ->
            e += "{ A. ";
            aa
            |> List.iter (fun (aa1, (fld, _, _t)) ->
                   e += "%s = %s;" $ fld $ aa1);
            e += "},";
            e += "{ B.";
            bb
            |> List.iter (fun (bb1, (fld, _, _t)) ->
                   e += "%s = %s;" $ fld $ bb1);
            e += "} " );
          e += ")" );
