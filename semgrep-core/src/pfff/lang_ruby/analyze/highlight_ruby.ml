...
let visit_program ~tag_hook _prefs (_program, toks) =
  ...
  |> List.iter (fun tok ->
         ...
         | T.T_NUM (_, ii) ->
             tag ii Number
         | T.K_NIL ii -> tag ii Null
         | T.T_ATOM (ii1, (_, ii2)) ->
             tag ii1 Atom;
             tag ii2 Atom
         | T.T_ATOM_BEG ii -> tag ii Atom
         ...
         | T.T_USER_BEG (_, ii) ->
             tag ii String
         | T.T_REGEXP_BEG ii -> tag ii Regexp
         | T.T_REGEXP_MOD (_, ii) -> tag ii Regexp
         (* ident  *)
         | T.T_LID (s, ii)
         | T.T_UID (s, ii) ->
             tag_name (s, ii) Normal
         | T.T_CLASS_VAR (_, ii) -> tag ii (Entity (E.Global, use2))
         | T.T_INST_VAR (_, ii) -> tag ii (Entity (E.Field, use2))
         | T.T_GLOBAL_VAR (_, ii) -> tag ii (Entity (E.Global, use2))
         (* keywords  *)
         | T.K_DEF ii
         | T.K_ALIAS ii ->
             tag ii Keyword
         ...
         | T.K_MODULE ii -> tag ii KeywordModule
         | T.K_YIELD ii -> tag ii KeywordConcurrency
         | T.K_AND ii
         | T.K_OR ii
         | T.K_NOT ii ->
         ...
         | T.K_END ii ->
             tag ii BadSmell
         | T.K_UNDEF ii -> tag ii BadSmell
         | T.T_TICK_BEG ii -> tag ii BadSmell
         (* symbols *)
         | T.T_PLUS ii
         | T.T_UPLUS ii
         | T.T_MINUS ii
