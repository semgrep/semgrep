...
let rec exists msg pos = function
  ...
  | Ctx_Msg (msg', Ctx_Pos (pos', rest)) ->
      if msg = msg' && pos = pos' then true else exists msg pos rest
  | Ctx_Pos (_, rest)
  | Ctx_Msg (_, rest) ->
      exists msg pos rest
  | Ctx_Merge (c1, c2) -> exists msg pos c1 || exists msg pos c2

let rec append c1 c2 =
  match c1 with
  | Ctx_Empty -> c2
  | Ctx_Msg (msg, Ctx_Pos (pos, rest)) ->
      if exists msg pos c2 then append rest c2
...
let rec format_ctx ppf ctx =
  let rec work ppf = function
    | Ctx_Empty -> ()
    | Ctx_Pos (pos, Ctx_Empty) -> fprintf ppf "at %a" format_pos pos
    | Ctx_Msg (msg, Ctx_Pos (pos, Ctx_Empty)) ->
        fprintf ppf "@[<v 0>in %s@,at %a@]" msg format_pos pos
    | Ctx_Msg (msg, Ctx_Empty) -> fprintf ppf "in %s" msg
    | Ctx_Pos (pos, ctx) ->
        fprintf ppf "at %a@," format_pos pos;
        work ppf ctx
    | Ctx_Msg (msg, Ctx_Pos (pos, ctx)) ->
        fprintf ppf "in %s@,at %a@," msg format_pos pos;
        work ppf ctx
    | Ctx_Msg (msg, ctx) ->
    ...
    (fun msg ->
      if b then
        ...
        if no_dup then
          ...
          if Hashtbl.mem dup_tbl full_msg then ()
          else (
...
let show_raise ctx code fmt =
  let fail m =
    if not (*conf.error_raises_exc*) false then output_msg_ctx stderr_ppf m ctx;
...
let err ?(ctx = Ctx_Empty) fmt =
  if (*conf.error_raises_exc *) false then show_raise ctx "ERROR" fmt
