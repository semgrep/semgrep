...
let rec light_db_of_files_or_dirs lang xs =
  ...
  let db =
    match lang with
    | "ml" -> Database_light_ml.compute_database ~verbose xs
    | "php" ->
        failwith
          "pfff_db is obsolete, use 'codegraph -lang php -derived_data -build \
           <dir>'"
    (*
        (match xs with
        | [_x] ->
...
*)
    | "js" -> Database_light_js.compute_database ~verbose xs
    | "cpp" ->
...
let main_action xs =
  ...
  let file =
    ...
    | None, [ dir ] -> Filename.concat dir Database_code.default_db_name
    | Some s, _ -> s
    | _ -> failwith "please use -o"
  in
  let file = Common2.relative_to_absolute file in
  let res = Common2.y_or_no (spf "writing data in %s" file) in
  if not res then failwith "ok I stop";
...
let gen_pleac pleac_src =
  ...
  let skeleton = Pleac.parse_skeleton_file skeleton_file in
  ...
  let code_sections = Pleac.parse_data_file pleac_data_file in
  let comment_style = Pleac.detect_comment_style pleac_data_file in

  Pleac.gen_source_files skeleton code_sections comment_style
    ~output_dir:!pleac_dir ~gen_mode:Pleac.OneDirPerSection ~ext_file
...
let extra_actions () =
  [
    ( "-db_of_graph_code",
      " <graph_file> (works with -o)",
      Common.mk_action_1_arg db_of_graph_code );
    ( "-gen_pleac",
      " <pleac_src> (works with -lang and -output_dir)\n",
...
let all_actions () = extra_actions () @ Test_program_lang.actions () @ []
