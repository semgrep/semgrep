...
let find_big_branching_factor graph_file =
  ...
  pr2
    (spf "step1: big parents = %d, initial candidates for removal = %d"
  ...
  while !progress do
     ...
     *)
    dead_candidates ()
    |> List.iter (fun node -> Hashtbl.replace hdead_candidates node true);

    (* step2: make sure none of the candidate are used by live entities *)
    let live =
      ref
        (Graph_code.all_nodes g
        |> Common.exclude (fun node -> Hashtbl.mem hdead_candidates node))
    in

    let make_live ~from node =
      if not (Hashtbl.mem hlive_done (from, node)) then (
        Hashtbl.add hlive_done (from, node) true;
        Hashtbl.replace hlivermakes_stats from
          (1
          +
          try Hashtbl.find hlivermakes_stats from with
          | Not_found -> 0));
    ...
    while !live <> [] do
      ...
      this_round
      |> List.iter (fun live_node ->
             let uses = Graph_code.succ live_node Graph_code.Use g in
             uses
             |> List.iter (fun use_of_live_node ->
                    if Hashtbl.mem hdead_candidates use_of_live_node then
                      make_live use_of_live_node ~from:live_node))
    done;

    pr2
      (spf "step2(%d): candidates for removal = %d" !pass
         (Hashtbl.length hdead_candidates));
    ...
    while !dead <> [] do
      ...
      |> List.iter (fun dead_node ->
             ...
             live_uses_of_dead_code
             |> List.iter (fun live_use_of_dead_node ->
                    let xs =
                    ...
                    let users =
                      xs
                      |> List.map (fun node -> users_of_node node)
                      |> List.flatten
                    in
    ...
    pr2
      (spf "step4(%d): candidates for removal = %d" !pass
         (Hashtbl.length hdead_candidates));
    ...
    *)
    incr pass;
    if Hashtbl.length hdead_candidates = !last_cnt then progress := false
  ...
  |> Common.take_safe 10
  |> List.iter (fun (k, v) ->
         pr2 (spf "livemaker: %s (%d)" (Graph_code.string_of_node k) v));
    ...
    |> Common.map_filter (fun node ->
           ...
           | filename, Entity_code.File ->
               ...
               *)
               if
                 children
                 |> List.for_all (fun node -> Hashtbl.mem hdead_candidates node)
...
let extract_transitive_deps xs =
  ...
  let start_nodes =
    ...
    |> List.map (fun path ->
           let node =
             if Sys.is_directory path then (path, E.Dir) else (path, E.File)
           in
           if not (GC.has_node node g) then
             failwith (spf "could not find %s" path);
           node)
  in
  let rec dfs depth xs =
    ...
    | n :: xs ->
        if Hashtbl.mem hdone n || depth > max_depth then ()
        else (
  ...
  let files =
    ...
    |> Common.map_filter (fun n ->
           ...
           with
           | Not_found -> None)
    |> Common.hashset_of_list |> Common.hashset_to_list
  in
...
let slice_dir_with_file file =
  ...
  Common.command2 cmd;
  Common.cat file
  |> List.iter (fun file -> Common.command2 (spf "rm -f %s/%s" dst file))

(* ---------------------------------------------------------------------- *)
let pfff_extra_actions () =
  [
    ( "-extract_transitive_deps",
      " <files or dirs> (works with -o)",
      Common.mk_action_n_arg extract_transitive_deps );
    ( "-find_big_branching_factor",
      " <file>",
...
let options () =
  [ ("-o", Arg.Set_string output_dir, " <dir> generate codeslice in dir") ]
