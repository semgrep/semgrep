...
let visit_program ~tag_hook _prefs (_astopt, toks) =
  ...
  let tag_if_lexer ii categ =
    if (not (Hashtbl.mem already_tagged ii)) && lexer_based_tagger then
      tag ii categ
  in
  ...
  let rec aux_toks xs =
    ...
    | T.Tmodule _ :: T.Talias _ :: T.TUpperIdent (_s, ii3) :: xs ->
        tag_if_lexer ii3 (Entity (E.Module, def2));
        aux_toks xs (* module use, and function call! *)
    | T.TUpperIdent (_, ii)
      :: T.TDot _
      :: T.TLowerIdent (_, ii2)
      :: T.TOParen _ :: xs ->
  ...
  |> List.iter (fun tok ->
         ...
         | T.Tfrozen ii ->
             tag ii Attribute
         (* Punctuation *)
         | T.TBang ii -> tag ii UseOfRef
         | T.TEq ii -> tag ii Punctuation
         | T.TSemiColon ii
         | T.TPipe ii
         | T.TComma ii
         | T.TOBracket ii
