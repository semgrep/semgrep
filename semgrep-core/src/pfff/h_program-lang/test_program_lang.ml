...
let test_big_grep file =
  ...
  let entities =
    Db.files_and_dirs_and_sorted_entities_for_completion
      ~threshold_too_many_entities:300000 db
  in
...
let layer_stat file =
  ...
  stats |> List.iter (fun (k, v) -> pr (spf " %s = %d" k v))
...
let actions () =
  [
    ("-test_load_db", " <file>", Common.mk_action_1_arg test_load_light_db);
    ("-test_big_grep", " <file>", Common.mk_action_1_arg test_big_grep);
    ("-test_layer", " <file>", Common.mk_action_1_arg test_layer);
    ("-test_refactoring", " <file>", Common.mk_action_1_arg test_refactoring);
  ]
