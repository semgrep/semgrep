...
let rec vof_elt = function
  | OrigElt v1 ->
      let v1 = OCaml.vof_string v1 in
      OCaml.VSum ("OrigElt", [ v1 ])
  | Removed v1 ->
      let v1 = OCaml.vof_string v1 in
      OCaml.VSum ("Removed", [ v1 ])
  | Added v1 ->
      let v1 = OCaml.vof_string v1 in
      OCaml.VSum ("Added", [ v1 ])
  | Esthet2 (v1, v2) ->
...
and vof_esthet = function
  | Comment -> OCaml.VSum ("Comment", [])
  | Newline -> OCaml.VSum ("Newline", [])
  | Space -> OCaml.VSum ("Space", [])
...
let rec add_if_need_comma add_str rh tl =
  ...
  | OrigElt str :: _t when str = "," || str = "(" -> List.rev_append rh tl
  | (OrigElt _str as h) :: t -> List.rev_append rh (Added add_str :: h :: t)
   ...
   *)
  | Added _str :: _t -> failwith "need comma: cannot handle this case!"
...
let rec search_prev_elt ?(ws = 0) acc =
  ...
  | Added _str :: _t -> failwith "search_prev_real_elt: cannot handle this case"
  | Esthet2 (Comment, _str) :: t -> search_prev_elt ~ws t
  | Esthet2 (Newline, str) :: _t -> (Esthet2 (Newline, str), ws)
  | Esthet2 (Space, str) :: t -> search_prev_elt ~ws:(ws + String.length str) t
  ...
  let rec aux acc toks =
    ...
    | tok :: t -> (
        ...
        | OriginTok _ -> (
            ...
            (* acc is reversed! *)
            | NoTransfo -> aux (elt :: acc) t
            | Remove -> aux (Removed (PI.str_of_info info) :: acc) t
            | Replace toadd ->
                (* could also be Removed::Added::_, now that we have
                 * drop_useless_space(), this should not matter anymore
                 *)
                aux
                  (Added (s_of_add toadd)
                  :: Removed (PI.str_of_info info)
                  :: acc)
                  t
            | AddAfter toadd -> aux (Added (s_of_add toadd) :: elt :: acc) t
            | AddBefore toadd -> aux (elt :: Added (s_of_add toadd) :: acc) t
            | AddArgsBefore xs ->
...
let drop_whole_line_if_only_removed xs =
  let before_first_newline, xxs =
    xs
    |> Common2.group_by_pre (function
         | Esthet2 (Newline, _) -> true
         | _ -> false)
  in
  let xxs =
    xxs
    |> Common.exclude (fun (_newline, elts_after_newline) ->
           let has_a_remove =
             elts_after_newline
...
let string_of_toks_using_transfo toks =
  Common2.with_open_stringbuf (fun (_pr_with_nl, buf) ->
      ...
      if !debug then
        xs |> List.iter (fun x -> pr2 (OCaml.string_of_v (vof_elt x)));
