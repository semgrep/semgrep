...
type incr_decr = Incr | Decr [@@deriving show]
type prefix_postfix = Prefix | Postfix [@@deriving show]
type class_kind = Class | Interface | Trait [@@deriving show]
type function_kind = Function | Method | LambdaKind | Arrow [@@deriving show]
 ...
 *)
let is_metavar_name s = s =~ "^\\(\\$[A-Z_][A-Z_0-9]*\\)$"

(* coupling: Metavariable.is_metavar_ellipsis *)
let is_metavar_ellipsis s = s =~ "^\\(\\$\\.\\.\\.[A-Z_][A-Z_0-9]*\\)$"
