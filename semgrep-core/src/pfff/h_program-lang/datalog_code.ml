...
let bddbddb_of_facts facts dir =
  ...
  (* build sets *)
  metas
  |> List.iter (fun (arule, xs) ->
         let listref =
           try Hashtbl.find hrules arule with
           | Not_found ->
               let aref = ref [] in
         ...
         |> List.iter (fun v ->
                let add_v v =
                  ...
                  let hdomain =
                    try Hashtbl.find hvalues domain with
                    | Not_found ->
  ...
  let domains_idx =
    hvalues |> Common.hash_to_list
    |> List.map (fun (domain, hdomain) ->
           let conv =
             hdomain |> Common.hashset_to_list |> Common.index_list_0
           in
           (domain, (conv, conv |> Common.hash_of_list)))
  in

  Common.command2 (spf "rm -f %s/*" dir);
  (* generate .map *)
  domains_idx
  |> List.iter (fun (domain, (map, _idx)) ->
         if domain <> "Z" then
           let file = Filename.concat dir (domain ^ ".map") in
           Common.with_open_outfile file (fun (pr_no_nl, _chan) ->
               let pr s = pr_no_nl (s ^ "\n") in

               map |> List.iter (fun (v, _int) -> pr (string_of_value v))));

  (* generate .tuples *)
  hrules |> Common.hash_to_list
  |> List.iter (fun (arule, xxs) ->
         let arule =
           match arule with
           | "point_to" -> "point_to0"
           | "assign" -> "assign0"
                 ...
                 pr
                   (spf "# %s"
                      ...
                      |> List.map (fun v ->
                             let domain = domain_of_value v in
                             let cnt =
                               try Hashtbl.find hcnt domain with
                               | Not_found ->
                                   let cnt = ref 0 in
  ...
  let fvals =
    try List.assoc "F" domains_idx |> fst with
    | Not_found -> []
  in
  let nvals =
    try List.assoc "N" domains_idx |> fst with
    | Not_found -> []
  in
      ...
      | header :: xs ->
          if header =~ "# \\(.*\\)" then
            ...
            let fld_domains =
              flds
              |> List.map (fun s ->
                     if s =~ "\\([A-Z]\\)[0-9]?:" then Common.matched1 s
            ...
            |> List.iter (fun s ->
                   ...
                   let args =
                     Common2.zip vs fld_translates
                     |> List.map (fun (i, arr) -> arr.(i))
                   in
