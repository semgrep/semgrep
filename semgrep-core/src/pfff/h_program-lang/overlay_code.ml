...
let load_overlay file = Common2.get_value file
let save_overlay overlay file = Common2.write_value overlay file
...
let check_overlay ~dir_orig ~dir_overlay =
  ...
  let links = Common.cmd_to_list (spf "find %s -type l" dir_overlay) in

  let links =
    links
    |> Common.map_filter (fun file ->
           try Some (Common.fullpath file) with
           | Failure s ->
               pr2 s;
               None)
  in
  let files2 =
    links
    |> List.map (fun file_or_dir ->
           Common.files_of_dir_or_files_no_vcs_nofilter [ file_or_dir ])
    |> List.flatten
  in
  pr2
    (spf "#files orig = %d, #links overlay = %d, #files overlay = %d"
       (List.length files) (List.length links) (List.length files2));
  let h = Hashtbl.create 101 in
  files2
  |> List.iter (fun file ->
         if Hashtbl.mem h file then pr2 (spf "this one is a dupe: %s" file);
         Hashtbl.add h file true);

  let _common, only_in_orig, only_in_overlay =
    Common2.diff_set_eff files files2
  in

  only_in_orig |> List.iter (fun l -> pr2 (spf "this one is missing: %s" l));
  only_in_overlay |> List.iter (fun l -> pr2 (spf "this one is gone now: %s" l));
  if not (null only_in_orig && null only_in_overlay) then
    failwith "Overlay is not OK"
    ...
    |> List.map (fun link ->
           ...
           | Unix.S_DIR ->
               ...
               |> List.map (fun child ->
                      ...
                      (overlay, orig))
           | Unix.S_REG -> [ (link, Common.fullpath link) ]
           | _ -> [])
    |> List.flatten
  in
  let data =
    equiv
    |> Common.map_filter (fun (overlay, orig) ->
           try
             Some
...
let adapt_layer layer overlay =
  {
    layer with
    Layer_code.files =
      layer.Layer_code.files
      |> Common.map_filter (fun (file, info) ->
             try Some (Hashtbl.find overlay.orig_to_overlay file, info) with
             | Not_found ->
...
let layers_in_dir dir =
  Common2.readdir_to_file_list dir
  |> Common.map_filter (fun file ->
         if file =~ "layer.*marshall" then Some (Filename.concat dir file)
         else None)
...
let adapt_layers ~overlay ~dir_layers_orig ~dir_layers_overlay =
  ...
  |> List.iter (fun layer_filename ->
         ...
         Layer_code.save_layer layer'
           (Filename.concat dir_layers_overlay
    ...
    Database_code.files =
      ...
      |> Common.map_filter (fun (file, info) ->
             ...
             | Not_found ->
                 ...
                 None);
    entities =
      db.Database_code.entities
      |> Array.map (fun e ->
             {
               e with
               Database_code.e_file =
                 (try
                    Hashtbl.find overlay.orig_to_overlay e.Database_code.e_file
                  with
                 | Not_found -> "not_found_file_overlay");
             });
  }
