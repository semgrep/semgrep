...
let empty_index () =
  { big_string = ""; pos_to_entity = Hashtbl.create 1; case_sensitive = false }
...
let naive_top_n_search2 ~top_n ~query xs =
  ...
  let rec aux ~n xs =
    if n = top_n then []
      ...
      | [] -> []
      | e :: xs ->
          if e.Db.e_name ==~ re then e :: aux ~n:(n + 1) xs else aux ~n xs
  in
  aux ~n:0 xs
...
let build_index2 ?(case_sensitive = false) entities =
  ...
  |> List.iter (fun e ->
         ...
         let s = if case_sensitive then s else String.lowercase_ascii s in
...
let top_n_search2 ~top_n ~query idx =
  ...
  let re =
    match words with
    | [ _ ] -> Str.regexp (".*" ^ query)
    | [ a; b ] -> Str.regexp (spf ".*\\(%s.*%s\\)\\|\\(%s.*%s\\)" a b b a)
    | _ ->
  ...
  let rec aux ~n ~pos =
    if n = top_n then []
      ...
      try
        ...
        let pos_mark = find_position_marker_before new_pos idx.big_string in
        let pos_next_mark = find_position_marker_after new_pos idx.big_string in
