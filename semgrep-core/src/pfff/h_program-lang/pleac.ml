...
type section = string
type comment_style = string (* comment_start *) * string (* comment_end *)
type sections = (section, code_excerpt) Common.assoc

type skeleton =
  (string (* section1 *) * (string (* section2 title *) * section) list) list
...
let parse_data_file file =
  file |> Common.cat
  |> Common2.split_list_regexp regexp_section_pleac_data
  |> skip_no_heading
  |> List.map (fun (s, group) ->
         if s =~ regexp_section_pleac_data then
           let _, section, _ = Common.matched3 s in
           (section, group)
         else failwith ("Pleac.parse_data_file: impossible: " ^ s))
...
let parse_skeleton_file file =
  file |> Common.cat
  |> Common2.split_list_regexp regexp_skeleton_section1
  |> skip_no_heading
  |> List.map (fun (s, group) ->
         if s =~ regexp_skeleton_section1 then
           let section1 = Common.matched1 s in
           ( section1,
             group
             |> Common2.split_list_regexp regexp_skeleton_section2
             |> skip_no_heading
             |> List.map (fun (s2, group) ->
                    if s2 =~ regexp_skeleton_section2 then
                      let section2 = Common.matched1 s2 in
                      ( section2,
                        group
                        |> Common2.return_when (fun s3 ->
                               if s3 =~ regexp_skeleton_section_number then
                                 Some (Common.matched1 s3)
                               else None) )
                    else failwith ("Pleac.parse_data_file: impossible: " ^ s))
           )
         else failwith ("Pleac.parse_data_file: impossible: " ^ s))
...
type gen_mode = OneFilePerSection | OneDirPerSection

let gen_source_files skeleton sections (comment_start, comment_end) ~gen_mode
    ~output_dir ~ext_file ~hook_start_section2 ~hook_line_body
    ~hook_end_section2 =
  if not (Common2.command2_y_or_no ("rm -rf " ^ output_dir)) then
    failwith "ok we stop";
  ...
  | OneFilePerSection ->
      skeleton
      |> List.iter (fun (section1, xs) ->
             let file =
               Filename.concat output_dir (mangle_to_generate_filename section1)
               ^ "." ^ ext_file
             in
             Common.with_open_outfile file (fun (pr_no_nl, _chan) ->
                 ...
                 pr (spf "%s %s %s" comment_start estet_sect1 comment_end);
                 xs
                 |> List.iter (fun (section2, secnumber) ->
                        let code_opt =
                          try Some (Hashtbl.find hsections secnumber) with
                          | Not_found ->
                              pr2
                                (spf "Section %s was not found in data file"
                                   secnumber);
                              None
                        in
                        pr
                          (spf "%s %s %s" comment_start estet_sect2 comment_end);
                        pr (spf "%s %s %s" comment_start section2 comment_end);
                        pr
                          (spf "%s %s %s" comment_start estet_sect2 comment_end);
                        code_opt
                        |> Option.iter (fun code -> code |> List.iter pr))))
  | OneDirPerSection ->
      skeleton
      |> List.iter (fun (section1, xs) ->
             let dir =
             ...
             |> List.iter (fun (section2, secnumber) ->
                    let file =
                      Filename.concat dir (mangle_to_generate_filename section2)
                      ^ "." ^ ext_file
                    in
                    ...
                    |> Option.iter (fun code ->
                           Common.with_open_outfile file
                             (fun (pr_no_nl, _chan) ->
                               let pr s = pr_no_nl (s ^ "\n") in
                               pr
                                 (spf "%s %s %s" comment_start estet_sect1
                                    comment_end);
                               pr
                                 (spf "%s %s %s" comment_start section2
                                    comment_end);
                               pr
                                 (spf "%s %s %s" comment_start estet_sect1
                                    comment_end);
                               pr
                                 (hook_start_section2
                                    (mangle_to_generate_filename section2));
                               code
                               |> List.iter (fun s -> pr (hook_line_body s));
                               pr
                                 (hook_end_section2
                                    (mangle_to_generate_filename section2))))))
