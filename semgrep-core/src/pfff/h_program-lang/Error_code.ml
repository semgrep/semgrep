...
type t = { typ : error_kind; loc : Parse_info.token_location; sev : severity }
...
let string_of_error_kind error_kind =
  ...
  | UnusedExport ((s, kind), file_def) ->
      spf "useless export of %s (%s) (consider forward decl in %s)" s
  ...
  | UnusedAssign s ->
      spf "useless assignement for %s; the value in %s is never used after." s s
  | UseOfUninitialized s -> spf "use of unitialized variable: %s" s
...
let warning tok err =
  let loc = PI.unsafe_token_location_of_info tok in
  Common.push { loc; typ = err; sev = Warning } g_errors

let info tok err =
  let loc = PI.unsafe_token_location_of_info tok in
  Common.push { loc; typ = err; sev = Info } g_errors

let error_loc loc err = Common.push (mk_error_loc loc err) g_errors
let warning_loc loc err = Common.push { loc; typ = err; sev = Warning } g_errors
let info_loc loc err = Common.push { loc; typ = err; sev = Info } g_errors
...
let score_of_error err = err |> rank_of_error |> score_of_rank
...
*)
         ...
         | OtherParsingError _
         | AstGenericError _
           when not !report_parse_errors ->
             true
         | FatalError _ when not !report_fatal_errors -> true
         | _ -> false)
...
let exception_to_error file e =
  ...
  | Parse_info.Ast_builder_error (s, tok) -> mk_error tok (AstBuilderError s)
  | Parse_info.Other_error (s, tok) -> mk_error tok (OtherParsingError s)
...
let adjust_errors xs =
  ...
  |> Common.exclude (fun err ->
         ...
         | UndefinedDefOfDecl _ ->
             ...
             file =~ "^include/" || file = "kernel/lib/lib.h"
             || file = "kernel/network/ip/ip.h"
             || file =~ "kernel/conf/" || false
         | _ -> false)
