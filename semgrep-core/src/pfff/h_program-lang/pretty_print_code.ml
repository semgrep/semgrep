...
let newline env =
  env.pushed <- false;
  if env.last_nl then env.emptyl <- true;
  env.last_nl <- true;
  env.cmargin <- 0;
  env.line <- env.line + 1;
  env.print "\n"

let newline_opt env = if env.last_nl then () else newline env

let space_or_nl env =
  if env.cmargin < 75 then print env " "
  else (
    newline env;
    spaces env)
...
let pop env = env.margin <- List.tl env.margin
...
let try_hard env f =
  ...
  | Fail ->
      ...
      f env';
      if env'.failed then raise Fail else f { env with level = 0 }
...
let list env f opar l sep cpar =
  let simple env = flat_list env f opar l sep cpar in
  let nested env = nl_nested_list env f opar l sep cpar in
...
let fun_args env f opar l sep cpar =
  ...
  in
  let nl_nested env = nl_nested_list env f opar l sep cpar in
...
let fun_params env f l =
  ...
  let simple env =
    ...
    if line <> env.line then fail ()
  in
  let nl_nested env = nested_list env f opar l sep cpar true in
  choice_right env simple nl_nested
...
let paren prio env f =
  ...
  env.priority <- prio;
  if prio >= old_prio || prio = -1 then f env
  else (
    print env "(";
    f env;
    print env ")");
...
let rec is_only_space s i =
  if i >= String.length s then true else is_space s i && is_only_space s (i + 1)
...
let rec find_cut x start i =
  if i < 20 then start else if space x.[i] then i else find_cut x start (i - 1)
...
let first_char_escape env s =
  if s = "" then 0
    ...
    | _c ->
        print env "{'";
        let size = ref 1 in
        while !size < String.length s && not (char_is_space s.[!size]) do
          incr size
        done;
        let size = !size in
