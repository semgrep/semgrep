...
let (json_of_tests_coverage : tests_coverage -> J.t) =
 fun cov ->
  J.Object
    (cov
    |> List.map (fun (cover_file, tests_score) ->
           ( cover_file,
             J.Array
               (tests_score
               |> List.map (fun (test_file, score) ->
                      J.Array
                        [ J.String test_file; J.String (spf "%.3f" score) ])) ))
    )

(* todo: should be autogenerated by ocamltarzan *)
let (tests_coverage_of_json : J.t -> tests_coverage) =
 fun j ->
  match j with
  | J.Object xs ->
      xs
      |> List.map (fun (cover_file, tests_score) ->
             ( cover_file,
               ...
               | J.Array zs ->
                   zs
                   |> List.map (fun test_file_score_pair ->
...
(* todo: should be autogenerated by ocamltarzan *)
let (json_of_lines_coverage : lines_coverage -> J.t) =
 fun cov ->
  J.Object
    (cov
    |> List.map (fun (file, cover) ->
      ...
      *)
                 ( "cov",
                   J.Array (cover.covered_sites |> List.map (fun l -> J.Int l))
                 );
                 ( "all",
                   J.Array (cover.all_sites |> List.map (fun l -> J.Int l)) );
               ] )))

let (lines_coverage_of_json : J.t -> lines_coverage) =
 fun j ->
  match j with
  | J.Object xs ->
      xs
      |> List.map (fun (file, cover) ->
             ( file,
               ...
               | J.Object
                   [
                     ("cov", J.Array covered_lines); ("all", J.Array call_sites);
                   ] ->
                   {
                     covered_sites =
                       covered_lines
