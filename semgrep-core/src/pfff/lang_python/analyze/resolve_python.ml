...
(* Type *)
(*****************************************************************************)
type context = AtToplevel | InClass | InFunction
(* TODO: InLambda *)

type env = { ctx : context ref; names : (string * resolved_name) list ref }

let default_env () = { ctx = ref AtToplevel; names = ref [] }
...
let with_new_context ctx env f = Common.save_excursion env.ctx ctx f
...
let params_of_parameters tok params =
  ...
  let collect_param_names (ix, out) = function
    | ParamPattern (pat, _) -> (ix + 1, param_pattern_name ix pat :: out)
    ...
    | ParamEllipsis _ ->
        (ix + 1, out)
  in
  let zipped = List.fold_left collect_param_names (0, []) params in
  match zipped with
  | _, names -> names
...
let resolve prog =
  ...
  let comprehension ke v (e, xs) =
    ...
    in
    let new_names =
      new_vars |> List.map (fun name -> (Ast.str_of_name name, Ast.LocalVar))
    in
          ...
          (fun (k, v) x ->
            ...
            | Name (name, ctx, resolved) ->
                ...
                | AugLoad -> (
                    ...
                    | Some x -> resolved := x
                    | None ->
                        () (* will be tagged as Error by highlighter later *))
                | Store
                | AugStore ->
                    let kind =
                      let s = Ast.str_of_name name in
                    ...
                    (* optional *)
                | Del ->
                    (* should remove from env *)
                    ()
                | Param -> resolved := Parameter (* optional *));
                k x
          ...
          (fun (k, v) x ->
            ...
            | FunctionDef (t, name, params, _typopt, _body, _decorators) ->
                ...
                with_added_env new_names env (fun () ->
                    with_new_context InFunction env (fun () -> k x));
                (* nested function *)
                if !(env.ctx) = InFunction then
            ...
            | TryExcept (_, stmts1, excepts, stmts2) ->
                v (Stmts stmts1);
                excepts
                |> List.iter (fun (ExceptHandler (_t, _typ, e, body)) ->
                       match e with
                       | None -> v (Stmts body)
                       | Some name ->
                           let new_names =
                             (fst name, LocalVar) :: !(env.names)
                           in
                           with_added_env new_names env (fun () ->
                               v (Stmts body)));
                v (Stmts stmts2)
            | Global (_, names) ->
                names
                |> List.iter (fun name -> env |> add_name_env name GlobalVar)
            (* TODO: NonLocal!! *)
            (* general case *)
            | _ -> k x);
      }
