        ...
        V.kexpr =
          (fun (k, _) x ->
            ...
            | Name (name, ctx, resolved) ->
                ...
                | _ when !in_decorator -> tag_name name Highlight_code.Attribute
        ...
        V.kstmt =
          (fun (k, _) x ->
            ...
            | ClassDef (_t, name, _bases, _body, _decorators) ->
                let kind = E.Class in
                tag_name name (Entity (kind, def2));
                Common.save_excursion in_class true (fun () -> k x)
            | ImportAs (_, (dotted_name, _dotsTODO), asname_opt) ->
                let kind = E.Module in
                dotted_name
                |> List.iter (fun name -> tag_name name (Entity (kind, use2)));
                asname_opt
                |> Option.iter (fun asname ->
            ...
            | TryExcept (_, _stmts1, excepts, _stmts2) ->
                excepts
                |> List.iter (fun (ExceptHandler (_t, _typ, e, _)) ->
                       match e with
                       | None -> ()
                       | Some name -> tag_name name (Local Def));
                k x
            (* general case *)
            | _ -> k x);
        V.ktype_ =
          (fun (k, _) x -> Common.save_excursion in_type true (fun () -> k x));
        V.kdecorator =
          (fun (k, _) x ->
            Common.save_excursion in_decorator true (fun () -> k x));
        V.kparameter =
          (fun (k, _) x ->
            (match x with
...
*)
    ...
    | T.CLASS _ii1 :: T.NAME (_s, ii2) :: xs ->
        if (not (Hashtbl.mem already_tagged ii2)) && lexer_based_tagger then
          tag ii2 (Entity (E.Class, def2));
        aux_toks xs
    | T.DEF _ii1 :: T.NAME (_s, ii2) :: xs ->
        (* todo: actually could be a method if in class scope *)
        if (not (Hashtbl.mem already_tagged ii2)) && lexer_based_tagger then
          tag ii2 (Entity (E.Function, def2));
        aux_toks xs
    (* uses *)
    | T.NAME (_s, ii1) :: T.DOT _ :: T.NAME (_s3, ii3) :: T.LPAREN _ :: xs ->
        if (not (Hashtbl.mem already_tagged ii3)) && lexer_based_tagger then (
          tag ii3 (Entity (E.Method, use2));
          if not (Hashtbl.mem already_tagged ii1) then tag ii1 (Local Use));
        aux_toks xs
    | T.NAME (s, ii1) :: T.LPAREN _ :: xs ->
        if (not (Hashtbl.mem already_tagged ii1)) && lexer_based_tagger then
          if Hashtbl.mem builtin_functions s then tag ii1 Builtin
  ...
  |> List.iter (fun tok ->
          ...
          * put together *)
         | T.TCommentSpace ii -> tag_if_not_tagged ii Comment
         | T.NEWLINE ii -> tag_if_not_tagged ii Comment
         (* values  *)
         | T.STR (_s, _pre, ii) -> tag ii String
         | T.FLOAT (_, ii)
         | T.INT (_, ii)
         | T.LONGINT (_, ii) ->
             tag ii Number
         | T.IMAG (_, ii) -> tag ii Number
         | T.TRUE ii
         | T.FALSE ii ->
             tag ii Boolean
...
*)
         ...
         | T.EXCEPT ii ->
             tag ii KeywordExn
         | T.CLASS ii -> tag ii KeywordObject
         | T.IMPORT ii
         | T.AS ii
         | T.FROM ii ->
         ...
         | T.RPAREN ii ->
             tag ii Punctuation
         | T.ADD ii -> tag ii Punctuation
         | T.SUB ii -> tag ii Punctuation
         | T.MULT ii
         | T.DIV ii
         | T.MOD ii
         | T.FDIV ii
