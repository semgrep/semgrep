...
let visit_program ~tag_hook _prefs (*db_opt *) (_program, toks) =
  ...
  let rec aux_toks xs =
    ...
    | T.TIdent (s, ii1) :: xs when PI.col_of_info ii1 = 0 ->
        if not (Hashtbl.mem atom_already_tagged s) then (
...
*)
  ...
  |> List.iter (fun tok ->
         ...
         | T.Tbegin ii ->
             tag ii Keyword
         | T.Tcatch ii -> tag ii KeywordExn
         | T.TIdent (("module" | "include" | "export"), ii) ->
             tag ii KeywordModule
         | T.TIdent ("record", ii) -> tag ii Keyword
         ...
         | T.TColon ii ->
             tag ii Punctuation
         | T.TComma ii -> tag ii Punctuation
         | T.TDec ii
         | T.TInc ii
         | T.TBang ii ->
             tag ii Operator
         (* todo? put in pink/bad-smell ? *)
         | T.TAssign ii -> tag ii Operator
         | T.TArrow ii
         | T.TQuestion ii
         | T.TSemiColon ii ->
             tag ii Punctuation
         ...
         | T.TIdent (_s, _ii) -> ()
         | T.TVariable (_s, ii) ->
             if not (Hashtbl.mem already_tagged ii) then tag ii (Local Use));
