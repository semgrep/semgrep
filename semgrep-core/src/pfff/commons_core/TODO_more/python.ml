...
let test_pycaml () =
  ...
  let triplet =
    pytuple3
      (pyfloat_fromdouble 1.0, pyfloat_fromdouble 0.5, pyfloat_fromdouble 0.2)
  in
  ...
  print_endline
    (string_of_float (pyfloat_asdouble (pytuple_getitem (triplet, 0)))
    ^ " "
    ^ string_of_float (pyfloat_asdouble (pytuple_getitem (triplet, 1)))
    ^ " "
    ^ string_of_float (pyfloat_asdouble (pytuple_getitem (triplet, 2))));
  ()

let test_pycaml2 () = raise Todo
...
let test_python_direct () =
  let _ =
    Pycaml.pyrun_simplestring
      (Common.unlines [ "import nltk"; "nltk.draw.tree.demo()" ])
  in
  ()
...
let actions () =
  [
    ("-test_pycaml1", "", Common.mk_action_0_arg test_pycaml);
    ("-test_pycaml2", "", Common.mk_action_0_arg test_pycaml2);
    ("-test_pycaml3", "", Common.mk_action_0_arg test_pycaml3);
    ("-test_pycaml4", "", Common.mk_action_0_arg test_pycaml4);
  ]
