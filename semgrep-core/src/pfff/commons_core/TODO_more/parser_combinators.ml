...
let collectbis xs = String.concat "" (List.map (String.make 1) xs)
...
let rawnumber = pred digit +++ several digit >| fun x -> INT (collect x)
let rawident = pred alpha +++ several alphanum >| fun x -> IDENT (collect x)
let rawsymbol = pred symbol +++ several symbol >| fun x -> SYM (collect x)
...
let token = (rawident ||| rawnumber ||| rawkeyword) +++ several space >| fst

(* pad: bugfix: was not defined in jon harrop article *)
let tokens = many token
let alltokens = tokens +++ fin >| fst
let lex (string : string) = val_of_parser (alltokens (list_of_string string))
...
and factor s =
  (atom +++ a (KWD "*") +++ factor >| (fun ((f, _), g) -> Mul (f, g)) ||| atom)
    s

and term s =
  (factor +++ a (KWD "+") +++ term >| (fun ((f, _), g) -> Add (f, g)) ||| factor)
    s

let expr = term +++ fin >| fst
let parse p string = val_of_parser (p (lex string))
