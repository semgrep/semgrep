...
let tests =
  Testutil.pack_suites "analyze_ml"
    [
      ...
      (*****************************************************************************)
      [
        ( "building light database",
          fun () ->
            let data_dir = Config_pfff.tests_path "ml/db" in
            let _db =
              Database_light_ml.compute_database ~verbose [ data_dir ]
            in
            () );
      ];
      (*****************************************************************************)
      ...
      Testutil.pack_tests "coverage_ml"
        [
          ( "basename to readable",
            fun () ->
              let dummy_coverage =
                { Coverage_code.covered_sites = []; all_sites = [] }
              in
              let cover =
                [
              ...
              Alcotest.(check (list string))
                ...
                [ "analyze/unit_analyze_ml.ml"; "analyze/coverage_ml.ml" ]
                cover' );
        ]
      (*****************************************************************************)
      (* Postlude *)
