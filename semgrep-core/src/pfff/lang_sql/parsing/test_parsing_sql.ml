...
let test_tokens_sql file =
  if not (file =~ ".*\\.sql") then pr2 "warning: seems not a .sql file";
...
let stress_parse_sql file =
  ...
  ys
  |> List.iter (fun (_heading, xs) ->
         let s = Common.join " " xs in
         try
           let _ = Parse_sql.parse_string s in
           ()
         with
         | exn ->
...
let actions () =
  [
    ("-lexer_sql", "   <file>", Common.mk_action_1_arg test_tokens_sql);
    ("-parse_sql", "   <file>", Common.mk_action_1_arg test_parse_sql);
    ("-stress_sql", "   <file>", Common.mk_action_1_arg stress_parse_sql);
  ]
