...
let rec string_of_shape_ n stack shape =
  ...
  | FunctionShape (_, shape) ->
      mk_indent n "function" ^ string_of_shape_ (n + 1) stack shape
  | ArrayShape -> mk_indent n "array"
  | ObjectShape { contents = id, map, maps } ->
      if List.mem id stack then mk_indent n (mk_id id ^ "...")
      else
        SMap.fold
          (fun prop shape str ->
            str
  ...
  | ClassShape (instance, static) ->
      ...
      ^ mk_indent n "}"
  | RequireShape m -> mk_indent n ("require " ^ m)
  | PropertyShape (shape, x) ->
      mk_indent n ("." ^ x) ^ string_of_shape_ (n + 1) stack shape
  | UnknownShape reason -> mk_indent n ("unknown: " ^ reason)
  | NewShape _class -> mk_indent n "new" ^ string_of_shape_ (n + 1) stack _class
  | MixinShape map -> mk_indent n "mixin" ^ string_of_shape_ (n + 1) stack map
  | ClassWithMixinShape (_class, mixin) ->
      mk_indent n "+"
      ^ string_of_shape_ (n + 1) stack _class
      ^ string_of_shape_ (n + 1) stack mixin
  | ApplyShape _function ->
      mk_indent n "(...)" ^ string_of_shape_ (n + 1) stack _function
  | ElementShape _array ->
      mk_indent n "[...]" ^ string_of_shape_ (n + 1) stack _array

let string_of_shape n shape = string_of_shape_ n [] shape
