...
(* Helpers *)
(*****************************************************************************)
let hcommon_methods = Common.hashset_of_list [ "construct"; "getInstance" ]
let is_common_method s = Hashtbl.mem hcommon_methods s
...
let _mk_entity ~root ~hcomplete_name_of_info info categ =
  ...
  let name = s in
  let fullname =
    try Hashtbl.find hcomplete_name_of_info info |> snd with
    | Not_found -> ""
  in

  {
    Database_code.e_name = name;
...
let compute_database ?(verbose = false) files_or_dirs =
  ...
  files
  |> List.iter (fun file ->
         if verbose then pr2 (spf "PHASE 2: %s" file);

         if file =~ ".*external/" then
           pr2 (spf "skipping external file: %s" file)
         else
             ...
             | T.T_ID ("JX", ii1)
               ...
               when PI.col_of_info ii1 <> 0 ->
                 ...
                 |> List.iter (fun entity ->
                        ...
                        | _ -> ());
                 aux_toks xs
             | T.T_PERIOD _ :: T.T_ID (s, _ii) :: T.T_LPAREN _ :: xs ->
                 if not (is_common_method s) then
                   Hashtbl.find_all hdefs s
                   |> List.iter (fun entity ->
                          (* todo: should check that method of appropriate class
                           * but class analysis is complicated in Javascript
                          ...
                          | _ -> ());
                 aux_toks xs
             | T.T_ID (s, ii) :: T.T_LPAREN _ :: xs ->
                 (* could be the tokens for the def *)
                 if not (Hashtbl.mem hdefs_pos ii) then
                   Hashtbl.find_all hdefs s
                   |> List.iter (fun entity ->
                          (* todo: should check that method of appropriate class
                           * but class analysis is complicated in Javascript
                           *)
                          if entity.Db.e_kind = E.Function then
                            entity.Db.e_number_external_users <-
                              entity.Db.e_number_external_users + 1);
                 aux_toks xs
             | [] -> ()
