                ...
                ( _i_1,
                  [
                    Left
                      (VarClassic
                        {
                          v_name = class_name, info_class;
                          v_type = _;
                          v_init = Some (_i_3, Object _body_object);
                        });
                  ],
                  _scopt ) ->
                (* could restrict to only toplevel first column var declarations ? *)
                Hashtbl.add h info_class (E.Class, spf "Misc.%s" class_name);
                Common.save_excursion in_class (Some class_name) (fun () ->
                    (* todo? really in_members ? or in_statics ? *)
                    Common.save_excursion in_members true (fun () -> k st))
            (* var Foo = (function() { ... })() *)
            | VarsDecl
                ( _i_1,
                  [
          ...
          (fun (k, _) e ->
            ...
            | Assign
                ( Period (V (class_name, info_class), _i_3, ("prototype", _i_4)),
                  (A_eq, _i_6),
                  Object _body_object ) ->
                Hashtbl.add h info_class (E.Class, spf "Prototype.%s" class_name);
                Common.save_excursion in_class (Some class_name) (fun () ->
                    Common.save_excursion in_members true (fun () -> k e))
            (* Foo = { } *)
            | Assign
                (V (class_name, info_class), (A_eq, _i_56), Object _body_object)
              ->
                (* could restrict to only toplevel first column var declarations ? *)
                Hashtbl.add h info_class (E.Class, spf "Misc.%s" class_name);
                Common.save_excursion in_class (Some class_name) (fun () ->
            ...
            | Apply
                ( Period (V ("JX", _i_1), _i_3, ("install", _i_4)),
                  ( _i_6,
                    Left (L (String (class_name, info_class)))
                    :: Right _i_9
                    :: _rest_args,
                    _i_13 ) ) ->
                Hashtbl.add h info_class (E.Class, spf "JX.%s" class_name);

                (* was just %s before, but then get conflict between for instance
                 * DOM.setContent and JX.DOM.setContent
                 *)
                Common.save_excursion in_class
                  (Some (spf "JX.%s" class_name))
                  (fun () -> k e)
            ...
            | Apply
                ( Period (V (class_name, info_class), _i_3, ("mixin", _i_4)),
                  ( _i_6,
                    [
                      Left (L (String (mixin_name, _info_mixin)));
                      Right _i_9;
                      Left (Object _body_object);
                    ],
                    _i_13 ) ) ->
                ...
                ( Period (V ("FB", _i_1), _i_3, ("subclass", _i_4)),
                  ( _i_6,
                    [
                      Left (L (String (class_name, info_class)));
                      Right _i_9;
                      Left (L (String (_parent_class, _i_10)));
                      Right _i_12;
                      Left (Object _body_object);
                    ],
                    _i_16 ) ) ->
                Hashtbl.add h info_class (E.Class, spf "%s" class_name);
                Common.save_excursion in_class (Some class_name) (fun () ->
                    Common.save_excursion in_statics true (fun () -> k e))
                ...
                ( V ("copy_properties", info_class),
                  ( _i_3,
                    [
                      Left
                        (Period
                          (V (class_name, _i_4), _i_6, ("prototype", _i_7)));
                      Right _i_9;
                      Left (Object _body_object);
                    ],
                    _i_13 ) ) ->
              ...
              ->
                let fullname =
                  ...
                  | Some _classname, true, true ->
                      pr2
                        ("WEIRD: both statics and members at "
                  ...
                  | Some classname, false, false -> (
                      ...
                      | _ ->
                          pr2
                            ("WEIRD: in class but no statics or members at "
                            ^ Parse_info.string_of_info info_method_name);
                          "")
                  | None, true, _
                  | None, _, true ->
                      pr2
                        ("WEIRD: no class but statics or members at "
                        ^ Parse_info.string_of_info info_method_name);
                      ""
                  | None, _, _ ->
