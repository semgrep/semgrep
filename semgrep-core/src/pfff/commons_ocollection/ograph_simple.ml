...
class ['key, 'a, 'b] ograph_mutable =
  ...
  object (o)
    ...
    val mutable nods : ('key, 'a) Oassocb.oassocb = build_assoc ()
    ...
    method del_leaf_node_and_its_edges i =
      ...
      if not succ#null then
        failwith "del_leaf_node_and_its_edges: have some successors"
      else
        let pred = o#predecessors i in
        pred#tolist |> List.iter (fun (k, edge) -> o#del_arc (k, i) edge);
        o#del_node i
    ...
    method leaf_nodes () =
      ...
      o#nodes#tolist
      |> List.fold_left
           (fun acc (k, _v) -> if (o#successors k)#null then acc#add k else acc)
           set
    ...
    method add_node_if_not_present i (e : 'a) =
      if nods#haskey i then () else o#add_node i e
