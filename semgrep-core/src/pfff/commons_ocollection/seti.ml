...
let fromlist = function
  | xs -> List.fold_left (fun a e -> add e a) empty xs
 ...
 fun x xs ->
  ...
  | Interv (i, j) :: zs ->
      if x > j then xs (*  pb not in *)
        ...
        (let _ = assert (j > i) in
         (* otherwise can lead to construct seti such as [7,6] when removing 6 from [6,6] *)
         match () with
...
let _ =
  assert_equal (remove 3 [ Interv (1, 7) ]) [ Interv (4, 7); Interv (1, 2) ]
...
let rec mem e = function
  ...
  | Exact x :: xs -> (
      ...
      | Inf -> mem e xs)
  | Interv (i, j) :: xs ->
      if e > j then false else if e >= i && e <= j then true else mem e xs
...
let inter xs ys =
    ...
    | x :: xs, y :: ys -> (
        ...
        | Interv (i1, j1), Interv (i2, j2) -> (
            ...
            | Equal -> (
                ...
                (*  [      ] *)
                | Sup ->
                    Interv (i1, j2) :: aux (Interv (j2 + 1, j1) :: xs) ys
                    (*  [    ] *)
                    (*  [ ] [       same *))
            | Inf -> (
                if j1 < i2 then aux xs (y :: ys)
                  (* need order ? *)
                  (*  [    ] *)
                  (*         [ ] *)
                else
  ...
  List.rev_map exactize
    (aux (List.rev_map intervise xs) (List.rev_map intervise ys))
        ...
        | Interv (i1, j1), Interv (i2, j2) -> (
            ...
            | Inf -> (
                if j1 < i2 then Interv (i1, j1) :: aux xs (y :: ys)
                  ...
                  match j1 <=> j2 with
                  | Equal ->
                      Interv (i1, i2 - 1) :: aux xs ys (* -1 cos exlude [ *)
                  (*  [    ] *)
                  (*     [ ] *)
                  | Inf ->
                      Interv (i1, i2 - 1) :: aux xs (Interv (j1 + 1, j2) :: ys)
                  (*  [    ]  *)
                  (*     [     ]   *)
                  | Sup ->
                      Interv (i1, i2 - 1) :: aux (Interv (j2 + 1, j1) :: xs) ys
                  (*  [       ] *)
                  (*     [ ]  *))
            | Sup -> (
...
let patch3 xs =
  let aux min xs =
    ...
    |> List.fold_left
         (fun (min, acc) e ->
           match e with
           | Exact i -> if i =|= min then (min, acc) else (i, Exact i :: acc)
           | Interv (i, j) -> (j, Interv (i, j) :: acc))
         (min, [])
  in
