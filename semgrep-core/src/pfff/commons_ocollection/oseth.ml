...
class ['a] oseth _xs =
  object (o)
    ...
    method null =
      try
        Hashtbl.iter (fun _k _v -> raise Common2.ReturnExn) data;
        false
      with
      | Common2.ReturnExn -> true
    ...
    method getone =
      ...
      try
        Hashtbl.iter
          (fun k _ ->
            x := Some k;
            raise Common2.ReturnExn)
          data;
        raise Not_found
      with
      | Common2.ReturnExn -> Common2.some !x
  end
