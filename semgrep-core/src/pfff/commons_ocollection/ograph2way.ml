...
class ['a] ograph2way asucc apred (*f1*) f2 =
  object (o)
    ...
    method del_node e =
      {<(* nods = nods#del e; *) pred = pred#delkey e; succ = succ#delkey e>}
    ...
    method ancestors xs =
      let rec aux xs acc =
        match xs#view with
        (* could be done with an iter *)
        | Empty -> acc
        | Cons (x, xs) ->
            (acc#add x |> fun newacc -> aux (o#predecessors x) newacc)
    ...
    method children xs =
      let rec aux xs acc =
        ...
        | Empty -> acc
        | Cons (x, xs) ->
            (acc#add x |> fun newacc -> aux (o#successors x) newacc)
            |> fun newacc -> aux xs newacc
      in
      aux xs (f2 ())
    (* (new osetb []) *)
