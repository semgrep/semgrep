...
let visit_program ~tag_hook _prefs (*db_opt *) (_ast, toks) =
  ...
  let rec aux_toks xs =
    ...
    | T.Tmod _ :: T.TIdent (_s, ii2) :: xs ->
        if (not (Hashtbl.mem already_tagged ii2)) && lexer_based_tagger then
          tag ii2 (Entity (Module, Def2 fake_no_def2));
        aux_toks xs
    ...
    | T.Tfn _ :: T.TIdent (_s, ii2) :: xs ->
        if (not (Hashtbl.mem already_tagged ii2)) && lexer_based_tagger then
          tag ii2 (Entity (Function, Def2 fake_no_def2));
        aux_toks xs
    ...
    | T.TColonColon _ :: T.TIdent (_s3, ii3) :: T.TOParen _ :: xs ->
        if (not (Hashtbl.mem already_tagged ii3)) && lexer_based_tagger then
          tag ii3 (Entity (Function, Use2 fake_no_use2));
        aux_toks xs
    ...
    | T.TArrow _ :: T.TIdent (_s3, ii3) :: xs ->
        if (not (Hashtbl.mem already_tagged ii3)) && lexer_based_tagger then
          tag ii3 (Entity (Type, Use2 fake_no_use2));
        aux_toks xs
    | _x :: xs -> aux_toks xs
  in
  let toks' =
  ...
  |> List.iter (fun tok ->
         ...
         | T.Tcontinue ii ->
             tag ii Keyword
         | T.Tmatch ii -> tag ii KeywordConditional
         | T.Tcrate ii
         | T.Tuse ii
         | T.Tmod ii ->
         ...
         | T.Tin ii ->
             tag ii Keyword
         | T.Tproc ii -> tag ii Keyword
         | T.Tmut ii -> tag ii UseOfRef
         | T.Tref ii
         | T.Tbox ii ->
             tag ii UseOfRef
         | T.Tunsafe ii -> tag ii BadSmell
         | T.TCppLine ii -> tag ii CppOther
         (* symbols *)
         | T.TEq ii ->
