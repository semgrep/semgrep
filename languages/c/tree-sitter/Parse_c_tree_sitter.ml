...
and field_declaration_list_item (env : env)
  ...
  | `Field_decl (v1, v2, v3, v4) ->
      ...
      in
      let _v4 = token env v4 (* ";" *) in
      v2 |> List_.map (fun (id, f) -> { fld_name = Some id; fld_type = f v1 })
  | `Prep_def x ->
...
let () = preproc_expression_ := preproc_expression
