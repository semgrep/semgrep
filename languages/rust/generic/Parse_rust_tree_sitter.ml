...
let rec macro_items_to_anys (xs : rust_macro_item list) : G.any list =
  ...
  *)
  ...
  | None -> xs |> List.map macro_item_to_any
  | Some res -> [ G.Args (List.map (fun e -> G.Arg e) res) ]
...
let rec map_delim_token_tree (env : env) (x : CST.delim_token_tree) :
  ...
  | `LPAR_rep_delim_tokens_RPAR (v1, v2, v3) ->
      let lparen =
        ...
        (* "(" *)
      in
      let tokens = List.map (map_delim_tokens env) v2 in
      let rparen =
        token env v3
        (* ")" *)
      in
...
and map_anon_choice_meta_5845bda tp_attrs (env : env)
  ...
  | `Type_param (v1, v2, v3) ->
      ...
      let tp_bounds =
        match v2 with
        | Some x -> map_trait_bounds env x |> List.map type_of_trait_bound
        | None -> []
      in
      let tp_default =
        match v3 with
  ...
  | `Life_param (v1, v2) ->
      ...
      in
      G.OtherTypeParam
        (("LifeTime", snd id), G.I id :: List.map (fun x -> G.T x) tp_bounds)
...
and map_anon_choice_life_675c24c (env : env) (x : CST.anon_choice_life_675c24c)
  ...
  | `Use_bounds (v1, v2, v3, v4, v5) ->
      ...
      G.OtherType (("UseBound", v1), List.map (fun x -> G.I x) v3) |> G.t
...
and map_expressions_with_attributes (env : env)
  ...
  match opt with
  | Some (v1, v2, v3) ->
      let attrs_first = List.map (map_outer_attribute_item env) v1 in
        ...
        List.map
          (fun (v1, v2, v3) ->
            let _comma =
              ...
              (* "," *)
            in
            let attrs = List.map (map_outer_attribute_item env) v2 in
            let expr = map_expression env v3 in
            (attrs, expr))
          v3
      in
...
and map_pattern (env : env) (x : CST.pattern) : G.pattern =
  ...
  | `Gene_pat (v1, v2, v3) ->
      ...
      G.OtherPat
        ( ("GenericPat", v2),
          G.Name v1 :: List.map (fun x -> G.Ta x) (Tok.unbracket v3) )
...
and map_type_parameters (env : env)
  ...
  let lt =
    ...
    (* "<" *)
  in
  let attrs = List.map (map_attribute_item env) v2 in
      ...
      let class_def =
        {
          ...
          G.cmixins = [];
          G.cparams = fb [];
          G.cbody = (l, fields |> List.map (fun x -> G.F x), r);
        }
      in
      let ent =
        {
