...
let rec fold_stmt f acc stmt =
  ...
  | Begin s
  | End s ->
      fold_stmt f (f acc stmt) s
  | While (_g, body) -> fold_stmt f (f acc stmt) body
  | Case c ->
      let acc = f acc stmt in
      let acc =
        List.fold_left (fun acc (_w, b) -> fold_stmt f acc b) acc c.case_whens
      in
  ...
  | ExnBlock b ->
      ...
      let acc =
        List.fold_left
          (fun acc rb -> fold_stmt f acc rb.rescue_body)
          acc b.exn_rescue
      in
      let acc =
        Utils_ruby.do_opt ~none:acc ~some:(fold_stmt f acc) b.exn_ensure
      in
...
let rec compute_cfg_succ stmt (succs : stmt Set_.t) =
  match stmt.snode with
  ...
  | ExnBlock eb ->
      ...
      let succs' =
        match (eb.exn_ensure, eb.exn_else) with
        | None, None -> succs
        | Some x, None
        | None, Some x ->
...
let preds_from_succs stmt =
  fold_stmt
    (fun () stmt ->
      Set_.iter (fun succ -> succ.preds <- Set_.add stmt succ.preds) stmt.succs)
    () stmt
...
module Abbr = struct
  ...
  let case ?default guard whens pos =
    match whens with
    | [] -> (
        match default with
  ...
  let nameclass ?lhs name ?inh body pos =
    let kind =
      NominalClass ((name :> identifier), (inh :> identifier option))
    in
...
let msg_id_of_string str =
  ...
  | s ->
      ...
      if all_but_one = "=" then ID_Assign all_but_one else ID_MethodName s

open Visitor

class type cfg_visitor =
  object
...
let visit_def_name vtor dn =
  visit vtor#visit_def_name dn (function
    | Instance_Method msg ->
    ...
    | Singleton_Method (id, msg) ->
        ...
        let msg' = visit_msg_id vtor msg in
        if id' == id && msg' == msg then dn else Singleton_Method (id', msg'))

let rec visit_literal vtor l =
  visit vtor#visit_literal l (function
    | Array star_lst ->
    ...
    | Hash pair_lst ->
        let lst' =
          map_preserve List.map
            ...
            pair_lst
        in
        if pair_lst == lst' then l else Hash lst'
    | Range (b, e1, e2) ->
        let e1' = visit_expr vtor e1 in
        let e2' = visit_expr vtor e2 in
...
let visit_class_kind vtor ck =
  visit vtor#visit_class_kind ck (function
    | MetaClass id ->
        let id' = visit_id vtor id in
...
let visit_method_param vtor p =
  visit vtor#visit_method_param p (function
    | Formal_meth_id _
    | Formal_amp _
    | Formal_star _ ->
        p
...
and visit_stmt_children vtor stmt =
        ...
        map_opt_preserve
          (fun cb ->
            ...
            | CB_Block (formals, body) ->
                let formals' =
                  map_preserve List.map (visit_block_param vtor) formals
                in
                let body' = visit_stmt vtor body in
                if formals' == formals && body' == body then cb
                else CB_Block (formals', body'))
          mc.mc_cb
      in
      if
        lhso == lhso' && mc.mc_target == targ' && mc.mc_msg == msg'
        && mc.mc_args == args' && mc.mc_cb == cb'
      then stmt
      else
        let mc' =
          { mc_target = targ'; mc_msg = msg'; mc_args = args'; mc_cb = cb' }
        in
        update_stmt stmt (I (Call (lhso', mc')))
  | Yield (lhso, args) ->
      let lhso' = map_opt_preserve (visit_lhs vtor) lhso in
      let args' = map_preserve List.map (visit_star_expr vtor) args in
      if lhso == lhso' && args == args' then stmt
      else update_stmt stmt (Yield (lhso', args'))
  | I (Assign (lhs, rhs)) ->
      let lhs' = visit_lhs vtor lhs in
      let rhs' = visit_tuple vtor rhs in
      if lhs == lhs' && rhs == rhs' then stmt
      else update_stmt stmt (I (Assign (lhs', rhs')))
  | For (blist, guard, body) ->
      let blist' = map_preserve List.map (visit_block_param vtor) blist in
      let guard' = visit_expr vtor guard in
      let body' = visit_stmt vtor body in
      if guard == guard' && body == body' && blist == blist' then stmt
      else update_stmt stmt (For (blist', guard', body'))
  | Begin s ->
  ...
  | While (g, body) ->
      ...
      let body' = visit_stmt vtor body in
      if g == g' && body == body' then stmt
      else update_stmt stmt (While (g', body'))
  | If (g, s1, s2) ->
      let g' = visit_expr vtor g in
      let s1' = visit_stmt vtor s1 in
      let s2' = visit_stmt vtor s2 in
      if g == g' && s1 == s1' && s2 == s2' then stmt
      else update_stmt stmt (If (g', s1', s2'))
  | Case c ->
      ...
      if guard' == c.case_guard && whens' = c.case_whens && else' == c.case_else
      then stmt
      else
        let c' =
          { case_guard = guard'; case_whens = whens'; case_else = else' }
        in
      ...
      let rescue' =
        map_preserve List.map
          (fun resc ->
            ...
            if guards' == resc.rescue_guards && rbody' = resc.rescue_body then
              resc
            else { rescue_guards = guards'; rescue_body = rbody' })
          e.exn_rescue
      in
      if
        body' == e.exn_body && else' == e.exn_else && ensure' == e.exn_ensure
        && rescue' == e.exn_rescue
      then stmt
      else
        let e' =
  ...
  | D (MethodDef (def_name, args, body)) ->
      ...
      let body' = visit_stmt vtor body in
      if def_name == def_name' && args == args' && body == body' then stmt
  ...
  | D (Undef msg_l) ->
      ...
      if msg_l == msg_l' then stmt else update_stmt stmt (D (Undef msg_l'))
  | Redo
  | Retry ->
      stmt
...
class alpha_visitor ~var ~sub =
  object (_self)
    ...
    method! visit_id id =
      match id with
      | Var (Local, s) ->
          if String.compare var s = 0 then ChangeTo (Var (Local, sub))
          else DoChildren
      | _ -> DoChildren
  end

let alpha_convert_local ~var ~sub s = visit_stmt (new alpha_visitor ~var ~sub) s

let rec locals_of_lhs acc (lhs : lhs) =
  match lhs with
...
let rec locals_of_any_formal acc (p : any_formal) =
  match p with
