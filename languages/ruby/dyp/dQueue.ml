...
let rev_map func (f, r) = (List.rev_map func r, List.rev_map func f)

let map func (f, r) =
  (List.rev (List.rev_map func f), List.rev (List.rev_map func r))

let to_list (f, r) = List.rev_append (List.rev f) (List.rev r)
let from_list l = (l, [])
...
let rec compare_list cmp x y =
  match (x, y) with
  | [], [] -> 0
  | _ :: _, [] -> 1
  | [], _ :: _ -> -1
  | hx :: xs, hy :: ys -> (
      match cmp hx hy with
      | 0 -> compare_list cmp xs ys
      | c -> c)

let cmp c ((f1, r1) as l1) ((f2, r2) as l2) =
  match (r1, r2) with
...
let list_to_string to_s = function
  | [] -> "[]"
  | hd :: tl ->
      Printf.sprintf "[%s%s]" (to_s hd)
        (List.fold_left (fun acc t -> acc ^ ", " ^ to_s t) "" tl)

let to_string to_s (f, r) =
  "Q" ^ list_to_string to_s f ^ list_to_string to_s (List.rev r)

let rec gen_list (gen : ?size:int -> Random.State.t -> 'a) ?(size = 50)
    (r : Random.State.t) : 'a list =
  let size = abs size in
  if Random.State.int r size = 0 then []
...
type 'a path = Top | Path of 'a path * 'a
...
let rec goto_front c = if at_left c then c else goto_front (move_left c)
let rec goto_back c = if at_right c then c else goto_back (move_right c)
