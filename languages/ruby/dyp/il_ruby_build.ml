...
let acc_seen acc1 acc2 = { acc1 with seen = StrSet.union acc1.seen acc2.seen }
...
let gen_super_args params : (star_expr list * expr option) option =
  ...
  | lst -> Some (List.rev_map work lst, None)

let make_call_expr acc targ msg (args : star_expr list) cb _pos :
    stmt acc * expr =
...
*)
let parse_regexp_options acc str pos :
    stmt acc * expr option * char option * bool =
    ...
    | ('n' | 'N' | 'e' | 'E' | 's' | 'S' | 'u' | 'U') as c -> (
        ...
        | Some c' ->
            Log.err ~ctx:(Log.of_tok pos)
              "multiple language modifiers for regexp: %c %c??" c c';
            (acc, fin, Some c', once))
...
let refactor_binop pos : Ast.binary_op -> binary_op = function
  ...
  | ( Ast.B Ast.Op_NEQ
    | Ast.B Ast.Op_NMATCH
    | Ast.Op_OP_ASGN _ | Ast.Op_ASSIGN | Ast.Op_AND | Ast.Op_OR | Ast.Op_kAND
    | Ast.Op_kOR | Ast.Op_ASSOC
    | Ast.B Ast.Op_DOT2
    | Ast.Op_DOT3 ) as bop ->
      Log.fatal (Log.of_tok pos)
...
let rec tuple_of_lhs (lhs : lhs) pos : tuple_expr =
  match lhs with
  | LId id -> TE (EId id)
  | LTup l ->
...
let make_assignable_msg (m : msg_id) : msg_id =
  match m with
...
let _block_formal_of_id : identifier -> block_formal_param = function
  ...
  | _ -> Log.fatal Log.empty "block_formal_of_id: non-id"

(* convert the last value in the statement given by add_f *)
let rec convert_to_return (add_f : tuple_expr -> stmt_node) acc stmt =
  ...
  | ExnBlock eb ->
      ...
      let convert_rescue rb =
        {
          rb with
          rescue_body = q_to_stmt (convert_to_return add_f acc rb.rescue_body);
        }
      in
      (* body doesn't return if there's an else stmt *)
      let body =
        match eb.exn_else with
        | Some _ -> eb.exn_body
        | None -> q_to_stmt (convert_to_return add_f acc eb.exn_body)
      in
      let rescue = List.map convert_rescue eb.exn_rescue in
      let ensure = eb.exn_ensure in
      (* ensure doesn't return a value *)
      let eelse =
        Option.map q_to_stmt
  ...
  | Begin _
  | End _ ->
      (*Log.fatal (Log.of_tok stmt.pos)*)
      failwith "BEGIN or END block can not be used inside a method"

open Visitor

class proc_transformer =
  object
    ...
    method! visit_stmt s =
      ...
      | D (ModuleDef _)
      | D (MethodDef _)
      | D (ClassDef _ (* new scope *))
      | While _
      | For _
      | I (Call _ (* nested blocks *)) ->
          SkipChildren
      | _ -> DoChildren
  end

let proc_transform block =
  match block with
  | CB_Arg _ -> block
...
let add_final_return return_f acc pos =
  if DQueue.is_empty acc.q then
    acc_enqueue (mkstmt (return_f (TE (EId Nil))) pos) acc
  else
...
let rec refactor_expr (acc : stmt acc) (e : Ast.expr) : stmt acc * Il_ruby.expr
  ...
  | ( Ast.Binop (_, (Ast.Op_OP_ASGN _, _), _)
    ...
    | Ast.S (Ast.Retry _)
    | Ast.S (Ast.Case _)
    | Ast.S (Ast.ExnBlock _)
    | Ast.D (Ast.EndBlock _)
    | Ast.D (Ast.BeginBlock _)
  ...
  | Ast.ScopedId (Ast.TopScope (pos, e)) ->
      ...
      let s =
        ...
        | Var (Constant, s) -> s
        | _ ->
            Log.fatal (Log.of_tok pos) "unknown right hand of uscope: %s"
  ...
  | Ast.ScopedId (Ast.Scope (e1, pos, _e2_var_or_method_name_now)) ->
      ...
      let s =
        ...
        | Var (Constant, s) -> s
        | _ ->
            Log.fatal (Log.of_tok pos) "unknown right hand of scope: %s"
  ...
  | Ast.Binop (e1, (Ast.B Ast.Op_NEQ, pos), e2) ->
      ...
      let acc =
        refactor_binop_into_mc acc (Some t1) e1 (Ast.B Ast.Op_EQ) e2 pos
      in
  ...
  | Ast.Binop (e1, (Ast.B Ast.Op_NMATCH, pos), e2) ->
      ...
      in
      let acc =
        refactor_binop_into_mc acc (Some t1) e1 (Ast.B Ast.Op_MATCH) e2 pos
      in
      let t = C.assign t2 (TE (EId True)) pos in
      let f = C.assign t2 (TE (EId False)) pos in
      let acc = acc_enqueue (C.if_s (EId t1') ~t:f ~f:t pos) acc in
  ...
  | Ast.Call (Ast.Id (("defined?", pos), Ast.ID_Lowercase), args, cb) ->
      let _ =
        Option.map
          (fun _ -> Log.fatal (Log.of_tok pos) "cb for 'defined?' ??")
          cb
      in
      let acc, v = fresh acc in
      let v' =
  ...
  | Ast.Id ((s, pos), Ast.ID_Lowercase) as e ->
      ...
      else if StrSet.mem s acc.seen then (acc, EId (Var (Local, s)))
...
and refactor_super acc lhs pos =
  match acc.super_args with
  | None -> Log.fatal (Log.of_tok pos) "super called outside of method"
  | Some (args, None) -> acc_enqueue (C.mcall ?lhs ID_Super args ()) acc
  | Some (args, Some e) ->
...
and construct_explicit_regexp acc pos re_interp mods : stmt acc * expr =
  ...
  let build_call acc lhs =
    let re_interp =
      List.map
        (function
          ...
          | Ast.StrChars (s, t) -> Ast.StrChars (escape_chars s [ '\\' ], t))
        re_interp
    in
    ...
    in
    let call =
      C.mcall ~lhs ~targ:(EId (UScope "Regexp")) (ID_MethodName "new") new_opts
        ()
    in
  ...
  if once then
    ...
    let if_e =
      C.if_s (EId glob) pos ~t:(C.expr (EId glob) pos)
...
and refactor_interp_string acc istr pos =
  ...
  in
  let rec helper acc expr_acc l =
    match l with
    | [] -> (acc, expr_acc)
    | hd :: tl ->
        let acc, e = refactor_contents acc hd in
        let acc, expr_acc =
          make_call_expr acc (Some expr_acc) (ID_Operator Op_Plus) [ SE e ] None
            pos
        in
...
and refactor_lit acc (l : Ast.literal) : stmt acc * expr =
  match l with
  | Ast.Char _
  | Ast.Complex _
  | Ast.Rational _ ->
...
and refactor_star_expr (acc : stmt acc) e : stmt acc * star_expr =
  match e with
...
and refactor_method_arg_no_cb acc e : stmt acc * star_expr =
  match e with
...
and refactor_method_args_and_cb acc (_lp, arg_list, _rp) cb =
  ...
  let acc, lst, cb_arg =
    match (List.rev arg_list, cb) with
        ...
        List.fold_left
          (fun (acc, lst) -> function
            | Ast.Binop (e1, Ast.(Op_ASSOC, _pos), e2) ->
                ...
                (acc, (e1', e2') :: lst)
            | _ ->
                Log.fatal (Log.of_tok pos) "non assoc expression in hash list?")
          (acc, []) l
      in
      (acc, List.rev rlst)
  | _ :: _ ->
...
and refactor_binop_into_mc acc res e1 bop e2 pos =
  ...
  let call =
    C.mcall ?lhs:res ~targ:e1' (ID_Operator (refactor_binop pos bop)) [ e2' ] ()
  in
  acc_enqueue call acc
...
and refactor_tuple_expr (acc : stmt acc) (e : Ast.expr) :
    stmt acc * Il_ruby.tuple_expr =
  match e with
  | Ast.Tuple l ->
  ...
  | Ast.Splat (_pos, Some e) -> (
      ...
      | TStar _ ->
          Log.fatal Log.empty
            "refactor_tuple_expr: nested / double star expression")
  | _ ->
...
and refactor_lhs acc e : stmt acc * lhs * stmt acc =
  let acc, lhs, after =
    ...
    | Ast.Call (Ast.DotAccess (targ, _pos, msg), args, None) ->
        ...
        (acc, last_arg, after)
    | Ast.Call _ -> Log.fatal Log.empty "unhandled methodcall on lhs?"
    | _ -> (
...
and refactor_id (acc : stmt acc) e : stmt acc * identifier =
  ...
  | _, ELit l ->
      Log.fatal
        (Log.of_tok (tok_of e))
        "lhs_of_expr: literal %s"
        (CodePrinter.show_literal l)

and string_of_lit_kind _kind = "TODO"

and refactor_msg (acc : stmt acc) msg : stmt acc * msg_id =
  match msg with
  (*
...
*)
  ...
  | e ->
      Log.fatal
        (Log.of_tok (tok_of e))
        "refactor_msg unknown msg: %s\n" (Ast_ruby.show_expr e)

and refactor_msg2 (acc : stmt acc) msg : stmt acc * msg_id =
  match msg with
  | Ast.MethodOperator (bop, pos) ->
      (acc, ID_Operator (refactor_binop pos (Ast.B bop)))
  | Ast.MethodUOperator (uop, pos) ->
      (acc, ID_UOperator (refactor_uop pos (Ast.U uop)))
...
and refactor_codeblock acc : Ast.expr -> codeblock = function
  ...
  | _ -> Log.fatal Log.empty "refactor_codeblock: non-codeblock"
...
and add_last_assign ~do_break (id : identifier) (s : stmt) : stmt =
  ...
  | ExnBlock exn ->
      ...
      let rescue =
        List.map
          (fun r ->
            C.rblock r.rescue_guards
              (add_last_assign ~do_break id r.rescue_body))
          exn.exn_rescue
      in
      let ensure = exn.exn_ensure in
      (* ensure does not return a value *)
      ...
      let cb' =
        {
          ...
          case_whens =
            List.map
              (fun (gs, ss) -> (gs, add_last_assign ~do_break id ss))
              cb.case_whens;
          case_else =
            (match cb.case_else with
            | None -> None
            | Some x -> Some (add_last_assign ~do_break id x));
        }
      in
      mkstmt (Case cb') s.pos
...
and refactor_method_call_assign (acc : stmt acc) (lhs : lhs option) = function
  ...
  | Ast.Call (Ast.Id (("next", p1), Ast.ID_Lowercase), (_, args, _), None) ->
      ...
      let _envBUG, es =
        refactor_list refactor_tuple_expr (acc, DQueue.empty) args
      in
  ...
  | Ast.Call
      ( Ast.Id (((("proc" | "lambda") as m), _pos), Ast.ID_Lowercase),
        (_, [], _),
        Some cb ) ->
      ...
      acc_enqueue call acc
  | Ast.Call
      ( Ast.Id (("define_method", pos), Ast.ID_Lowercase),
      ...
      let name =
        (* H.msg_of_str *) Ast.MethodId ((mname, atompos), Ast.ID_Lowercase)
      in
      let body =
        {
          Ast.body_exprs = cb_body;
          rescue_exprs = [];
          ensure_expr = None;
          else_expr = None;
        }
  ...
  | Ast.Call
      ( Ast.ScopedId (Ast.Scope (targ, _pos2, _msg_var_or_methodname_now)),
        args,
        cb ) ->
  ...
  | Ast.Call (msg, args, cb) ->
      let () =
        ...
        | ( Ast.Id ((("proc" | "lambda"), pos), Ast.ID_Lowercase),
            ...
            Log.err ~ctx:(Log.of_tok pos)
              "unsupported proc/lambda with &block param"
        | _ -> ()
      in
      let acc, msg' =
        match msg with
        | Ast.Id (("super", _), Ast.ID_Lowercase) -> (acc, ID_Super)
...
and refactor_assignment (acc : stmt acc) (lhs : Ast.expr) (rhs : Ast.expr)
  ...
  | ( Ast.Call
        ( Ast.DotAccess (targ, _, Ast.MethodOperator (Ast.Op_AREF, _)),
          (_, args, _),
          None ),
      _ ) ->
      let acc, targ' = refactor_expr acc targ in
      let acc, rhs_arg = refactor_star_expr acc rhs in
      let acc, (lhs_args : star_expr DQueue.t) =
        refactor_list refactor_star_expr (acc, DQueue.empty)
          (args |> Ast.args_to_exprs)
      in
      ...
      let acc, args =
        ...
        then
          ...
          let call =
            C.mcall ~lhs:tmp ~targ:(ELit lhs_ary) (ID_Operator Op_Plus)
              [ SE (ELit rhs_ary) ] ()
          in
...
and refactor_stmt (acc : stmt acc) (e : Ast.expr) : stmt acc =
  ...
  | Ast.D
      (Ast.Alias
        ( p3,
          Ast.MethodId ((s1, p1), (Ast.ID_Global as k1)),
  ...
  | Ast.D (Ast.Undef (pos, e1)) ->
      let acc, e1' =
        refactor_list refactor_symbol_or_msg (acc, DQueue.empty) e1
      in
  ...
  | Ast.S (Ast.Return (pos, args)) ->
      let acc, args' =
        refactor_list refactor_tuple_expr (acc, DQueue.empty)
          (args |> Ast.args_to_exprs)
      in
  ...
  | Ast.S (Ast.Yield (pos, args)) ->
      let acc, args' =
        refactor_list refactor_method_arg_no_cb (acc, DQueue.empty)
          (args |> Ast.args_to_exprs)
      in
  ...
  | Ast.Binop
      ( e1,
        ...
        Ast.Binop
          ( l,
            ( Ast.B
                (( Ast.Op_PLUS | Ast.Op_MINUS | Ast.Op_TIMES | Ast.Op_REM
                 | Ast.Op_DIV | Ast.Op_CMP | Ast.Op_EQ
                 | Ast.Op_EQQ (*| Ast.Op_NEQ*)
                 | Ast.Op_GEQ | Ast.Op_LEQ | Ast.Op_LT | Ast.Op_GT | Ast.Op_BAND
                 | Ast.Op_BOR | Ast.Op_MATCH (*| Ast.Op_NMATCH*)
                 | Ast.Op_XOR | Ast.Op_POW | Ast.Op_AREF | Ast.Op_ASET
                 | Ast.Op_LSHIFT | Ast.Op_RSHIFT ) as op),
              _pos1 ),
            r ) ) ->
      let acc, e1', after = refactor_lhs acc e1 in
  ...
  | Ast.Call _ as m -> refactor_method_call_assign acc None m
  ...
  *)
  | Ast.Binop
      ( Ast.Id ((s, _pos'), Ast.ID_Lowercase),
        (Ast.Op_OP_ASGN Ast.Op_AND, pos),
        rhs )
    when not (StrSet.mem s acc.seen || is_special s) ->
      let id' = Var (Local, s) in
      let acc = add_seen s acc in
      Log.err ~ctx:(Log.of_tok pos) "removing dead code: %s"
        (Ast_ruby.show_expr rhs);
      acc_enqueue (C.assign (LId id') (TE (EId Nil)) pos) acc
  (* special case for 'x ||= e' when this is the first assignment to x.
     In this case, x is always set to e (the || can never fail) *)
  | Ast.Binop
      ( (Ast.Id ((s, _pos'), Ast.ID_Lowercase) as lhs),
        (Ast.Op_OP_ASGN Ast.Op_OR, pos),
        rhs )
    when not (StrSet.mem s acc.seen || is_special s) ->
      refactor_assignment acc lhs rhs pos
  ...
  | ( Ast.Id _ | Ast.Literal _ | Ast.Atom _ | Ast.Tuple _ | Ast.Hash _
    | Ast.Array _ | Ast.Unary _ | Ast.DotAccess _ | Ast.ScopedId _ (* TODO *)
    | Ast.Binop _ ) as e ->
      let acc, e' = refactor_expr acc e in
      acc_enqueue (C.expr e' (tok_of e)) acc
  | Ast.S (Ast.While (pos, true, g, body)) ->
      (* do .. while *)
  ...
  | Ast.S (Ast.While (pos, false, g, body)) ->
      (* while .. do *)
      let gpos = tok_of g in
  ...
  | Ast.S (Ast.Until (pos, b, g, body)) ->
      refactor_stmt acc
        (Ast.S (Ast.While (pos, b, Ast.Unary ((Ast.Op_UNot, pos), g), body)))
  ...
  | Ast.D (Ast.MethodDef (pos, meth, params, body)) ->
      ...
      in
      let body_acc =
        { (acc_emptyq in_acc) with super_args = gen_super_args params' }
      in
      let body_acc = refactor_body body_acc body pos in
      let body_acc =
        if DQueue.is_empty body_acc.q then
          acc_enqueue (C.expr (EId Nil) pos) body_acc
      ...
      let body_acc =
        {
          q = DQueue.append in_acc.q body_acc.q;
  ...
  | (Ast.CodeBlock _ | Ast.Lambda _) as s ->
      Log.fatal
        (Log.of_tok (tok_of s))
        "refactor_stmt: unknown stmt to refactor: %s\n" (Ast_ruby.show_expr s)

and refactor_method_name (acc : stmt acc) e : stmt acc * def_name =
  match e with
  (* todo: can factorize with next case when same msg type *)
  | Ast.SingletonM
      (Ast.ScopedId (Ast.Scope (targ, _pos, _msg_var_or_methodnamenow))) ->
...
and refactor_rescue pos acc (_, gs, exnvar_opt, rescue_body) :
    stmt acc * rescue_block =
  ...
  let body_acc =
    acc_seen { q = DQueue.empty; seen = set; super_args = acc.super_args } acc
  in
...
and refactor_rescue_guard acc (e : Ast.expr)
    (exnvar_opt : Ast.exception_variable option) : StrSet.t * rescue_guard =
  match exnvar_opt with
  | Some (pos, bind_e) ->
      let obj = Ast.Id (("StandardError", pos), Ast.ID_Uppercase) in
      refactor_rescue_guard acc
        (Ast.Binop (obj, (Ast.Op_ASSOC, pos), bind_e))
        exnvar_opt
  | None -> (
      match e with
      | Ast.Binop (exn_e, (Ast.Op_ASSOC, pos), bind_e) ->
          ...
          let () =
            if not (DQueue.is_empty after.q) then
              Log.fatal (Log.of_tok pos) "methodcall in rescue binder??"
          in
          let bind =
            match bind_lhs with
            | LId id -> id
          ...
          in
          if not (DQueue.is_empty acc.q) then
            Log.fatal (Log.of_tok pos)
              "rescue binding created hoisted expression?";
...
and refactor_method_formal (acc : stmt acc) t _pos :
    stmt acc * method_formal_param =
  match t with
  ...
  | Ast.Formal_rest _ ->
      ...
      in
      (acc, Formal_star s)
  | Ast.Formal_tuple _f_lst ->
      Log.fatal Log.empty "refactor_method_formal: formal_tuple?"
  | Ast.Formal_default ((f, _), _, s) -> (
      ...
      let s' = C.seq (DQueue.to_list default_acc.q) pos in
      match s'.snode with
      | I (Expression e) ->
          let e' = TE e in
...
and refactor_block_formal acc t pos : stmt acc * block_formal_param =
  match t with
  | Ast.Formal_hash_splat _
  | Ast.Formal_kwd _
  | Ast.ParamEllipsis _ ->
  ...
  | Ast.Formal_id (str, pos) ->
      ( add_seen str acc,
        Formal_block_id (refactor_id_kind pos Ast.ID_Lowercase, str) )
  ...
  | Ast.Formal_default _ ->
      Log.fatal (Log.of_tok pos) "refactor_block_formal: default arg?"
...
and refactor_case acc case pos =
  let acc, g' =
    match case.Ast.case_guard with
    | None -> (acc, EId True)
  ...
  let acc, whens' =
    List.fold_left
      (fun (acc, whens) (_, g, body) ->
        ...
        in
        let g' =
          match DQueue.to_list glist with
          | [] -> assert false
          | [ x ] -> x
          | lst -> TTup lst
        in
  ...
  in
  let else' =
    refactor_stmt_list (acc_emptyq whens')
      (case.Ast.case_else |> Ast.opt_stmts_to_stmts)
  in
  let default =
    if DQueue.is_empty else'.q then None
    else Some (C.seq (DQueue.to_list else'.q) pos)
  in
...
let refactor_ast ?env ast =
  ...
  let acc = { q = DQueue.empty; seen; super_args = None } in
