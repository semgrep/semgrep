...
let uniq_list cmp lst =
  let rec u = function
    ...
    | [ x ] -> [ x ]
    | x1 :: x2 :: tl ->
        if cmp x1 x2 = 0 then u (x1 :: tl) else x1 :: u (x2 :: tl)
  in
  u (List.sort cmp lst)
...
let enter_scope _dyp = Stack.push Env.empty env_stack
let leave_scope _dyp = ignore (Stack.pop env_stack)
...
let split_single_string_to_array str pos =
  let chunks =
    List.map
      (fun chunk -> Str.split ws_re chunk)
      (Str.split_delim bslash_spc_re str)
  in
  let strings =
    let rec reduce acc chunks =
      match (acc, chunks) with
      | _, [] -> List.rev acc (* done *)
  ...
  in
  let strings =
    List.map (fun s -> Arg (Literal (String (Single (s, pos))))) strings
  in
...
let split_double_string_to_array sc pos =
  ...
  *)
  ...
  let rec parse acc curr = function
    | [] -> if curr = [] then acc (* delim at end *) else ds curr :: acc
    | `Delim :: tl ->
  ...
  Array (pos, lst |> List.map (fun x -> Arg x), pos)
(* TODO pos1 *)

let str_of_interp sc =
  match sc with
  | [] -> ""
  | [ Ast_ruby.StrChars (s, _) ] -> s
  | _ -> failwith "unexpected escapes in string"

let merge_string_lits s1 s2 =
  match (s1, s2) with
...
let process_user_string m str pos =
  match m with
...
let rec replace_end expr new_e =
  match expr with
  | Binop (l, (o, p), r) -> Binop (l, (o, p), replace_end r new_e)
  | Call (m, (lp, [], rp), None) ->
      Call (replace_end m new_e, (lp, [], rp), None)
...
let well_formed_do guard _body =
  match ends_with guard with
  | Call (_, _, Some (CodeBlock ((_, false, _), _, _))) -> raise Dyp.Giveup
  | _ -> ()
...
let command_codeblock cmd cb =
  ...
  | DotAccess (_, p, _)
  | ScopedId (Scope (_, p, _)) ->
      Call (cmd, fb p [], Some cb)
...
let expr_priority = function
  | Unary ((U Op_UBang, _), _)
  | Unary ((U Op_UTilde, _), _)
  | Unary ((U Op_UPlus, _), _) ->
      2000
  | Unary ((U Op_UMinus, _), _) -> 1900
  | Binop (_, (B Op_POW, _), _) -> 1800
  | Binop (_, (B Op_DIV, _), _)
  | Binop (_, (B Op_REM, _), _)
  | Binop (_, (B Op_TIMES, _), _) ->
      1700
  | Binop (_, (B Op_MINUS, _), _) -> 1500
  | Binop (_, (B Op_PLUS, _), _) -> 1500
  | Binop (_, (B Op_LSHIFT, _), _)
  | Binop (_, (B Op_RSHIFT, _), _) ->
      1400
  | Binop (_, (B Op_BAND, _), _) -> 1300
  | Binop (_, (B Op_BOR, _), _)
  | Binop (_, (B Op_XOR, _), _) ->
      1200
  | Binop (_, (B Op_LEQ, _), _)
  | Binop (_, (B Op_LT, _), _)
  | Binop (_, (B Op_GEQ, _), _)
  | Binop (_, (B Op_GT, _), _) ->
      1100
  | Binop (_, (B Op_MATCH, _), _)
  | Binop (_, (B Op_NMATCH, _), _)
  | Binop (_, (B Op_NEQ, _), _)
  | Binop (_, (B Op_CMP, _), _)
  | Binop (_, (B Op_EQ, _), _)
  | Binop (_, (B Op_EQQ, _), _) ->
      1000
  | Binop (_, (B Op_DOT2, _), _)
  | Binop (_, (Op_DOT3, _), _) ->
      800
  | Binop (_, (Op_AND, _), _) -> 750
  | Binop (_, (Op_OR, _), _) -> 700
  | Ternary _ -> 650
  | Binop (_, (Op_ASSIGN, _), _)
  | Binop (_, (Op_OP_ASGN _, _), _) ->
      600
...
let prune_uop uop arg pos =
  ...
  if p' < p then raise Dyp.Giveup else e
...
let prune_right_assoc tk l op r =
  ...
  if pr < p || pl <= p then raise Dyp.Giveup else e
...
let prune_tern e1 e2 e3 pos1 pos2 =
  ...
  if p1 <= p then raise Dyp.Giveup else e
...
let resolve_block_delim with_cb no_cb =
  match (with_cb, no_cb) with
  | _, Call (_, (_, [], _), None) ->
      Printf.eprintf "here2??\n";
      [ with_cb; no_cb ]
  | Call (_m1', _args1, Some _do_block), Call (_m2', (_, args_ne, _), None) ->
      (* look for cmd arg1,...,(argn do block end) *)
      if rhs_do_codeblock (Utils.last args_ne |> arg_to_expr) then [ with_cb ]
      else [ with_cb; no_cb ]
  | _ -> assert false
...
let merge_stmt xs =
  wrap xs (fun xs ->
      ...
      | ( [ (Binop (_, (Op_ASSIGN, _), S (ExnBlock _)) as correct) ],
          ...
          [ correct ]
      | ( [
            (S (ExnBlock { body_exprs = [ Binop (_, (Op_OP_ASGN _, _), _) ]; _ })
            as correct);
          ],
      ...
      | ( [ Binop (l, (((Op_ASSIGN | Op_OP_ASGN _) as op), pos), r) ],
          (Binop _ | Ternary _) ) ->
          let l, op, r = fix_broken_neq l op r in
          [ Binop (l, (op, pos), r) ]
      (* we can't use is_cond_modifier to check for a rescue modifier,
         so we do it here *)
      | ( [
            ( S (If (_, S (ExnBlock _), _, _))
            | S (Unless (_, S (ExnBlock _), _, _))
            | S (Until (_, _, S (ExnBlock _), _))
            | S (While (_, _, S (ExnBlock _), _)) );
          ],
