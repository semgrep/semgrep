...
let map_modifier (env : env) (x : CST.modifier) : G.attribute =
  ...
  | `Publ_83d19bc tok -> (* "public(package)" *) G.attr G.Protected (token env tok)
  | `Publ_7c2e49a tok -> (* "public(friend)" *) G.attr G.Protected (token env tok)
...
let map_mutable (env : env) (x : CST.anon_choice_AMP_c6caa5d) =
  ...
  | `AMPmut tok -> (* "&mut" *) (token env tok, [G.KeywordAttr (G.Mutable, token env tok)])
...
let map_num_literal (env : env) (x : CST.num_literal) =
  match x with
  | `Typed_num_lit tok -> 
      (* pattern [0-9][0-9_]*(?:u8|u16|u32|u64|u128|u256)? *) map_typed_num_literal env tok
  | `Unty_num_lit tok -> 
...
let map_literal_value (env : env) (x : CST.literal_value) : G.literal =
  match x with
  | `Addr_lit tok -> map_num(map_address_literal env tok) (* pattern @0x[a-fA-F0-9]+ *)
  | `Bool_lit tok ->  map_bool_literal env tok
  | `Num_lit tok -> map_num (map_num_literal env tok)
  | `Hex_str_lit tok -> G.String (map_hex_string env tok) (* pattern "x\"[0-9a-fA-F]*\"" *)
  | `Byte_str_lit tok -> G.String (map_byte_string env tok) (* pattern "b\"(\\\\.|[^\\\\\"])*\"" *)
...
let get_members_from_member_list(member_list) =
  ...
  | Member_mod_only(mod_name, alias) -> failwith "Can't exctract a member from single module name"
  | Member_list_addr_members (address, mems) -> failwith "You need to be careful here because some are mods and some are members"
...
let map_inner_directive(address, mod_name, name, label): G.directive_kind =
  ...
  | "Self", _ ->
    G.ImportFrom
      (use, G.DottedName [ address ], [ (mod_name, alias) ])
  | _ ->
    G.ImportFrom
      (use, G.DottedName [ address; mod_name ], [ (name, alias )])
...
let map_member_to_directive(member_list) : G.directive_kind list =  
  ...
  | Member_list_addr (address, mod_name, mems) -> 
            mems |> List.map (fun (name, label) -> map_inner_directive(address, mod_name, name, label))
  | Member_list_addr_members (address, mems)-> 
      let out_list =
      mems |> List.map (fun x ->
        match x with
        | Member_mod_only(mod_name, label) -> [map_inner_directive_mod(address, mod_name, label)]
        | Member_list_no_addr_mod (mod_name, mems) -> 
            mems |> List.map (fun (name, label) -> map_inner_directive(address, mod_name, name, label))
        | Member_list_mem_only (mems) -> failwith "We should have converted members to modules"
        | Member_list_addr (address, mod_name, mems) -> failwith "Shouldn't have address in address"
        | Member_list_addr_members (address, mems) -> failwith "Shouldn't have address in address"
      ) in
    out_list |> List.flatten 
...
let map_parameter (is_mutable, is_dollar, ident, func_type) : G.parameter =
  let func_type=
    ...
    | true -> 
      { t = G.TyRef (fake "&mut" , func_type); t_attrs = [ G.KeywordAttr (G.Mutable, fake "&mut") ] }
    | false ->func_type in
  G.Param
    {
      G.pname = Some ident;
...
let rec map_use_member (env : env) (x : CST.use_member): member_list =
  match x with
  | `Choice_id_COLONCOLON_LCURL_use_member_rep_COMMA_use_member_opt_COMMA_RCURL x -> 
      (match x with
      | `Id_COLONCOLON_LCURL_use_member_rep_COMMA_use_member_opt_COMMA_RCURL (v1, v2, v3, v4, v5, v6, v7) -> 
...
let map_label (env : env) ((v1, v2) : CST.label):  G.ident  =
  let v1 = token env v1 in (* "'" *)
  let v2 = str env v2 in (* identifier *)
  v2



let map_ability_decls (env : env) ((v1, v2, v3) : CST.ability_decls): G.type_ list =
  let v1 = (* "has" *) token env v1 in
  let ability_list = List.map (fun (v1, v2) ->
...
let map_postfix_ability_decls (env : env) ((v1, v2, v3, v4) : CST.postfix_ability_decls) : G.type_ list  =
  let v1 = (* "has" *) token env v1 in
  let ability_list = List.map (fun (v1, v2) ->
    let v1 = map_ability env v1 in
    let v2 = (* "," *) token env v2 in
...
let map_number (env : env) (x : CST.num_literal) =
  match x with
  | `Typed_num_lit tok -> (* pattern [0-9][0-9_]*(?:u8|u16|u32|u64|u128|u256)? *) str env tok
...
let map_type_parameters (env : env) ((v1, v2, v3, v4, v5) : CST.type_parameters): G.type_parameters  =
  let lt = (* "<" *) token env v1 in
  let first_param = map_type_parameter env [] v2 in
  let params = 
    v3 |> List.map(fun (v1, v2) ->
      let v1 = (* "," *) token env v1 in
...
let map_spec_pragma (env : env) ((v1, v2, v3, v4) : CST.spec_pragma) =
  ...
  in
  let v4 = (* ";" *) token env v4 in
  let all_spec_prop = all_spec_prop |> List.map (fun x -> G.E x) in
...
let map_condition_properties (env : env) ((v1, v2, v3, v4) : CST.condition_properties) =
  ...
  let all_spec_props =
    ...
    | None -> props
  in
  let any_props = all_spec_props |> List.map (fun x -> G.E x) in
  let v4 = (* "]" *) token env v4 in
...
match x with 
  | `Choice_DOLLAR_id x ->
    ...
    | `Module_iden_COLONCOLON_id (v1, v2, v3) ->
        ...
        let v2 = (* "::" *) token env v2 in
        let member = (* identifier *) str env v3 in
        H2.name_of_ids [address;  mod_name; member], (Some address, Some mod_name, Some member)
...
let map_use_module_members (env : env) (x : CST.use_module_members) =
  match x with
  | `Choice_num_lit_COLONCOLON_LCURL_use_member_rep_COMMA_use_member_opt_COMMA_RCURL (v1, v2, v3, v4, v5, v6, v7) ->
    (* Here we have a list of multiple *)
    let address = map_anon_choice_num_lit_a33e50c env v1 in
    let v2 = (* "::" *) token env v2 in
    let v3 = (* "{" *) token env v3 in
    let member_list = map_use_member env v4 in
    let member_list_list = List.map (fun (v1, v2) ->
      let v1 = (* "," *) token env v1 in
    ...
    let clean_memes = all_mems |> List.map (fun x ->
      match x with  
      | Member_list_mem_only mems ->  
        let mods= 
        mems |> List.map (fun (v1, v2) ->
                          Member_mod_only (v1, v2)) in
        mods
      | Member_list_no_addr_mod (mod_name, mems) ->  [Member_list_no_addr_mod (mod_name, mems)]
      | Member_list_addr (address, mod_name, mems) -> failwith("Shouldn't have an address")
      | Member_mod_only(mod_name, alias) -> failwith("Can't exctract a member from single module name")
      | Member_list_addr_members (address, mems) -> failwith("Should not already be a address list"))
    in
     let flat_members = clean_memes |> List.flatten in
     Member_list_addr_members (address, flat_members)
  | `Module_iden_COLONCOLON_LCURL_use_member_rep_COMMA_use_member_opt_COMMA_RCURL (v1, v2, v3, v4, v5, v6, v7) ->
    let address, mod_name = map_module_identity env v1 in
    let v2 = (* "::" *) token env v2 in
    let v3 = (* "{" *) token env v3 in
    let member_list = map_use_member env v4 in
    ...
    let member_list_sing =
      (member_list_list |> List.map(fun(v1) -> (
  ...
  let map_use_module (env : env) ((v1, v2) : CST.use_module) : directive_kind list=
    ...
    let directive =
      ...
      | None ->   [ G.ImportFrom (use, G.DottedName [ adderss_ident ], [ (mod_name_ident, None) ]) ] in
      directive
    



let map_use_module_member (env : env) ((v1, v2, v3) : CST.use_module_member): member_list =
  let address, mod_name = map_module_identity env v1 in
  let v2 = (* "::" *) token env v2 in
...
let map_struct_signature (env : env) attrs ((v1, v2, v3, v4) : CST.struct_signature) =
...
let map_spec_apply_pattern (env : env) ((v1, v2, v3) : CST.spec_apply_pattern) =
  ...
  in
  let pattern = (* pattern [0-9a-zA-Z_*]+ *) map_spec_apply_name_pattern env v2 in
  ...
  in
  G.OtherPat (("SpecApplyPattern", G.fake ""), pub_int @ [pattern] @ type_params)

let map_use_fun (env : env) ((v1, v2, v3, v4, v5, v6) : CST.use_fun): G.directive_kind list  =
  let fun_ = (* "fun" *) token env v1 in
  let _, (address, mod_name, member) = map_module_access_ env v2 in
  let v3, tok = (* "as" *) str env  v3 in
  let use = fake "use" in
  let _, (alias_address, alias_mod_name, alais_member)  = map_module_access_ env v4 in
  let v5 = (* "." *) str env v5 in
  let function_ident = (* unknown token description *) str env v6 in
  let directive =  match (address, mod_name) with
                        ...
                        [
                          G.ImportFrom
                            ( use,
                              ...
                              [ (mod_name_ident, Some (alias_mod_name_ident, G.empty_id_info ())) ] );
                        ]
                      | None ->
                        [ G.ImportFrom (use, G.DottedName [ address_ident ;mod_name_ident], [ (function_ident, None ) ]) ])
                  |_ -> [] in
   directive
...
let rec map_function_type_parameters (env : env) ((v1, v2, v3, v4) : CST.function_type_parameters): G.parameter list  =
    let v1 = (* "|" *) token env v1 in
    let type_list =
      List.map (fun (v1, v2) ->
    ...
    in
    let v4 = (* "|" *) token env v4 in
    let all_params = List.map(fun (x) ->
      map_type_to_param env x) all_types in
    all_params
...
match x with
  | `Choice_apply_type x  ->   
    ...
    | `Apply_type (v1, v2) -> 
        ...
        let out_type =
          match v2 with
          | Some x ->  G.TyApply (G.TyN mod_access |> G.t, (map_type_arguments env x)) |> G.t
...
and map_datatype_fields (env : env) (x : CST.datatype_fields) =
  ...
  | `Posi_fields (v1, v2, v3, v4) ->
      ...
      let all_fileds =
        all_types       |> List.mapi (fun idx field_type ->
          let var_def =
          ...
          G.fld (ent, G.FieldDefColon var_def)) in
      (lp, all_fileds, rp)
...
and map_type_arguments (env : env) (x: CST.type_arguments): G.type_arguments =
  ...
  | `LT_type_rep_COMMA_type_opt_COMMA_GT (v1, v2, v3, v4, v5) ->
      ...
      in
      let all_types_args = first_type :: types |> List.map (fun x -> G.TA x) in
  ...
  | `Id_EQ_choice_COLONCOLON_module_access (v1, v2, v3) -> 
      ...
      let lh_exp =
        ...
        | `Module_access x -> 
            ...
            G.N mod_name |> G.e
        | `Lit_value x -> 
           G.L (map_literal_value env x) |> G.e
      in
...
let map_struct_item (env : env) (x : CST.struct_item) : G.stmt =
  ...
  | `Native_struct_defi (v1, v2, v3, v4) ->
      ...
      let (struct_, abilities, struct_ent) = map_struct_signature env (native_attrs @ public_attrs) v3 in
      let v4 = (* ";" *) token env v4 in
  ...
  | `Struct_defi (v1, v2, v3, v4) ->
      let public_attrs =
        ...
        | None -> []
      in
      let (struct_, abilities, struct_ent) = map_struct_signature env public_attrs v2 in
      ...
      let all_abilities =
        ...
        | None -> abilities
      in
      (struct_, all_abilities, struct_ent,  fields) in
  let struct_def =
    {
...
let map_spec_block_target (env : env) (x : CST.spec_block_target): G.any =
  ...
  | `Spec_blk_target_schema (v1, v2, v3) ->
      let schema = G.I (str env v1) in (* "schema" *)
      let identifier: G.ident = str env v2 in (* identifier *)
      let type_params =
        match v3 with
        | Some x -> Some (map_type_parameters env x)
        | None -> None
...
| G.OtherPat (("ExprToPattern", _), [ E e ])-> 
         (match e.e with
          ...
          |_ -> Log.err (fun m -> m "Unsupported pattern in let binding %s \n\n \n %s " (G.show_pattern left) (G.show_expr right));
...
let map_use_declaration (env : env) ((v1, v2, v3, v4): CST.use_declaration) : G.directive list =
  let attrs =
    (match v1 with
      | Some tok -> 
        (* "public" *)   [ G.KeywordAttr (G.Public, token env tok) ]
      | None -> []
    )
      in
  ...
  let v4 = (* ";" *) token env v4 in
  directives |>  
  List.map (fun x -> { d = x; d_attrs = attrs })
  ...
  let rec map_annotation_item (env : env) (x : CST.annotation_item) =
    ...
    | `Choice_anno_expr x ->
      ...
      | `Anno_list (v1, v2, v3, v4, v5, v6) -> 
          ...
          G.Assign
              (G.N name_name |> G.e, sc, G.Container (G.Dict, (v2, full_expr_list, v6)) |> G.e)
            |> G.e)
    | `Ellips tok  (* "..." *) -> G.Ellipsis (token env tok) |> G.e


  and map_anon_choice_lit_value_3ef3d77 (env : env) (x : CST.anon_choice_lit_value_3ef3d77): G.expr =
    match x with
    | `Lit_value x -> 
        G.L (map_literal_value env x) |> G.e
    | `Anno_item x -> 
    ...
    | `Module_access x -> 
         G.N (map_module_access env x )|>G.e
    | `COLONCOLON_module_access (v1, v2) -> 
        let v1 = (* "::" *) token env v1 in
        let mod_name = map_module_access env v2 in
        G.N (mod_name)|>G.e
...
let rec map_bind (env : env) (x : CST.bind):  G.pattern =
  ...
  | `Opt_mut_var_id (v1, v2) -> 
      ...
      let name_pattern = 
        ...
        | _ -> G.PatId (name, G.empty_id_info ()) in
      let final_pattern =
        match v1 with (* mut& *)
        | Some x -> G.PatDisj (G.PatType { t= G.TyN name_name;  t_attrs = [ G.KeywordAttr (G.Mutable, token env x ) ] } , name_pattern)
        | None -> name_pattern
      in
      final_pattern
  | `Bind_unpack (v1, v2, v3) ->
      ...
      let type_arg_kind =
        ...
        | None -> G.TyN type_arg_name in
      let full_pattern =
        match v3 with (*bind_fields *)
        | Some x -> 
           let lbrace, fields, rbrace  = map_bind_fields env x in
           G.PatDisj
            (G.PatType (type_arg_kind |> G.t), G.PatRecord (lbrace, fields, rbrace))
  ...
  and map_bind_field (env : env) (x : CST.bind_field)=
    ...
    | `Choice_opt_mut_choice_exp_opt_COLON_bind x ->
      ...
      | `Opt_mut_choice_exp_opt_COLON_bind (v1, v2, v3) ->
          (let v1 = (* Todo deal with "mut" *) 
            match v1 with
            | Some tok -> Some (token env tok) 
            | None -> None
          in
          let expr =( match v2 with
               | `Exp x -> map_expression env x) in
          let dot_id = 
                match expr.e with
                | G.N name -> 
               ...
               | G.L lit ->
                ...
                [lit_ident]
                | _  -> Log.err(fun m -> m "Not a lit %s \n %s" (G.show_expr expr) (G.show_expr_kind expr.e)); 
            ...
            let out =
              ...
              | None -> let ident  = List.nth dot_id 0 in
                (dot_id, G.PatId (ident , G.empty_id_info ())) in
            out
            )
          ...
          | `Spread_op tok -> 
            ...
            ([ident] ,G.IdSpecial (G.Spread, spread_tok) |> G.e|> H2.expr_to_pattern )
          )
    | `Ellips tok -> 
  ...
  and map_bind_fields (env : env) (x : CST.bind_fields) =
    ...
    | `Bind_posi_fields (v1, v2, v3, v4) ->
        ...
        let all_bind_fields =
          match v3 with
          | Some x ->  bind_fields @ [map_bind_field env x ](* Directly use option *)
          | None -> bind_fields
        in
        let rp = (* ")" *) token env v4 in
...
and map_bind_list (env : env) (x : CST.bind_list) =
  ...
  | `LPAR_rep_bind_COMMA_opt_bind_RPAR (v1, v2, v3, v4) -> 
      let lp = token env v1 in (* "(" *)
      let bind_list = List.map (fun (v1, v2) ->
        let bind = map_bind env v1 in
        let v2 = token env v2 in (* "," *)
        bind
      ) v2 in
      let all_binds = match v3 with
...
and map_lambda_bindings (env : env) ((v1, v2, v3, v4) : CST.lambda_bindings) =
  ...
  in
  let rp = token env v4 in (* "|" *) 
  let bind_params = List.map (fun x -> G.ParamPattern x) all_binds in
  let params = (lp, bind_params, rp) in
  (lp, params, rp)

and map_function_parameters (env : env) ((v1, v2, v3, v4) : CST.function_parameters): G.parameters =
  let lb = (* "(" *) token env v1 in
  let function_params =
    List.map (fun (v1, v2) ->
...
and map_spec_function_signature (env : env) attrs ((v1, v2, v3, v4) : CST.spec_function_signature) =
...
and map_macro_signature (env : env) ((v1, v2, v3, v4, v5, v6) : CST.macro_signature) =
  let attrs =
    match v1 with
...
and map_function_signature (env : env) attrs body ((v1, v2, v3, v4, v5, v6, v7) : CST.function_signature) =
  let mod_one =
    match v1 with
    | Some x -> [map_modifier env x]
    | None -> []
...
and map_enum_variant (env : env) ((v1, v2) : CST.variant) =
  let ident = str env v1 in (* identifier *)
  let orTypeElm =
    match v2 with
...
and map_enum_variants (env : env) ((v1, v2, v3, v4) : CST.enum_variants) : G.type_definition_kind  =
    let lb = (* "{" *) token env v1 in
    let orTypeElms = List.map (fun (v1, v2) ->
...
and map_arg_list (env : env) ((v1, v2, v3, v4) : CST.arg_list) =
  ...
  let all_expr =
    ...
    | None -> expr in
  let all_args = all_expr |> List.map (fun arg -> G.Arg arg)
  in
  let rb = (* ")" *) token env v4 in
  (lb, all_args, rb)
...
and map_cast_expression (env : env) ((v1, v2, v3) : CST.cast_expression): G.expr =
...
and map_dereference_expression (env : env) ((v1, v2) : CST.dereference_expression) : G.expr=
  let v1 = (* "*" *) token env v1 in
  let expr = map_expression env v2 in
  G.DeRef (v1, expr) |> G.e
  ...
  let func_expr =
    G.DotAccess
      ( expr_,
        dot,
        G.FN
          (H2.add_type_args_opt_to_name (H2.name_of_id (func_name_full_str , func_name_full_tok)) type_args)
      )
    |> G.e 
  in
...
and map_typed_metavariable( env : env)((v1,v2,v3,v4,v5) : CST.typed_metavariable) =
  let v1 = (* "(" *) token env v1 in
...
and map_expression (env : env) (x : CST.expression): G.expr =
  ...
  | `Choice_call_exp x ->
    ...
    | `While_exp (v1, v2, v3, v4, v5) -> 
        ...
        let while_stmt = G.While (while_, G.Cond cond, body |> G.exprstmt) |> G.s in
    ...
    | `Loop_exp (v1, v2) -> 
        ...
        let loop_stmt =  G.While (loop_, G.Cond cond, body |> G.exprstmt) |> G.s in
        G.stmt_to_expr loop_stmt
    | `Ret_exp (v1, v2, v3) -> 
        let ret = (* "return" *) token env v1 in
        let ret_label = (* todo what do with label?*)
          match v2 with
          | Some x -> Some (
              map_label env x
            )
    ...
    | `Quan_exp (v1, v2, v3, v4, v5) -> 
        ...
        G.OtherStmt (G.OS_Todo, binds @ constraint_ @ invariant) |> G.s |> G.stmt_to_expr
    | `Iden_exp (v1, v2) -> 
  ...
  | `Choice_brk_exp x ->
    (match x with
      ...
      | `Pack_exp (v1, v2, v3) -> 
          ...
          let struct_type =
            G.TyN (H2.add_type_args_opt_to_name name type_args) |> G.t in
          G.New (G.fake "new", struct_type, G.empty_id_info (), fields)
          |> G.e
...
and map_if_expression (env : env) ((v1, v2, v3, v4, v5, v6) : CST.if_expression): G.expr =
  ...
  let else_statement =
    ...
    | None -> None
  in
  let if_stmt = G.If (_if, G.Cond cond, then_statement, else_statement) |> G.s in
...
and map_block_ (env : env) ((v1, v2, v3, v4, v5) : CST.block): G.stmt list =
  ...
  let uses =
    v2
    |> List.map (map_use_declaration env)
    |> List.flatten
    |> List.map (fun x -> G.DirectiveStmt x |> G.s) in
  let stmts = List.map (map_block_item env) v3 in  (*todo map to statmet*)
  let final_expr =
    v4
...
and map_match_body (env : env) ((v1, v2, v3, v4) : CST.match_body): (G.pattern * G.expr) list  =
  let v1 = (* "{" *) token env v1 in
  let match_arms =
    List.map (fun (v1, v2) ->
...
and map_match_expression (env : env) ((v1, v2, v3, v4, v5) : CST.match_expression): G.expr =
...
and map_spec_apply (env : env) ((v1, v2, v3, v4, v5, v6, v7, v8) : CST.spec_apply) =
  ...
  in
  let all_patterns = (pattern :: patterns) |> List.map (fun x -> G.P x) in
  let v6 =
    match v6 with
    | Some tok -> Some ( (* "," *) token env tok)
    | None -> None
  ...
  let except_patterns =
    ...
    | Some (v1, v2, v3, v4) -> 
        ...
        let all_patterns = (pattern :: patterns) |> List.map (fun x -> G.P x)
        in
        let v4 =
          match v4 with
          | Some tok -> Some ((* "," *) token env tok)
  ...
  (G.OtherPat (("SpecApply", apply_), [ apply_expr; to_ ] @ all_patterns @ except_patterns))  |> H2.pattern_to_expr |> G.exprstmt
...
and map_spec_body (env : env) ((v1, v2, v3, v4) : CST.spec_body): G.stmt =
  let v1 = (* "{" *) token env v1 in
  let uses =
    List.map (map_use_declaration env) v2
    |> List.flatten
    |> List.map (fun x -> G.DirectiveStmt x |> G.s) 
  in
 let members = List.map (map_spec_block_memeber env) v3
...
| `Native_spec_func (v1, v2, v3, v4) ->
    ...
    let entity, params, ret_type = map_spec_function_signature env [attr] v3 in
...
match x with
  | `Choice_un_exp_ x ->
      ...
      | `Borrow_exp x -> let expr, _attr = map_borrow_expression env x in (*todo deal with attr*)
              expr
...
and map_vector_expression (env : env) ((v1, v2, v3, v4) : CST.vector_expression) =
  let _types = (*todo do something with these types*)
    ...
    | `Vect_type_rep_COMMA_type_opt_COMMA_GT_LBRACK (v1, v2, v3, v4, v5, v6) ->
        ...
        all_types
  in
  let expr_list = List.map (fun (v1, v2) ->
    let v1 = map_expression env v1 in
    let v2 = token env v2 in
    v1
  ) v2 in
  let values = match v3 with
...
and map_move_or_copy_expression (env : env) ((v1, v2) : CST.move_or_copy_expression): G.expr =
  let v1 =
    match v1 with
...
and map_function_item (env : env) (x : CST.function_item): G.stmt =
  ...
  | `Macro_func_defi (v1, v2, v3, v4) ->
      ...
      let (attrs, ident, type_parameters, function_parameters, ret_type) = map_macro_signature env v3 in (*todo what to do with func params*)
      let marco_smts = (map_block_ env v4) |> List.map (fun x -> G.S x) in
      let ident: G.ident =ident in
      let entity = G.basic_entity ~attrs: (attrs @ modfier_attrs)  ~tparams:type_parameters  ident in
...
let map_enum_item (env : env) (x : CST.enum_item) =
  ...
  | `Enum_defi (v1, v2, v3, v4) ->
      ...
      let _all_abilities = (*todo deal with abilities*)
        match v4 with
...
|`Choice_SEMI_rep_choice_use_decl_opt_RCURL (v1, v2, v3) ->
    ...
    let stmt_list_list =
      List.map (fun x ->
        ...
        | `Friend_decl x ->
          [ map_friend_declaration env x |> (fun x -> G.DirectiveStmt x) |> G.s ]
    ...
    let v3 =
      ...
      | None ->  None
    in
    let stmt_list= stmt_list_list |> List.flatten in
    stmt_list
  | `Ellips tok -> [G.Ellipsis (token env tok) |> G.e |>G.exprstmt]
...
let map_semgrep_statement (env : env) (xs : CST.semgrep_statement) =
  let stmt_list_list =
    ...
    | `Use_decl x -> (map_use_declaration env x)
                     |> List.map (fun x -> G.DirectiveStmt x |> G.s)
    | `Friend_decl x ->  [ map_friend_declaration env x |> (fun x -> G.DirectiveStmt x) |> G.s ]
    | `Cst x -> 
      [ map_constant env x ]
...
let map_source_file (env : env) (x : CST.source_file) =
  match x with
  | `Rep_module_defi xs ->
    G.Pr (xs |> List.map (map_module_definition env))
