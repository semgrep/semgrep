...
(* Prelude *)
(*****************************************************************************)
(* Parsing YAML using the ocaml-yaml lib (https://github.com/avsm/ocaml-yaml)
 * and returning an AST_yaml document.
 *
 * We're not using tree-sitter this time because the tree-sitter-yaml grammar is
 * complicated (see https://github.com/tree-sitter-grammars/tree-sitter-yaml).
 * This is partly because YAML is whitespace sensitive and this does fit very
 * well with parser generators like tree-sitter.
 *
 * This is why instead of using tree-sitter-yaml to parse YAML files we use
 * the OCaml library ocaml-yaml (which itself relies on a C library). We
 * also use this parser to parse YAML rules for semgrep. To get positions, we
 * use the low-level ocaml-yaml Stream API, which we now parse into AST_yaml
 * (we used to go directly to the generic AST).
 *
 ...
 *    forked from the unmaintained https://github.com/ikatyang/tree-sitter-yaml
 *    but really hard to read grammar.
 *  - kit-ty-kate yaml parser using menhir https://github.com/kit-ty-kate/yummy
 *    but toy project
 *
 * history: the code in this file used to be in yaml_to_generic.ml but with
 * the introduction of the intermediate AST_yaml.ml the code is now split in
 * two.
 *)
...
(* TODO: get rid of, could be handle by using Result.t in make_alias *)
exception UnrecognizedAlias of Tok.t

(* TODO: should export in .mli as it is not captured in this module *)
...
let tok (index, line, column) (str : string) (env : env) : Tok.t =
...
let do_parse (env : env) : E.t * E.pos =
...
*)
let apply_optional_tag (tag : string option) (tag_pos : E.pos) (e : A.value) env
    : A.value =
  match tag with
  | None -> e
  | Some tag_str ->
...
let scalar (tag, pos, value, (style : Y.scalar_style)) env : A.value * E.pos =
  ...
  *)
  ...
  let expr =
    ...
    then A.Metavar (value, mk_tok pos "" env)
    else
      let token = mk_tok ~style pos value env in
      if quoted then A.S (value, token)
      else
        match value with
        | "__sgrep_ellipses__" -> A.Ellipsis (Tok.fake_tok token "...")
...
let sequence (tag, start_pos, (es, end_pos)) env : A.value * E.pos =
  let expr = A.Sequence (mk_bracket (start_pos, end_pos) es env) in
  let expr = apply_optional_tag tag start_pos expr env in
  (expr, end_pos)

(* Mappings are dictionaries in the generic AST *)
let mappings (tag, start_pos, (es, end_pos)) env : A.value * E.pos =
  let expr =
    match es with
    | [ A.Ellipsis t ] -> A.Ellipsis t
    | _ -> A.Mapping (mk_bracket (start_pos, end_pos) es env)
  in
  let expr = apply_optional_tag tag start_pos expr env in
  (expr, end_pos)

let make_mapping (pos1, pos2) ((key, value) : A.value * A.value) env :
    A.value * E.pos =
...
let make_doc start_pos (doc, end_pos) env : A.value list =
  ...
  | [] -> []
  | [ x ] -> [ x ]
  | xs -> [ A.Sequence (mk_bracket (start_pos, end_pos) xs env) ]

let parse_with_env (env : env) : A.value list =
  (* Parse states *)
  let rec read_stream () : A.value list =
  ...
  and read_documents acc : A.value list * E.pos =
    match do_parse env with
  ...
  and read_sequence acc : A.value list * E.pos =
    ...
    | v, pos ->
        ...
        (seq :: rest, end_pos)
  and read_mappings acc : A.value list * E.pos =
    (* Capture the mutable `last_event` field prior to parsing, as the *)
    ...
    | v, pos ->
        ...
        (map :: rest, end_pos)
  and read_mapping first_node : A.value =
    let key, pos1 =
...
let preprocess_yaml (str : string) : string =
...
let mask_unicode (str : string) : string =
...
let parse_yaml_file ~is_target (file : Fpath.t) (str : string) : A.document =
...
let make_pattern_expr (e : A.value list) : A.any =
  match e with
  ...
  | [ A.Mapping (_lp, [ x ], _rp) ] -> A.V x
  | [ A.Sequence (_lp, [ x ], _rp) ] -> A.V x
  | [ x ] -> A.V x
...
let any (str : string) : A.any =
  ...
  in
  let xs = parse_with_env env in
  make_pattern_expr xs

let parse (file : Fpath.t) : AST_yaml.document =
