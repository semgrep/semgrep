...
and map_form (env : env) (x : CST.form) =
  ...
  | `Vec_lit (v1, v2) ->
      R.Case
        ( "Vec_lit",
          let _v1TODO = R.List (List.map (map_metadata_lit env) v1) in
      ...
      R.Case
        ( "Ns_map_lit",
          ...
          let v3 =
            ...
            | `Kwd_lit tok -> R.Case ("Kwd_lit", (* kwd_lit *) token env tok)
          in
          let _v4 = List.map (map_gap env) v4 in
  ...
  | `Var_quot_lit (v1, v2, v3, v4) ->
      R.Case
        ( "Var_quot_lit",
          let _v1TODO = R.List (List.map (map_metadata_lit env) v1) in
          let v2 = (* "#'" *) token env v2 in
          let _v3 = List.map (map_gap env) v3 in
  ...
  | `Sym_val_lit (v1, v2, v3) ->
      R.Case
        ( "Sym_val_lit",
          let v1 = (* "##" *) token env v1 in
          let _v2 = List.map (map_gap env) v2 in
  ...
  | `Tagged_or_ctor_lit (v1, v2, v3, v4, v5, v6) ->
      R.Case
        ( "Tagged_or_ctor_lit",
          ...
          let _v5 = List.map (map_gap env) v5 in
  ...
  | `Unqu_spli_lit (v1, v2, v3, v4) ->
      R.Case
        ( "Unqu_spli_lit",
          let _v1 = R.List (List.map (map_metadata_lit env) v1) in
          let v2 = (* "~@" *) token env v2 in
          let _v3 = List.map (map_gap env) v3 in
          let v4 = map_form env v4 in
          R.Tuple [ v2; v4 ] )
...
and map_metadata_lit (env : env) ((v1, v2) : CST.metadata_lit) =
  ...
  let _v2 =
    match v2 with
    | Some xs ->
        let _ = List.map (map_gap env) xs in
        ()
    | None -> ()
  in
  R.Tuple [ v1 ]
