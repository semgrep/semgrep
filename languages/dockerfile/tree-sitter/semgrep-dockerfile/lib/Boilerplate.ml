...
let map_anon_choice_SLAS_d948f33 (env : env) (x : CST.anon_choice_SLAS_d948f33) =
  (match x with
...
let map_expose_port (env : env) (x : CST.expose_port) =
  ...
  | `Expa_choice_SLAS (v1, v2) -> R.Case ("Expa_choice_SLAS",
      let v1 = map_expansion env v1 in
      let v2 = map_anon_choice_SLAS_d948f33 env v2 in
      R.Tuple [v1; v2]
    )
  | `Pat_e0f3805_opt_choice_SLAS (v1, v2) -> R.Case ("Pat_e0f3805_opt_choice_SLAS",
      ...
      let v2 =
        ...
        | Some x -> R.Option (Some (
            map_anon_choice_SLAS_d948f33 env x
          ))
        | None -> R.Option None)
      in
      R.Tuple [v1; v2]
