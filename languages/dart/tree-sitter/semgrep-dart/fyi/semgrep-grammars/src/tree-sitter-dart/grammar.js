...
module.exports = grammar({
  ...
  extras: ($) => [$.comment, $.documentation_comment, /\s/],
  ...
  rules: {
    ...
    program: ($) =>
      seq(
        ...
        repeat(
          prec.dynamic(22, seq(optional($._metadata), $._top_level_definition))
        ),
        //for testing:
        repeat($._statement)
      ),
    ...
    _top_level_definition: ($) =>
      choice(
        ...
        $.mixin_declaration,
        $.type_alias,
        seq(optional($._external_builtin), $.function_signature, $._semicolon),
        seq(optional($._external_builtin), $.getter_signature, $._semicolon),
        seq(optional($._external_builtin), $.setter_signature, $._semicolon),

        seq($.function_signature, $.function_body),
        seq(
          $.getter_signature,
          // optional($._type),
          // $._get,
    ...
    template_substitution: ($) =>
      seq(
        "$",
        choice(seq("{", $._expression, "}"), $.identifier_dollar_escaped)
      ),
    ...
    spread_element: ($) => seq("...", optional("?"), $._expression),
    ...
    arguments: ($) =>
      seq("(", optional(seq($._argument_list, optional(","))), ")"),
    ...
    wildcard: ($) =>
      seq(optional($._metadata), "?", optional($._wildcard_bounds)),

    _wildcard_bounds: ($) =>
      choice(seq("extends", $._type), seq($.super, $._type)),

    dimensions: ($) =>
      prec.right(repeat1(seq(optional($._metadata), "[", "]"))),
    ...
    local_function_declaration: ($) =>
      seq(optional($._metadata), $.lambda_expression),
    ...
    switch_block: ($) =>
      seq("{", repeat(choice($.switch_label, $._statement)), "}"),
    ...
    default_case: ($) =>
      choice(seq(repeat($.label), "default", ":", repeat1($._statement))),
    ...
    break_statement: ($) =>
      seq($._break_builtin, optional($.identifier), $._semicolon),
        ...
        seq(
          optional(
            choice(
              ...
              seq(commaSep(field("init", $._expression)), $._semicolon)
            )
          ),
    ...
    requires_modifier: ($) => choice("transitive", $._static),
    ...
    part_directive: ($) =>
      seq(optional($._metadata), "part", $.uri, $._semicolon),
    ...
    enum_body: ($) => seq("{", commaSep1TrailingComma($.enum_constant), "}"),

    enum_constant: ($) =>
      seq(optional($._metadata), field("name", $.identifier)),
    ...
    superclass: ($) =>
      choice(seq("extends", $._type_not_void, optional($.mixins)), $.mixins),
    ...
    mixin_declaration: ($) =>
      seq(
        ...
        $.identifier,
        optional($.type_parameters),
        optional(seq("on", $._type_not_void_list)),
        optional($.interfaces),
        $.class_body
      ),
      ...
      seq(
        ...
        repeat(
          choice(
            ...
            seq(optional($._metadata), seq($.method_signature, $.function_body))
          )
        ),
        "}"
      ),
    ...
    declaration: ($) =>
      choice(
        ...
        seq(optional($._external), $.operator_signature),
    ...
    _external_and_static: ($) => seq($._external, optional($._static)),
    ...
    factory_constructor_signature: ($) =>
      seq($._factory, sep1($.identifier, "."), $.formal_parameter_list),
    ...
    constant_constructor_signature: ($) =>
      seq($.const_builtin, $.qualified, $.formal_parameter_list),
    ...
    variable_declaration: ($) =>
      seq($._declared_identifier, optional(seq(",", commaSep1($.identifier)))),

    initialized_variable_definition: ($) =>
      seq(
        $._declared_identifier,
        optional(
          seq(prec(DART_PREC.BUILTIN, "="), field("value", $._expression))
        ),
    ...
    _type_not_function: ($) =>
      choice($._type_not_void_not_function, $.void_type),
    _type_not_void_not_function: ($) =>
      choice(
        seq(
          $._type_name,
    ...
    _named_parameter_type: ($) =>
      seq(optional($._metadata), optional($._required), $.typed_identifier),
    ...
    _type_not_void_list: ($) => commaSep1($._type_not_void),

    _type_name: ($) =>
      seq(
        alias($.identifier, $.type_identifier),
        optional($._type_dot_identifier)
        // optional($._nullable_type),
      ),
    ...
    function_body: ($) =>
      choice(
        seq(optional("async"), "=>", $._expression, $._semicolon),
    ...
    positional_parameters: ($) =>
      seq("[", commaSep1($._default_formal_parameter), "]"),
    _optional_postional_formal_parameters: ($) =>
    ...
    _simple_formal_parameter: ($) =>
      choice($._declared_identifier, seq(optional($._covariant), $.identifier)),
    ...
    spread_parameter: ($) =>
      seq(optional($._metadata), $._type, "...", $._declared_identifier),
    ...
    _covariant: ($) => prec(DART_PREC.BUILTIN, "covariant"),
    ...
    _factory: ($) => prec(DART_PREC.BUILTIN, "factory"),
    ...
    _required: ($) => prec(DART_PREC.BUILTIN, "required"),
    _set: ($) => prec(DART_PREC.BUILTIN, "set"),
    _static: ($) => prec(DART_PREC.BUILTIN, "static"),
    ...
    _late_builtin: ($) => prec(DART_PREC.BUILTIN, "late"),
    _external_builtin: ($) => prec(DART_PREC.BUILTIN, "external"),
    ...
    documentation_comment: ($) =>
      choice($._documentation_block_comment, seq("///", /.*/)),
  },
});
