...
let map_list (f: 'a -> 'b) (x: ('a * Token.t) list) (tail: 'a option): 'b list =
  let rec remove_separator (x: ('a * Token.t) list) = match x with
    | (head, _) :: tail -> head :: remove_separator(tail)
    | [] -> []
  in  
  
  let values = match tail with
...
let map_list_1 (f: 'a -> 'b) (head: 'a) (x: (Token.t * 'a) list): 'b list =
  let rec remove_separator (x: (Token.t * 'a) list) = match x with
...
let map_path_to_ident (env: env) (x: CST.path): G.ident =
  ...
  let name = List.fold_left (fun a (s, _) -> String.concat "::" [a; s]) "" path_segments in
  
  (name, name_to_token env v1)

let rec map_qualified_name (env: env) (x: CST.qualified_name): G.qualified_info =
  let map_segment (x: CST.qualified_name_segment): G.ident * G.type_arguments option =
  ...
  let type_arguments = match type_arguments with
...
and map_type_arguments (env: env) (x: CST.type_argument_list): G.type_arguments =
...
and map_modifiers (env: env) (x: CST.modifier list) =
  match x with
  | `Modi_mut tok :: tail -> (G.KeywordAttr (G.Mutable, token env tok)) :: (map_modifiers env tail)
...
and map_pattern (env: env) (x: CST.pattern): G.pattern =
  ...
  let map_pattern_struct (x: CST.pattern_struct) =
    let map_field_pattern (x: CST.pattern_struct_binding): G.dotted_ident * G.pattern =
      match x with
      ...
      | `Choice_pat_7fdeb71 name -> 
        ([ map_name env name ], G.PatId ((map_name env name), G.empty_id_info()))
    in
...
and map_let_statement (env: env) (x: CST.let_statement): G.stmt =
  ...
  let pattern = map_pattern env pattern
  and expr = map_expression env expr in
...
and map_if_expression (env: env) (x: CST.if_expression): G.expr = 
  ...
  and elze = match elze with
  | Some(_, `If_exp e) -> Some(G.ExprStmt (map_if_expression env e, G.sc) |> G.s)
...
and map_match_expression (env: env) (x: CST.match_expression) =
  let map_case (x: CST.match_case) =
    ...
    let case = match pattern with
    | `Choice_choice_wild x -> G.Case (token env arrow, map_pattern env x)
    | `Choice_true x -> G.CaseEqualExpr (token env arrow, G.L (map_literal env x) |> G.e)
    in
  ...
  G.stmt_to_expr (G.Switch (token env mmatch, Some(G.Cond condition), cases) |> G.s)
...
and map_return_statement (env: env) (x: CST.return_statement): G.stmt =
  ...
  | Some expr -> G.Return (token env return, Some (map_expression env expr), token env sc)
...
and map_break_statement (env: env) (x: CST.break_statement) =
  ...
  let stmt = match expr with
  | Some expr -> G.Break (token env break, G.LDynamic (map_expression env expr), token env sc)
...
and map_binary_expression (env: env) (x: CST.binary_expression) = 
    ...
    | `Choice_tuple_exp_AMPAMP_choice_tuple_exp (lhs, tok, rhs) -> 
      let lhs = map_expression env lhs
      and rhs = map_expression env rhs in
...
and map_attributes (env: env) (x: CST.attribute_list list): G.attribute list =
  let rec map_argument (x: CST.attribute_argument): G.argument =
    match x with
    | `Choice_true x -> 
      G.Arg (G.e (G.L (map_literal env x)))
    ...
    | `Path_COLON_attr_arg_list (path, _, (lp, v1, v2, _, rb)) ->
      let arguments = map_list_1 (fun x -> H2.argument_to_expr (map_argument x)) v1 v2 in
      let arguments = G.Container (G.Tuple, (token env lp, arguments, token env rb)) |> G.e in
  ...
  let map_attribute (x: CST.attribute): G.attribute =
    ...
    | Some (lp, v1, v2, _ , rp) -> (token env lp, map_list_1 (map_argument) v1 v2, token env rp)
    | None -> (G.fake "(", [], G.fake ")")
    in
...
and map_type_parameters (env: env) (x: CST.type_parameter_list): G.type_parameters =
...
and map_function_signature (env: env) (x: CST.function_signature): function_signature =
  let map_param (x: CST.parameter_declaration): G.parameter =
    match x with
    | `Ellips x ->
      G.ParamEllipsis (token env x)

    | `Rep_choice_modi_ref_choice_pat_7fdeb71_COLON_choice_type_tuple (modifiers, param_name, _, ttype) ->
  ...
  let fn_name : G.entity = { 
    ...
    tparams = match type_parameters with
...
and map_function_declaration (env: env) (x: CST.function_declaration): G.definition =
  let (signature, body) = x in

  let ((entity, params, return_type), tok) = map_function_signature env signature in
...
and map_import_declaration (env: env) (x: CST.import_declaration): G.directive =
  let (use, path, alias, _) = x in
  let path = G.DottedName (map_path env path) in
  
  let directive = match alias with
  | Some (_, alias) -> G.ImportAs (token env use, path, Some (map_name env alias, G.empty_id_info()))
  | None -> G.ImportAs (token env use, path, None)
  in
  
  { d = directive; d_attrs = [] }

and map_module_declaration (env: env) (x: CST.module_declaration): G.definition =
  ...
  (entity, G.ModuleDef { 
    mbody = G.ModuleStruct (Some [map_name env name], body) 
  })
...
and map_typealias_declaration (env: env) (x: CST.typealias_declaration): G.definition =
...
and map_trait_declaration (env: env) (x: CST.trait_declaration): G.definition =
  let map_function (x: CST.trait_function): G.field =
    ...
    let ((entity, parameters, return_type), tok) = map_function_signature env signature in
    ...
    let definition = G.s (G.DefStmt (entity, G.FuncDef { 
      fkind = (G.Function, tok);  
      fparams = parameters;
      frettype = return_type;
...
and map_struct_declaration (env: env) (x: CST.struct_declaration): G.definition =
  let map_fields (env: env) (x: CST.member_declaration_list): G.field list G.bracket =
    let map_field (x: CST.member_declaration): G.field =
      match x with
      | `Rep_attr_list_choice_pat_7fdeb71_COLON_choice_type_tuple (attributes, name, _, ttype) ->
        let entity: G.entity = {
          name = map_name_to_entity_name env name;
          attrs = map_attributes env attributes;
          tparams = [];
        ...
        let definition = (entity, (G.FieldDefColon {
          vinit = None;
...
and map_enum_declaration (env: env) (x: CST.enum_declaration) =
  let map_variants (env: env) (x: CST.member_declaration_list): G.field list G.bracket =
    let map_variant (x: CST.member_declaration): G.field =
      ...
      | `Rep_attr_list_choice_pat_7fdeb71_COLON_choice_type_tuple (attributes, name, _, ttype) ->
        ...
        let variant = (entity, G.EnumEntryDef { 
          ee_args = Some (G.fake "(", [G.ArgType (map_type env ttype)], G.fake ")");
          ee_body = None
        }) in
  ...
  let entity: G.entity = {
      name = map_name_to_entity_name env name;
      attrs = (G.KeywordAttr (G.EnumClass, token env enum)) :: (map_attributes env attributes);
      tparams = match type_parameters with
      | Some type_parameters -> map_type_parameters env type_parameters
      | None -> []
  } in
