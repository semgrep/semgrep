...
let position_of_token_location loc =
  {
    line = loc.Tok.line;
    col = adjust_column loc.Tok.column;
    offset = loc.Tok.bytepos;
  }
...
let position_range min_loc max_loc =
  ...
  ( {
      line = min_loc.Tok.line;
      col = adjust_column min_loc.Tok.column;
      offset = min_loc.Tok.bytepos;
    },
...
let location_of_token_location loc =
  ...
  { path = Fpath.v loc.Tok.file; start; end_ }
