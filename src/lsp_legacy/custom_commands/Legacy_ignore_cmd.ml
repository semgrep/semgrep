...
let command_handler (session : Legacy_session.t) (arg_list : Yojson.Safe.t list)
    =
  let args =
    match arg_list with
    | args :: _ -> args
  ...
  | Ok { path; fingerprint } ->
      let session =
        let session =
          Legacy_session.add_skipped_fingerprint session fingerprint
        in
        let metrics =
          {
      ...
      in
      ( session,
        Some (Legacy_scan_helpers.scan_file session Lsp.Uri.(of_path path)) )
  | Error e ->
