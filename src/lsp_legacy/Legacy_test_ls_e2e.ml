...
type server_info = {
  server : Legacy_rpc_server.t;
  handler : Legacy_rpc_server.handler;
}
  ...
  let handler : Legacy_rpc_server.handler =
    {
      ...
      on_notification = Legacy_notification_handler.on_notification;
    }
  in
  { server; handler }
...
let send (info : server_info) packet : server_info * Reply.t =
  ...
  | Legacy_lsp_.State.Stopped -> Alcotest.failf "Cannot send, server stopped"
  | _ -> (
      let server, reply_result =
        Legacy_rpc_server.handle_client_message ~handler:info.handler
          info.server packet
      in
...
let send_semgrep_search info ?language ~includes ~excludes pattern =
  let params =
    Legacy_search.mk_params ~lang:language ~fix:None ~includes ~excludes pattern
  in
