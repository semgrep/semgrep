...
let initialize_server server
  ...
  let metrics =
    let client_metrics =
      ...
      |> Legacy_ls_metrics.client_metrics_of_yojson
      |> Result.value ~default:Legacy_ls_metrics.client_metrics_default
    in
    { server.session.metrics with client_metrics }
  in
  let server =
    {
      Legacy_rpc_server.session =
        {
          server.session with
          workspace_folders;
          user_settings;
