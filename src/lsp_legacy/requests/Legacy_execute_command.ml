...
let supported_commands =
  [ Legacy_ignore_cmd.command; Legacy_autofix_cmd.command ]

let handle_execute_request (session : Legacy_session.t) (command : string)
    arg_list : Legacy_session.t * Legacy_lsp_.Reply.t option =
  match
    [
      (Legacy_ignore_cmd.command, Legacy_ignore_cmd.command_handler);
      (Legacy_autofix_cmd.command, Legacy_autofix_cmd.command_handler);
    ]
