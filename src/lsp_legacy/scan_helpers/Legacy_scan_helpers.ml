...
let run_semgrep ?(targets : Fpath.t list option) ?rules ?git_ref
  ...
  | _ ->
      ...
      let matches =
        ...
        (fun () ->
          Legacy_processed_run.of_matches ~skipped_fingerprints ?git_ref
            ~only_git_dirty res)
      ...
      Legacy_session.send_metrics session ?core_time:res.core.time ~profiler
        ~cli_output;
      (matches, scanned)

let run_semgrep_detached ?targets ?rules ?git_ref (session : Legacy_session.t) =
  Lwt_platform.detach
...
let scan_something ?targets session =
  ...
  let files = Legacy_session.scanned_files session in
  Lwt.return
    (Legacy_diagnostics.diagnostics_of_results ~is_intellij:session.is_intellij
       results files)
...
let scan_open_documents (session : Legacy_session.t) =
  Logs.app (fun m -> m "Scanning open documents");
  let session_targets = Legacy_session.targets session in
  let open_documents =
  ...
  let get_diagnostics () =
    ...
    Lwt.return
      (Legacy_diagnostics.diagnostics_of_results
         ~is_intellij:session.is_intellij results files)
  in
...
let refresh_rules session =
  Reply.later (fun send ->
      ...
      let%lwt () = Legacy_session.cache_session session in
      let%lwt () = send (end_progress token) in
      let%lwt () =
