...
let next_rules_and_file (session : Legacy_session.t) =
...
let rec search_single_target (session : Legacy_session.t) =
...
let start_search (session : Legacy_session.t) (id : Jsonrpc.Id.t)
  ...
  | None ->
      ...
      (session, Legacy_lsp_.Reply.empty)
  | Some params -> (
      ...
      | Ok rules ->
          ...
          in
          (session, Legacy_lsp_.Reply.now (Legacy_lsp_.respond_json id json)))

let search_next_file (session : Legacy_session.t) (id : Jsonrpc.Id.t) _params :
