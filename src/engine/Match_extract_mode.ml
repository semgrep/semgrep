...
let extract_and_concat erule_table (xtarget : Xtarget.t)
...
let extract_as_separate erule_table (xtarget : Xtarget.t)
    (matches : Pattern_match.t list) : extracted_target_and_adjuster list =
  matches
  |> Common.map_filter (fun m ->
         match extract_of_match erule_table m with
...
let extract ~match_hook ~timeout ~timeout_threshold
  ...
  in
  let separate = extract_as_separate erule_table xtarget separate_matches in
  let combined = extract_and_concat erule_table xtarget combine_matches in
  separate @ combined
