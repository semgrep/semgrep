...
let is_relevant_rule_for_xtarget hmemo r xconf xtarget =
  ...
  let is_relevant =
    if xconf.filter_irrelevant_rules then (
      match Analyze_rule.regexp_prefilter_of_rule ~hmemo r with
      | None -> true
...
let group_rules xconf rules xtarget =
  ...
  let relevant_taint_rules, relevant_nontaint_rules, skipped_rules =
    ...
    |> Common.partition_either3 (fun r ->
           let relevant_rule =
             is_relevant_rule_for_xtarget hmemo r xconf xtarget
           in
           match r.R.mode with
