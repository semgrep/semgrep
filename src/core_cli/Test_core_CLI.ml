...
let run_main (caps : Cap.all_caps) base (cmd : string) : (unit, exn_res) result
    =
  ...
  CapProcess.apply_in_child_process
    ...
    (fun () ->
      try
        ...
        Ok
          (Core_CLI.main_exn caps base (Array.of_list ("semgrep-core" :: args)))
      with
...
let semgrep_core_tests (caps : Cap.all_caps) base : Testo.t list =
  Testo.categorize "semgrep-core CLI (e2e)"
    [
      t "--help" (fun () ->
          match run_main caps base "handle --help" with
      ...
      t "handle -rules <rule> -l <lang> <single_file>" (fun () ->
          ...
          in
          run_main caps base cmd |> assert_Ok);
...
let tests (caps : Cap.all_caps) base : Testo.t list =
  semgrep_core_tests caps base
