...
let shebang_re = lazy (Regex.regexp "^#![ \t]*([^ \t]*)[ \t]*([^ \t].*)?$")
let split_cmd_re = lazy (Regex.regexp "[ \t]+")
...
let parse_shebang_line s =
  let matched = Regex.exec_noerr ~rex:(Lazy.force shebang_re) s in
  match matched with
  | None -> None
  | Some matched -> (
      ...
      | [| _; "/usr/bin/env" as arg0; arg1 |] -> (
          ...
          | Some packed_args ->
              let args =
                Regex.split_noerr ~rex:(Lazy.force split_cmd_re)
 ...
 *)
let regexp pat =
  let rex = Regex.regexp pat in
  let f path =
    let s = get_first_block path in
    Regex.pmatch_noerr ~rex s
  in
  Test_path f
