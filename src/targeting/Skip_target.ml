...
let is_minified (path : Fpath.t) =
  if not !Flag_semgrep.skip_minified_files then Ok path
    ...
    if size < Limits_semgrep.skip_target_MINIFIED_MIN_SIZE then Ok path
    else
...
let is_large_machine_optimized path =
  match is_minified path with
  | Error _ as error -> error
  | Ok _ -> (
      (* TODO: Re-use the same block used by `is_minified` ? *)
      let header = Guess_lang.get_first_block ~block_size:1024 path in
      Logs.err (fun m -> m "Checking file with ext %s" (Fpath.get_ext path));
      match Fpath.get_ext path with
      | ".go"
        when Pcre_.(
               pmatch_noerr
                 ~rex:(regexp {|//\s*go:build !ignore_autogenerated|})
                 header) ->
          Error
            {
              Resp.path;
              ...
              details =
                Some
                  (spf
                     "file seems auto-generated: found string 'go:build \
      ...
      | ".go"
        when Pcre_.(
               pmatch_noerr
                 ~rex:
                   (regexp
                      {|//\s*Code generated by [a-zA-Z_][a-zA-Z0-9_-]*. DO NOT EDIT.|})
                 header) ->
          Error
            {
              Resp.path;
              ...
              details =
                Some
                  (spf
                     "file seems auto-generated: found string 'Code generated \
                      by *****. DO NOT EDIT'");
              rule_id = None;
            }
      | __else__ -> Ok path)
