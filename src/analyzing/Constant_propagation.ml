  ...
  object (self : 'self)
    ...
    method! visit_definition (env, ctx) x =
      ...
      | { name = EN (Id (id, _ii)); _ }, FuncDef _fdef -> (
          ...
          | Some _
          | None ->
              self#with_context_visit_definition (env, ctx) x (* ctx *))
    ...
    method with_context_visit_assign_lhs (env, ctx) x =
      self#with_context_visit_expr (env, ctx) x
    ...
    method! visit_expr (env, ctx) x =
      ...
      | Assign (e1, _, e2)
      | AssignOp (e1, _, e2) ->
          self#with_context_visit_assign_lhs
            (env, { ctx with in_lvalue = true })
            e1;
...
let stats_of_prog prog : stats =
  ...
  let visitor =
    object (_self : 'self)
      ...
      method! with_context_visit_stmt (env, ctx) x =
        (match x.s with
        | For
            ( _,
              ForEach
            ...
            incr stat.lvalue
        | _ -> ());
        super#with_context_visit_stmt (env, ctx) x

      method! with_context_visit_assign_lhs (env, _ctx) x =
      ...
      method! with_context_visit_expr (env, ctx) x =
        match x.e with
        | Call
        ...
        | N (Id (id, { id_resolved = { contents = Some (_kind, sid) }; _ })) ->
            ...
            let stat = get_stat_or_create var env.var_stats in
            incr stat.rvalue;
            super#with_context_visit_expr (env, ctx) x (* ??? *)
        | _ -> super#with_context_visit_expr (env, ctx) x
    end
  in
