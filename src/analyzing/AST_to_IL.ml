...
let lval_of_ret env tok ret =
  match ret with
  | `Var name -> lval_of_base (Var name)
  | `Tmp -> fresh_lval env tok
  | `Void ->
      ...
      fresh_lval ~str:"_FIXME_expected_void" env tok

let add_call env tok eorig ~ret mk_call : exp =
  match ret with
  | `Void ->
      add_instr env (mk_i (mk_call None) eorig);
      mk_unit tok NoOrig
  | (`Tmp | `Var _) as ret ->
      let lval = lval_of_ret env tok ret in
...
and map_assign_rhs_minimizing_tmps env lval rhs eorig =
  ...
  | Some name -> (
      (* If the l-value is a simple variable, then should not create
         ...
         it in that variable! *)
      let rhs_exp = map_assign_rhs env ~ret:(`Var name) rhs in
...
and map_assign env ~g_expr lhs tok rhs : exp =
  ...
  | G.DeRef _ -> (
      ...
      | Fixme (kind, any_generic) ->
          ...
          let rhs_exp = map_assign_rhs env ~ret:(`Var fixme_var) rhs in
  ...
  | G.Container (((G.Tuple | G.List | G.Array) as ckind), (tok1, lhss, tok2)) ->
      ...
      let tmp = fresh_var env tok2 in
      let tmp_lval = lval_of_base (Var tmp) in
      let rhs_exp = map_assign_rhs env ~ret:(`Var tmp) rhs in
...
and map_expr_aux env ?(ret = `Tmp) g_expr : exp =
  ...
  | G.Call ({ e = G.Special (G.Op op, tok); _ }, args) -> (
      ...
      if ret <> `Void then mk_e (Operator ((op, tok), args)) eorig
        ...
        | obj :: args' ->
            ...
            add_call env tok eorig ~ret (fun res -> Call (res, method_, args')))
  | G.Call
      ( ({
           e =
             G.N
  ...
  | G.Lambda fdef ->
      ...
      let lval = lval_of_ret env (snd fdef.fkind) ret in
      let fdef =
...
and map_stmt_aux env st : stmt list =
  ...
  | G.DefStmt (ent, G.VarDef { G.vinit = Some e; vtype = opt_ty; vtok = _ }) ->
      ...
      let ss1, needs_assign_st =
        ...
        | Some name -> (
            let ss, e' = map_expr_with_pre_stmts env ~ret:(`Var name) e in
      ...
      let assign_st =
        ...
        | Some e' ->
            [ mk_s (Instr (mk_i (Assign (lv, e')) (Related (G.S st)))) ]
        | None -> []
      in
      let ss2 = map_type_opt_with_pre_stmts env opt_ty in
      ss1 @ ss2 @ assign_st
