...
let scan_subset_cmdline_term : Scan_CLI.conf Term.t =
  ...
  let combine allow_dynamic_dependency_resolution allow_untrusted_validators
      ...
      gitlab_secrets_outputs _historical_secrets ignore_semgrepignore_files
      include_ incremental_output json json_outputs junit_xml junit_xml_outputs
  ...
  Term.(
    ...
    $ SC.o_historical_secrets $ SC.o_ignore_semgrepignore_files $ SC.o_include
