...
let o_x_dump_scan_config_path : string Term.t =
  let info =
    Arg.info
...
let o_x_use_saved_scan_config_path : string Term.t =
  let info =
    Arg.info
...
let scan_subset_cmdline_term : Scan_CLI.conf Term.t =
  ...
  let combine allow_local_builds allow_untrusted_validators autofix
      ...
      _x_gradle_module_attribution x_mem_policy x_tr x_pro_naming x_mcp
      x_run_taint_once =
  ...
  Term.(
    ...
    $ SC.o_vim_outputs $ SC.o_x_dependency_paths
    $ SC.o_x_gradle_module_attribution $ SC.o_x_mem_policy $ SC.o_x_tr
