...
let login_url_regex =
  Regex.pcre_compile {|https://semgrep.dev/login\?cli-token=.*"|}

let prog_regex = Regex.pcre_compile {|Pr([\s\S]*)|}
      ...
      Testutil_files.with_chdir root (fun () ->
          ...
          let%lwt () =
            ...
            |> Lwt_list.iter_s (fun file ->
                   ...
                   in
                   assert (Regex.unanchored_match prog_regex resp);
                   Lwt.return_unit)
          in
  ...
  with_session caps (fun info ->
      ...
      *)
      ...
      Testutil_files.with_chdir root (fun () ->
          ...
          assert (Regex.unanchored_match login_url_regex url);
