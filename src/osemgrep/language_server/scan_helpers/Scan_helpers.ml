...
let run_semgrep ?(targets = None) ?(rules = None) ?(git_ref = None)
  ...
  let res =
    ...
    in
    scan_func ~respect_git_ignore:false ~file_match_results_hook:None
...
let scan_file ?(content = None) server uri =
  ...
  let results =
    match content with
    | Some _ ->
        let existing_results, _ = run_semgrep ~targets:(Some [ file ]) server in
        results @ existing_results
    | None -> results
  in
  let results =
    Common.map (fun (m : Out.cli_match) -> { m with path = file_path }) results
  in
  let files = [ file ] in
  Session.record_results server.session results files;
  let diagnostics =
