...
ACC.assistedservicepromotion = {
  bindAll: function () {
    $(document).on(
      "click",
      ".asm-customer360-promotions-addToCart",
      function (e) {
        ACC.assistedservicepromotion.handleCouponToCartAction(
          "apply",
          this,
          "Failed to Apply to cart"
        );
      }
    );
    $(document).on(
      "click",
      ".asm-customer360-promotions-removefromCart-remove",
      function (e) {
        ACC.assistedservicepromotion.handleCouponToCartAction(
          "remove",
          this,
  ...
  handleCouponToCartAction: function (couponToCartAction, element, errorMsg) {
    ...
    var couponUrl =
      ACC.config.encodedContextPath + "/cart/voucher/" + couponToCartAction;
    return $.ajax({
      url: couponUrl,
      type: "POST",
      data: params,
      success: function (data) {
        ...
        $(
          ".asm-customer360-promotions-addToCart[data-index=" + index + "]"
        ).toggleClass(cssClassToToggle);
        $(
          ".asm-customer360-promotions-removefromCart[data-index=" + index + "]"
        ).toggleClass(cssClassToToggle);
      },
...
(function () {
  ...
  window.smartedit.addOnReprocessPageListener = function (callbackFn) {
    if (!_isFunction(callbackFn)) {
      throw new Error(
        "SmartEditAddon - Cannot register page reprocessing listener. Provided callback must be a function."
      );
    }
...
(function () {
  if (window.Imager) {
    Imager.prototype.replaceImagesBasedOnScreenDimensions = function (image) {
      ...
      cwidth = Imager.getClosestValue(
        window.innerWidth,
        $.extend([], $(image).data("width"))
      );
  ...
  postNewMessage: function () {
    ...
    $(".ct-add-new-msg-btn").on("click touchstart", function (e) {
      ...
      $.colorbox({
        ...
        onComplete: function () {
          ACC.customerticketingaddon.disableMessage(
            $(".js-add-message-status")
          );
          if (!$.trim($("#message").val())) {
            $('button[id="updateTicket"]').attr("disabled", "disabled");
          }
  ...
  bindTicketAddActions: function () {
    $(document).on("click", "#addTicket", function (event) {
      ...
      ACC.customerticketingaddon.formPostAction(
        "support-tickets?ticketAdded=true"
      );
    });
  },
...
(function () {
  var listOfActions = Array.prototype.slice.call(
    document.querySelectorAll(
      '[class^="AccountOrderDetailsOverviewComponent"]'
    ),
    0
  );
  ...
  if (firstVisibleAction) {
    var target = firstVisibleAction.querySelectorAll(
...
ACC.cancelorderaction = {
  _autoload: [
    [
      "bindToCancelCompleteOrderButton",
      $(".js-cancel-complete-order-link").length != 0,
    ],
  ...
  bindToCancelCompleteOrderButton: function () {
    $(document).on("click", ".js-cancel-complete-order-link", function (event) {
      ...
      $.each($('[id^="item_quantity_"]'), function (i) {
        $('[name^="cancelEntryQuantityMap[' + i + ']"]').val(
          $("#item_quantity_" + i).val()
        );
      });
  ...
  disableEnableCancelSubmit: function () {
    ...
    $.each($('[id^="item_quantity_"]'), function (i) {
      if (
        parseInt($('[name^="cancelEntryQuantityMap[' + i + ']"]').val()) > 0
      ) {
...
ACC.returnorderaction = {
  _autoload: [
    [
      "bindToReturnCompleteOrderButton",
      $(".js-return-complete-order-link").length != 0,
    ],
  ...
  bindToReturnCompleteOrderButton: function () {
    $(document).on("click", ".js-return-complete-order-link", function (event) {
      event.preventDefault();
      $.each($('[id^="item_quantity_"]'), function (i) {
        $('[name^="returnEntryQuantityMap[' + i + ']"]').val(
          $("#item_quantity_" + i).val()
        );
      });
  ...
  disableEnableReturnSubmit: function () {
    ...
    $.each($('[id^="item_quantity_"]'), function (i) {
      if (
        parseInt($('[name^="returnEntryQuantityMap[' + i + ']"]').val()) > 0
      ) {
        submitDisabled = false;
      }
    });
        ...
        $(this).after(
          $("<div>")
            ...
            .append(
              ...
              $("<span>").addClass("bpNo").text(item.bpNo),
      ...
      this.construct = function (settings) {
        return this.each(function () {
          ...
          var numPages = Math.ceil(
            $(table).find("tbody tr").length / config.size
          );
          if (numPages > 1) {
            while (numPages > curr) {
              $(
...
ASM.storefinder = {
  ...
  createListItemHtml: function (data, id) {
    ...
    item +=
      ...
      data.displayName +
      '" id="asm-store-filder-entry-' +
      id +
      '" class="js-asm-store-finder-input" data-id="' +
      id +
      '">';
    item +=
      '<label for="asm-store-filder-entry-' +
      id +
      '" class="js-select-store-label">';
    item += '<span class="entry__info">';
    item += '<span class="entry__name">' + data.displayName + "</span>";
    item +=
      '<span class="entry__address">' +
      data.line1 +
      " " +
      data.line2 +
      "</span>";
  ...
  refreshNavigation: function () {
    ...
    var to =
      page * 10 + 10 > ASM.storefinder.storeData.total
        ? ASM.storefinder.storeData.total
        : page * 10 + 10;
    $(".js-asm-store-finder-pager-item-to").html(to);
    $(".js-asm-store-finder-pager-item-all").html(
      ASM.storefinder.storeData.total
    );
  ...
  loadGoogleMap: function () {
    ...
    if ($(".js-asm-store-finder-map").length > 0) {
      ...
      var map = new google.maps.Map(
        document.getElementById("asm-store-finder-map"),
        mapOptions
      );
      ...
      if (
        ...
        ASM.storefinder.originalStore["latitude"] !=
          storeInformation["latitude"]
      ) {
        var directionsDisplay = new google.maps.DirectionsRenderer();
        var directionsService = new google.maps.DirectionsService();
        var originPoint = new google.maps.LatLng(
          ASM.storefinder.originalStore["latitude"],
          ASM.storefinder.originalStore["longitude"]
        );
      ...
      map.addListener("click", function (e) {
        if (ACC.config.googleApiKey != "") {
          ...
          window.open(
            "https://www.google.de/maps/dir/" + markerPosition,
            "_blank"
          );
        }
      });
  ...
  bindSearch: function () {
    $(document).on("submit", "#storeFinderForm", function (e) {
      ...
      if (q.length > 0) {
        ...
        if ($(".js-asm-storefinder-alert").length < 1) {
          ...
          $("#storeFinder").before(
            '<div class="js-asm-storefinder-alert alert alert-danger alert-dismissable" ><button class="close" type="button" data-dismiss="alert" aria-hidden="true">×</button>' +
              emptySearchMessage +
              "</div>"
          );
        }
    ...
    $(document).on("click", "#findStoresNearMe", function (e) {
      e.preventDefault();
      ASM.storefinder.getInitStoreData(
        null,
        ASM.storefinder.coords.latitude,
        ASM.storefinder.coords.longitude
      );
    });
  },
