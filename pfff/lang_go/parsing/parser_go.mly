...
sgrep_spatch_pattern: 
 ...
 | item LSEMICOLON item LSEMICOLON item_list EOF 
    { pr2 "item2"; Items ($1 @ $3 @ rev_and_fix_items $5) }

item: 
 | stmt   { pr2 "stmt"; [IStmt $1] }
 | import { $1 |> List.map (fun x -> IImport x) }
 | xfndcl { pr2 "itop"; [ITop $1] }
 | package { pr2 "package"; [ITop $1] }
...
(* stmt_list requires ; as a separator, so Go allows empty stmt
   ...
   *)
| (*empty*)   { pr2 "empty"; Empty }

| compound_stmt   { pr2 "compound"; $1 }
| common_dcl      { pr2 "common"; DeclStmts $1 }
| non_dcl_stmt    { pr2 "non dcl"; $1 }

compound_stmt: "{" stmt_list "}" 
  { Block ($1, rev_and_fix_stmts $2, $3) }

non_dcl_stmt:
|   simple_stmt { pr2 "simple statemnt"; SimpleStmt $1 }
