...
(** Similar to Commons.memoized, but also consumes a mutex in order to ensure
   ...
   a single [Mutex.t] + [Base.Hashtbl.t]) can evolve e.g., to a striped /
...
type ('k, 'v) t = { mtx : Mutex.t; ht : ('k, 'v) Base.Hashtbl.t }

let create ?(initial_size = 101) () =
  { mtx = Mutex.create (); ht = Base.Hashtbl.Poly.create ~size:initial_size () }

let remove t k = Mutex.protect t.mtx (fun () -> Base.Hashtbl.remove t.ht k)
let length t = Mutex.protect t.mtx (fun () -> Base.Hashtbl.length t.ht)
...
let iter f t =
  ...
  let snapshot =
    Mutex.protect t.mtx (fun () ->
        Base.Hashtbl.fold t.ht ~init:[] ~f:(fun ~key ~data acc ->
            (key, data) :: acc))
  in
...
let call_and_remember ?(should_cache = Fun.const true) t key_fn f k =
  ...
  match Mutex.protect t.mtx (fun () -> Base.Hashtbl.find t.ht k') with
  | Some v -> v
  | None ->
     ...
     *)
      ...
      if not (should_cache v) then v
        ...
        Mutex.protect t.mtx (fun () ->
            match Base.Hashtbl.find t.ht k' with
            (* Someone beat us to the insert! So it goes; discard our copy. *)
            | Some v' -> v'
            | None ->
                Base.Hashtbl.set t.ht ~key:k' ~data:v;
                v)
