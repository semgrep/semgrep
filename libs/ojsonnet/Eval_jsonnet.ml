...
let rec lookup (env : V.env) tk local_id =
  let entry =
    ...
    | Not_found ->
        error tk (spf "could not find '%s' in the environment" local_id)
  in
  match entry with
  | Nonrec (lazy v) -> v
  | Rec binds -> (
      ...
      | Some { body; env } ->
          eval_expr
            (V.bind_all env
               (Common.map (fun (id, _) -> (id, V.Rec binds)) binds))
            body
...
and eval_expr (env : V.env) (v : expr) : V.value =
  ...
  | Array (l, xs, r) ->
      let elts =
        xs |> Common.map (fun x -> lazy (eval_expr env x)) |> Array.of_list
      in
      Array (l, elts, r)
  ...
  | IdSpecial (Self, tk) -> (
      match env.self with
      | Self self
      | SelfWithSuper { self; _ } ->
          Object self
      | NoSelf -> error tk "cannot access `self` outside of object")
  | IdSpecial (Super, tk) -> error tk "super is not first class"
  (* begin
       match env.self with
         | SelfWithSuper {super; _} -> Object super
         | Self _ -> error tk "cannot access `super` in object with no parent"
         | NoSelf -> error tk "cannot access `super` outside of object"
     end *)
  | Call
      ( (ArrayAccess
  ...
  | Local (_, binds, _, e) ->
      let bind =
        V.Rec
          (binds
          |> Common.map (fun (B (id, _, body)) -> (fst id, V.{ env; body })))
      in
      eval_expr
        (V.bind_all env
           (Common.map (fun (B (id, _, _)) -> (fst id, bind)) binds))
        e
  | ArrayAccess (IdSpecial (Super, _), index) ->
      eval_super_access env (eval_bracket eval_expr env index)
  | ArrayAccess (v1, v2) ->
      eval_array_access env (eval_expr env v1) (eval_bracket eval_expr env v2)
  | Call (f, args) ->
      eval_call env (eval_expr env f) (mk_fstr f)
        (eval_bracket eval_args env args)
  | UnaryOp ((op, tk), e) -> (
      match op with
      | UBang -> (
...
and eval_std_method env e0 (method_str, tk) (l, args, r) =
  ...
  | "makeArray", [ Arg e; Arg e' ] -> (
      ...
      | Primitive (Double (n, tk)), Lambda fdef ->
          if Float.is_integer n then
            ...
            in
            Array (fk, Array.init n (fun i -> lazy (eval_expr env (e i))), fk)
  ...
  | "objectHasEx", [ Arg e; Arg e'; Arg e'' ] -> (
      ...
      | V.Object o, Primitive (Str (s, _)), Primitive (Bool (b, _)) ->
          ...
          let eltopt =
            field_names
            |> List.find_opt (fun { V.fld_name; _ } -> fst fld_name = s)
          in
          let b =
            match eltopt with
            | None -> false
  ...
  | _else_ ->
      eval_call env (eval_expr env e0) (mk_fstr e0)
        (eval_bracket eval_args env (l, args, r))

and search_layers level fld = function
  | [] -> None
  | { V.fields; _ } :: layers -> (
      match
        fields |> List.find_opt (fun { V.fld_name = fld', _; _ } -> fld = fld')
      with
      | Some field -> Some (field, level)
      | None -> search_layers (level + 1) fld layers)

and access_field tk ((l, self_obj, r) as self) (_, obj, _) fld =
  match
    obj.V.field_names
    |> List.exists (fun { V.fld_name = fld', _; _ } -> fld = fld')
  with
  | false ->
      error tk
        (spf "field '%s' not present in %s" fld
           (V.show_field_names obj.V.field_names))
  | true -> (
      ...
      | None ->
          error tk
            (spf
               "Internal error:\n\
                field '%s' present in field_names but not present in %s" fld
               (obj.V.layers |> V.show_layers))
      | Some ({ fld_value = { body; env }; _ }, lvl) ->
          let self =
            match Common.drop (lvl + 1) self_obj.V.layers with
            | [] -> V.Self self
            | super_layers ->
                V.SelfWithSuper
                  {
                    self;
                    super = (l, { self_obj with layers = super_layers }, r);
                  }
          in
          eval_expr { env with self } body)
...
and eval_super_access env (l, index, _) =
  ...
  | Primitive (Str (fld, tk)) ->
      let self, super =
        match env.self with
        | SelfWithSuper { self; super } -> (self, super)
        | Self _ -> error tk "cannot access `super` in object with no parent"
        | NoSelf -> error tk "cannot access `super` outside of object"
      in
      access_field tk self super fld
...
and eval_array_access _env e (l, index, _) =
  ...
  | V.Object obj, Primitive (Str (fld, tk)) -> access_field tk obj obj fld
...
and eval_call env f (fstr : string) (largs, args, _rargs) =
  match f with
  | Lambda
      { env = env'; body = { f_tok = _; f_params = _, params, _; f_body = eb } }
    ->
...
and eval_plus_object _env _tk (l, objl, _) (_, objr, r) : V.object_ A.bracket =
  let join_hidden ~oldh ~newh =
    ...
    | Hidden, ForcedVisible ->
        Visible
  in
  let rfields =
    objr.V.field_names
    |> Common.map
         (fun (V.{ fld_name = fld, _; fld_hidden = hiddenr, htk; _ } as field)
         ->
           match
             List.find_opt
               (fun V.{ fld_name = fld', _; _ } -> fld = fld')
               objl.V.field_names
           with
           | None -> field
           | Some { fld_hidden = hiddenl, _; _ } ->
               {
                 field with
                 fld_hidden = (join_hidden ~oldh:hiddenl ~newh:hiddenr, htk);
               })
  in
  ...
  let field_names =
    rfields
    @ (objl.V.field_names
      |> List.filter (fun V.{ fld_name = fld, _; _ } ->
             not
             @@ List.exists
                  (fun V.{ fld_name = fld', _; _ } -> fld = fld')
                  rfields))
...
and eval_obj_inside env (l, x, r) : V.value =
  ...
  | Object (assertsTODO, fields) ->
      ...
      let field_names =
        fields |> Common.map (fun fld -> V.{ fld with fld_value = () })
      in
...
and manifest_value (v : V.value) : JSON.t =
  ...
  | Lambda { body = { f_tok = tk; _ }; _ } ->
      error tk (spf "Lambda value: %s" (sv v))
  | Array (_, arr, _) ->
      J.Array
        (arr |> Array.to_list
        |> Common.map (fun entry -> manifest_value (Lazy.force entry)))
  | V.Object ((_l, { field_names; _ }, _r) as self) ->
      ...
      let xs =
        ...
        |> Common.map_filter (fun { V.fld_name; fld_hidden; _ } ->
               ...
               | A.ForcedVisible ->
                   let j =
                     manifest_value (access_field fk self self (fst fld_name))
                   in
                   Some (fst fld_name, j))
      in
      J.Object xs
