 ...
 *)
 ...
 and eval_expr (env : V.env) (v : expr) : V.value =
   ...
   | Local (_tlocal, B (id,_,b), _tsemi, e) ->
      eval_expr (V.bind env (fst id) (lazy (eval_expr env b))) e
   | ArrayAccess (IdSpecial (Super,tk),index) -> eval_super_access env tk (eval_bracket eval_expr env index)
       ...
       | Lambda f, Array (l, eis, r) ->
           ...
           let elts' =
             ...
             |> List.filter_map (fun (ei, ji) ->
                    ...
                    | V.Primitive (Bool (false, _)), _ -> None
  ...
  *)
  and eval_std_filter_element _env = failwith "eval_std_filter_element"
 (* and eval_std_filter_element (env : V.env) (tk : tok) (f : function_definition)
     (ei : V.lazy_value) : V.value * V.env =
   match f with
 ...
 and access_field tk self ((_,{V.fields; _},_) as _e) fld =
  match
    fields
    |> List.find_opt (fun field ->
            fst field.V.fld_name = fld)
  with
    | None -> error tk (spf "field '%s' not present in %s" fld (V.show_field_names fields))
    | Some {fld_value = {body; env}; _} -> eval_expr {env with self = Some self} body

 and eval_super_access env tk (l,index,_) =
    let self,super_fields = 
      match env.self with
        | Some ((_,{super = Some o; _},_) as self) -> self, o
        | Some (_,{super = None; _}, _) -> error tk "current object has no `super`"
        | None -> error tk "cannot access `super` outside of object"
    in
    match index with
      | Primitive (Str (fld, tk)) -> access_field tk self super_fields fld
...
and eval_array_access _env e (l,index,_) =
 ...
 and eval_call env e0 (largs, args, _rargs) =
  ...
  and eval_plus_object _env _tk ((l,objl,_) as objl_bracket) (_,objr,r) : V.object_ A.bracket =
    ...
    in
    let fields = lflds' @ objr.V.fields in
    (l,{V.asserts; fields; super = Some objl_bracket},r)
 ...
 and eval_obj_inside env (l, x, r) : V.value =
   ...
   | Object (assertsTODO, fields) ->
      ...
      V.Object (l, {fields ; asserts ; super = None}, r)
 ...
 *)
 ...
 and eval_program_with_env (env : V.env) (e : Core_jsonnet2.program) : V.value =
   eval_expr env e
   [@@profiling]
 
 and eval_program (e : Core_jsonnet2.program) : V.value =
   ...
   | V.Object ((_l, {fields ; _}, _r) as self) ->
       ...
       let xs =
         ...
         |> Common.map_filter (fun { V.fld_name; fld_hidden; fld_value } ->
                ...
                | A.Visible
                | A.ForcedVisible ->
                    let j = manifest_value (eval_expr {fld_value.env with self = Some self} fld_value.body) in
                    Some (fst fld_name, j))
       in
       J.Object xs
