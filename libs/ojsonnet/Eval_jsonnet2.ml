 ...
 *)
 ...
 and eval_expr (env : V.env) (v : expr) : V.value =
   ...
   | IdSpecial (Self, tk) -> V.Self tk
   | IdSpecial (Super, tk) ->
    begin
      match env.self with
        | None -> error tk "Cannot access super outside of an object"
        | Some (_, { super = None; _}, _) -> error tk "Current object has no super class"
        | Some (_, { super = Some obj; _}, _) -> Object obj
     end
   | Call
       ( (ArrayAccess
   ...
   | Local (_tlocal, binds, _tsemi, e) ->
      let locals =
       let locals =
         binds
         |> List.fold_left
              (fun locals (B (id, _teq, e_i)) ->
                let binding = lazy (eval_expr {env with locals} e_i) in
                Map_.add (fst id) binding locals)
              env.locals
       in
       eval_expr { env with locals } e
   | ArrayAccess (v1, v2) ->
       let e = eval_expr env v1 in
       let index = (eval_bracket eval_expr) env v2 in
       eval_array_access env e index
 ...
 and eval_std_filter_element (env : V.env) (tk : tok) (f : function_definition)
     (ei : V.value Lazy.t) : V.value * V.env =
   match f with
   | { f_params = _l, [ P (id, _eq, _default) ], _r; f_body; _ } ->
       (* similar to eval_expr for Local *)
       let locals = Map_.add (fst id) ei env.locals in
...
and eval_array_access env e ((l, index, _r) as index_w_tk) =
  match (e, index) with
  ...
  | (V.Object (_l, {fields; _}, _r) as obj,
      ...
      | Some fld -> Lazy.force fld.fld_value
    end
  | (V.Self tk, (Primitive (Str _))) ->
    begin
      match env.self with
        | Some obj -> eval_array_access env (V.Object obj) index_w_tk
        | None -> error tk "Cannot access self outside of an object"
    end
 ...
 and eval_plus_object _env _tk _objl _objr : V.object_ A.bracket =
   failwith "TODO: eval_plus_object"
 ...
 and eval_obj_inside env (l, x, r) : V.value =
   ...
   | Object (assertsTODO, fields) ->
    let fields =
      fields |> Common.map_filter (
        fun {fld_name = FExpr (tk, field, _); fld_hidden; fld_value} ->
          match eval_expr
      )
      in
      failwith ""
