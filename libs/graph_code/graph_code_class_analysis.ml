  ...
  |> G.iter_nodes (fun n1 ->
         ...
         | E.Class ->
             ...
             |> List.iter (fun n2 ->
                    ...
                    | _ ->
                        failwith
                          (spf "class_hierarchy: impossible edge %s --> %s"
...
let toplevel_methods g dag =
  ...
  let rec aux env n =
    ...
    in
    methods_here
    |> List.iter (fun (s, priv, n2) ->
           if Set.mem s env then ()
           else if
    ...
    let env =
      methods_here |> List.fold_left (fun acc (s, _p, _) -> Set.add s acc) env
    in
...
let dispatched_methods g dag node =
  ...
  if G.has_node node g then Graphe.succ node dag |> List.iter aux
