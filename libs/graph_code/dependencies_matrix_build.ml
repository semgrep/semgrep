...
let count_column j m =
  ...
  let cnt = ref 0 in
  for i = 0 to n - 1 do
    if m.(i).(j) > 0 && i <> j then cnt := !cnt + formula m.(i).(j)
  done;
  !cnt
...
let reduced_matrix nodes dm =
  ...
  for i = 0 to n - 1 do
    for j = 0 to n - 1 do
      ...
      if i <> j then m.(i).(j) <- dm.matrix.(xi).(xj)
    done
  done;
  (a, m)
...
let partition_matrix nodes dm =
  ...
  let rec step1 nodes =
    ...
    let elts_with_empty_columns, rest =
      nodes |> List.partition (fun node -> is_empty_column node m dm)
    in
    let xs =
      sort_by_count_rows_low_first elts_with_empty_columns dm.matrix dm
    in
  ...
  and step2 nodes =
    ...
    let elts_with_empty_lines, rest =
      nodes |> List.partition (fun node -> is_empty_row node m dm)
    in
    ...
    | Node (n, xs) ->
        ...
        then
          ...
          let new_gopti, dotdotdot_entry =
            Graph_code_opti.adjust_graph_pack_some_children_under_dotdotdot n
              to_pack !gopti
          in
          gopti := new_gopti;
          Node
  ...
  let rec aux tree =
    ...
    | Node (n, xs) ->
        if null xs then Node (n, [])
          ...
          in
          let children_nodes = xs |> List.map (function Node (n2, _) -> n2) in
          ...
          let nodes_reordered =
            optional_manual_reordering n nodes_reordered constraints_opt
          in
          ...
          Node
            ( n,
              ...
              |> List.map (fun n2 ->
                     let xs =
                       try
                         Hashtbl.find h_children_of_children_nodes n2
  ...
  let rec aux xs =
    ...
    | x :: xs -> (
        ...
        | Focus (n2, _style) ->
            ...
            if not (List.mem n children) then Expand n :: x :: xs
            else x :: aux xs)
  in
  aux xs
...
let config_of_path (path : config_path) gopti =
  ...
  |> List.fold_left
       (fun (config, gopti) e ->
         match e with
         | Expand node -> (expand_node_opti node config gopti, gopti)
         | Focus (node, kind) ->
