...
let build g =
  ...
  |> G.iter_nodes (fun n ->
         ...
         try
           ...
           add
             (At
                ( entity_of_str str,
                  nodeinfo.G.pos.Parse_info.pos.Pos.file,
                  nodeinfo.G.pos.Parse_info.pos.Pos.line ));
           let t =
  ...
  |> G.iter_use_edges (fun n1 n2 ->
          ...
          *)
         | (s1, E.Class), (s2, E.Class) -> add (Extends (s1, s2))
         | ( (s1, (E.Function | E.Method)),
             (s2, (E.Function | E.Prototype | E.Method)) ) ->
         ...
         | ( (s1, (E.Function | E.Method | E.Global | E.Constant)),
             ...
             match info with
             | None -> add (UseData (entity_of_str s1, entity_of_str s2, None))
             | Some { G.read = false; G.write = false } ->
                 failwith
                   (spf "use access with neither read or write: %s -> %s" s1 s2)
             | Some info ->
                 if info.G.read then
                   add
                     (UseData (entity_of_str s1, entity_of_str s2, Some false));
                 if info.G.write then
                   add (UseData (entity_of_str s1, entity_of_str s2, Some true))
             )
         | _ -> ());
...
let hook_use_edge_for_prolog ctx in_assign (src, dst) g _loc =
  ...
  | E.Field ->
      ...
      in
      let newinfo =
        if in_assign then { info with G.write = true }
