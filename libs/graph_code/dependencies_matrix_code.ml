 ...
 *)
and tree = Node of Graph_code.node * tree list
and config = tree

let basic_config g =
  Node (G.root, Graph_code.children G.root g |> List.map (fun n -> Node (n, [])))

let basic_config_opti gopti =
  Node
    ( G.root,
      Graph_code_opti.children G.root gopti |> List.map (fun n -> Node (n, []))
    )
...
and deps_style = DepsIn | DepsOut | DepsInOut
...
let string_of_config_path_elem = function
  | Expand n -> spf "Expand(%s)" (G.string_of_node n)
  | Focus (n, style) ->
      spf "Focus%s(%s)"
        (match style with
        | DepsIn -> "<-"
...
let parents_of_indexes dm =
  ...
  let rec aux acc tree =
    ...
    | Node (_, []) ->
        ...
        incr i
    (* a node *)
    | Node (n, xs) -> xs |> List.iter (aux (n :: acc))
  in
  aux [] dm.config;
  arr
...
let distance_entity (i, j) arr =
  ...
  let rec aux xs ys =
    ...
    | [], _ -> 0
    | x :: xs, y :: ys -> if x =*= y then aux xs ys else 1
  in
  aux xs ys
...
let is_internal_helper j dm =
  ...
  for i = 0 to Array.length mat - 1 do
    if mat.(i).(j) > 0 && i <> j && distance_entity (j, i) arr > 0 then
      has_users_outside_parent := true
  done;
...
let score_upper_triangle dm exclude_nodes =
  let score = ref 0 in
  let exclude_idx =
    exclude_nodes |> List.map (fun n -> hashtbl_find_node dm.name_to_i n)
  in

  for i = 0 to Array.length dm.matrix - 1 do
    for j = i + 1 to Array.length dm.matrix - 1 do
      if List.mem i exclude_idx || List.mem j exclude_idx then ()
