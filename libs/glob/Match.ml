...
type compiled_pattern = { source : loc; re : Regex.t }
...
let quote_char buf c = add buf (Regex.quote (String.make 1 c))
...
let run matcher path =
  let res = Regex.pmatch_noerr ~rex:matcher.re path in
  if !debug then
