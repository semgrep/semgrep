...
let smaller t1 t2 = if compare t1.Tok.bytepos t2.Tok.bytepos < 0 then t1 else t2
let larger t1 t2 = if compare t1.Tok.bytepos t2.Tok.bytepos > 0 then t1 else t2
...
class ['self] any_range_visitor =
  let pos_within pos (t1', t2') =
    ...
    pos >= t1'.Tok.bytepos && pos <= t2'_charpos
  in
  let range_within (t1, t2) (t1', t2') =
    let _, _, t2_charpos = Tok.end_pos_of_loc t2 in
    let _, _, t2'_charpos = Tok.end_pos_of_loc t2' in
    t1.Tok.bytepos >= t1'.Tok.bytepos && t2_charpos <= t2'_charpos
  in
