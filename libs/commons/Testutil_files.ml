...
let rec write root files =
  List.iter (write_one root) files
...
let read root =
  let rec read path =
    ...
    | S_REG ->
        File (name, Common.read_file (Fpath.to_string path))
    | S_LNK ->
        Symlink (name, Unix.readlink (Fpath.to_string path))
    | _other ->
        failwith ("Testutil_files.read: unsupported file type: "
  ...
  | _other ->
      failwith ("Testutil_files.read: root must be a directory: "
...
let mkdir ?(root = Sys.getcwd () |> Fpath.v) path =
  ...
  if not (Sys.file_exists root_s) then
    failwith ("Testutil_files.mkdir: root folder doesn't exist: "
               ^ root_s);
  mkdir path
...
let () = Testutil.test "Testutil_files" (fun () ->
