  ...
  steps: semgrep.github_bot.get_token_steps + [
    ...
    },
    {
      ...
      run: |||
        ...
        # Add a `Co-authored-by` byline with the name and email of the author of
        # each commit in the PR, as reported by Git. This is added by GitHub
        # automatically when the PR is merged, but it ends up below the line
        # that we use to delineate sensitive content that should not be synced
        # out. Duplicating it here seems safer than complicating the logic to
        # remove sensitive content.
        git log "$MERGE_BASE..HEAD" --format='format:%aN <%aE>' | sort | uniq | sed -e 's/^/Co-authored-by: /' >> /tmp/pr_body
