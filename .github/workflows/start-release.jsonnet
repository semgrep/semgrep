...
local notify_success_job = {
  ...
  steps: [
    ...
    },
    {
      ...
      run: semgrep.slack.curl_notify_start_release(version, 'Release Validation has succeeded! Please review the PRs in semgrep-app, semgrep-rpc, and semgrep-action that were generated by this workflow.'),
    },
  ],
};
...
// ----------------------------------------------------------------------------
{
  ...
  jobs: {
    ...
    'notify-failure':
      semgrep.slack.notify_failure_start_release_job(
        version,
        'Release Validation has failed. Please see https://github.com/${{github.repository}}/actions/runs/${{github.run_id}} for more details!'
      ) + {
