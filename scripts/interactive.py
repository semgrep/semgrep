...
def main(language: str, directory: str):
    ...
    for i, expr in enumerate(exprs):
        directories = cached_query(
            expr, [os.path.join(directory, d) for d in directories], i == len(exprs) - 1, start_time)


def cached_query(expr: str, paths: List[str], show: bool, start_time: float) -> List[str]:
