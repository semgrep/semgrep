...
let ( >> ) o fld =
  ...
  | J.Object xs ->
      ...
      o
  | _ ->
      failwith (spf "could not find %s" fld)
...
let parse_card j =
  ...
  | J.Object
      ...
      then None
        ...
        | J.Null ->
            None
        | J.Object [ ("__typename", J.String "PullRequest") ] ->
            None
        | _ ->
            error j "not a card" )
  | _ ->
...
let get_cards_of_column colname board =
  ...
  | Some x ->
      x.cards
  | None ->
      failwith (spf "could not find column %s in the board" colname)
