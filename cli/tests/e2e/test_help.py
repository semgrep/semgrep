...
@pytest.mark.parametrize("help_flag", ["--help", "-h"])
@pytest.mark.osempass
def test_scan_help_text(tmp_path, snapshot, help_flag):
    """
    Test to make sure scan help text doesn't change unintentionally
    """
    runner = SemgrepRunner(
        env={"SEMGREP_SETTINGS_FILE": str(tmp_path / ".settings.yaml")}
    )
    result = runner.invoke(cli, ["scan", help_flag], env={})
    snapshot.assert_match(result.output, "help.txt")
