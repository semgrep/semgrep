...
def shouldafound(
    ...
    if not yes:
        click.echo("This information, including the following lines of code, will be sent to Semgrep.dev:", err=True)
        click.echo(json.dumps(data, indent=2), err=True)
        if not click.confirm("Please confirm the code does not contain any sensitive information, and you wish to send!", err=True):
