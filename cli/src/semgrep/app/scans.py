...
class ScanHandler:
    ...
    def report_findings(
        ...
        # load.
        try_until = datetime.now(UTC).replace(tzinfo=None) + timedelta(minutes=30)
        slow_down_after = datetime.now(UTC).replace(tzinfo=None) + timedelta(minutes=2)
        ...
        while True:
            ...
            if datetime.now(UTC).replace(tzinfo=None) > try_until:
            ...
            sleep(5 if datetime.now(UTC).replace(tzinfo=None) < slow_down_after else 30)
