...
def add(name: str, elapsed: float, count: int = 1) -> None:
    e = entries[name]
...
# the OCaml way: measure("foo", lambda: ...)
def measure(name: str, func: Callable[[], T]) -> T:
...
def simple_profiling(func: Callable[P, R]) -> Callable[P, R]:
    ...
    def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
        ...
        try:
            ...
            t2 = time()
            add(f"{func.__module__}.{func.__qualname__}", t2 - t1)

    return wrapper


def export() -> list[out.ProfilingEntry]:
...
def import_(entries: list[out.ProfilingEntry]) -> None:
    """Import profiling data from semgrep-core"""
    for e in entries:
        add(name=e.name, elapsed=e.total_time, count=e.count)
