...
not_whitespace = test_char(lambda c:c not in whitespace_chars,"not whitespace")


@generate
def p_key():
    yield string("\"").optional()
    key = yield not_whitespace.until(string("\"") | string(":") | string(",") | string(" ")).concat()
    yield string("\"").optional()
    return key
...
@generate
def p_value():
    return (yield not_whitespace.many().concat())
...
def blob():
    keys = yield p_multikey
    yield string(":\n")
    foo = yield (p_key_value | blob).sep_by(string("\n"))
    return (keys,foo)
...
# @generate
# def p_package():
#     at_prefix = "@" if (yield string("@").optional()) else ""
#     package = yield not_whitespace.until(string("@")).concat()
#     return at_prefix + package

# @generate
# def p_version():
#     return (yield not_whitespace.until(string(",") | string("\"") | string(":")).concat())

# @generate
# def p_source():
#     yield string("\"").optional()
#     package = yield p_package
#     yield string("@")
#     version = yield p_version
#     yield string("\"").optional()
#     return (package,version)

# @generate
# def p_sources():
#     sources = yield p_source.sep_by(string(", "),min=1)
#     yield string(":")

#     # Safe because of the min=1 arg
#     return sources[0][0],sources

# @generate
# def p_field(depth):
#     yield string(" " * depth)
#     field = yield not_whitespace.many().concat()
#     yield string(" ")
#     value = yield not_whitespace.many().concat()
#     return (field,value.strip("\""))

# # We don't do anything with these, don't bother parsing them
# @generate
# def p_sub_dependencies():
#     yield string("dependencies:\n")
#     yield p_field.sep_by(string("\n"))
#     return None

# @generate
# def p_fields():
#     fields = yield p_field.sep_by(string("\n"))
#     yield string("\n")
#     yield p_sub_dependencies.optional()
#     return dict(fields)

# @generate
# def p_dependency():
#     package, _srcs = yield p_sources
#     yield string("\n")
#     line,_col = yield line_info
#     fields = yield p_fields
...
# @generate
# def p_dependencies():
#     return (yield p_dependency.sep_by(string("\n\n")))
