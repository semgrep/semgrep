    ...
    List.filter_map
      ...
      *)
              ...
              | (Minified | Binary), false ->
                  ...
                  then
                    eprintf
                      "ignoring gibberish file: %s\n%!"
                      (Src_file.source_string partial_doc_src) ;
                  None
              | _ ->
                  incr num_analyzed ;
                  let doc_src =
                    if Src_file.length partial_doc_src < peek_length
                    then
...
let color_conv =
  let parser when_use_color =
    ...
    | "never" ->
        Ok Never
    | s ->
        Error (`Msg ("Invalid argument for --color option: " ^ s))
  in
...
let output_format_conv =
  let parser s =
    ...
    | "semgrep" ->
        Ok Semgrep
    | s ->
        Error (`Msg ("Invalid output format: " ^ s))
  in
