...
let rec full_match ~dots env last_loc pat =
  ...
  | Dots _ :: pat ->
      full_match ~dots env last_loc pat
  | [ End ] ->
      Complete (env, last_loc)
  | End :: _ ->
      assert false
...
let case_insensitive_equal a b =
  ...
  try
    for i = 0 to len - 1 do
      if Char.lowercase_ascii a.[i] <> Char.lowercase_ascii b.[i]
      then raise Exit
    done ;
    true
  with
...
let rec extend_last_loc ~max_line_num last_loc (doc : Doc_AST.node list) =
  ...
  | Atom (loc, _) :: doc ->
      if loc_lnum loc <= max_line_num
      then extend_last_loc ~max_line_num loc doc
      else Some last_loc
...
let doc_matches_dots ~dots last_loc doc =
  ...
  | Some max_line_num, doc ->
      if loc_lnum last_loc <= max_line_num
      then extend_last_loc ~max_line_num last_loc doc
      else None
...
let within_ellipsis_range ~dots loc =
  match dots with
  | None ->
      true (* <-- difference with 'is_skippable' *)
  | Some max_line_num ->
      loc_lnum loc <= max_line_num
...
let rec match_
  ...
  | List pat1 :: pat2, doc ->
    ...
    | [] ->
        ...
        assert (pat1 <> []) ;
        if pat_matches_empty_doc pat1 && pat_matches_empty_doc pat2
        then Complete (env, last_loc)
        else Fail
    | List doc1 :: doc2 ->
    ...
    | Atom (loc, _) :: doc_tail ->
        ...
        assert (pat1 <> []) ;
        if is_skippable_doc_atom ~dots loc
        then match_ conf ~dots env last_loc pat doc_tail cont
        else if pat_matches_empty_doc pat1
        then match_ conf ~dots env last_loc pat2 doc cont
        else Fail )
  | Dots _ :: pat_tail, doc ->
  ...
  | Atom (_, p) :: pat_tail, doc ->
    ...
    | doc_head :: doc_tail ->
      ...
      | Atom (loc, d) ->
          if not (within_ellipsis_range ~dots loc)
          then Fail
          else
            let match_result =
              match (p, d) with
              | Metavar name, Word value ->
                ...
                | Some (_loc0, value0) ->
                    ...
                    if String.equal value value0
                    then match_ conf ~dots:None env loc pat_tail doc_tail cont
                    else Fail )
