...
let append_block_to_accumulator
    open_punct close_punct open_loc close_loc block_nodes acc =
...
let rec parse_line pending_braces acc (tokens : Lexer.token list) :
  ...
  | Close_paren close_loc :: tokens ->
    ...
    | (Bracket | Curly) :: _ ->
        None
    | [] ->
        parse_line pending_braces (Atom (close_loc, close_paren) :: acc) tokens
    )
  ...
  | Close_curly close_loc :: tokens ->
    ...
    | (Paren | Bracket) :: _pending_braces ->
        None
    | [] ->
        parse_line pending_braces (Atom (close_loc, close_curly) :: acc) tokens
    )
  ...
  List.map
    (fun (token : Lexer.token) ->
      ...
      | Open_paren loc ->
          Atom (loc, open_paren)
      | Close_paren loc ->
          Atom (loc, close_paren)
      | Open_bracket loc ->
          Atom (loc, open_bracket)
      | Close_bracket loc ->
          Atom (loc, close_bracket)
      | Open_curly loc ->
          Atom (loc, open_curly)
      | Close_curly loc ->
          Atom (loc, close_curly))
    tokens
