...
class TargetManager:
    ...
    @staticmethod
    def filter_by_size(arr: Set[Path], max_target_size: int) -> Set[Path]:
        """
        Return all the files whose size doesn't exceed the limit.

        If max_target_size is zero or negative, all paths are returned.
        ...
        if max_target_size <= 0:
            ...
            return set(
                ...
                and os.path.getsize(path) <= max_target_size
            )
    ...
    def filtered_files(self, lang: Language) -> Set[Path]:
        ...
        targets = self.filter_by_size(targets, self.max_target_size)
