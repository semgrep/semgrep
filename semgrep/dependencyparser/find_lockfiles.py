...
class DependencyTrie:
    ...
    def find_dependencies(self,path: Path) -> Optional[Dict[str,List[LockfileDependency]]]:
        curr = self.root
...
def make_dependency_trie(target: Path) -> DependencyTrie:
    ...
    def go(current: Path, seen_paths: FrozenSet[Path]) -> DependencyTrie:
        if current.is_file() and current.name.lower() in TARGET_LOCKFILE_FILENAMES:
            dep_trie.insert(current,list(parse_lockfile_str(current.read_text(),current)))
        else:
            for entry in os.scandir(current):
                ...
                if entry.is_dir() and (
                    not (resolved_path in seen_paths)
                ):
                ...
                if entry.is_file() and entry.name.lower() in TARGET_LOCKFILE_FILENAMES:
                    dep_trie.insert(current,parse_lockfile_str(current.read_text(),current))
